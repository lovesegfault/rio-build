...
- **Devshell cargo builds go through [kache](https://github.com/kunobi-ninja/kache)** (`RUSTC_WRAPPER`, local-only): compiled artifacts are content-addressed and shared across worktrees via `~/.cache/kache`. Nix builds/CI are hermetic and unaffected. Opt out with `KACHE_DISABLED=1` in `.env.local` (free — the wrapper stays in place, so no `target/` invalidation). Debug unexpected misses with `kache why-miss <crate>`. Edge case: sqlx reads `.sqlx/` outside rustc's dep-info, so if `cargo xtask regen sqlx` changes `.sqlx/` with no `.rs` change and something looks stale, bust it with `kache purge --crate-name <crate>`.
