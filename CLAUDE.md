...
- **Devshell cargo builds go through [kache](https://github.com/kunobi-ninja/kache)** (`RUSTC_WRAPPER`): compiled artifacts are content-addressed and shared across worktrees via `~/.cache/rio-build/kache/<env-salt>/`. Local-only is enforced *inside* the wrapper script (allowlisted `KACHE_*` env + pinned config — survives any spawner, including xtask's `.env.local` reload), and the store epoch is salted by the toolchain/linker closure so restored binaries never reference GC'd store paths. `RUSTC_WRAPPER` is the wrapper's own store path — per-shell and immutable, so sibling worktrees on different revs cannot cross-contaminate epochs. Nix builds/CI are hermetic and unaffected. Opt out with `KACHE_DISABLED=1` in `.env.local` (free — no rebuild penalty; on disabled rebuilds the wrapper sweeps each compile's own restores — anchored by the unit's `-C extra-filename` — out of its `--out-dir`, so if one still errors with "output file … is not writeable" either the sweep took its conservative arm (non-cargo-shaped argv: a missing or non-16-hex extra-filename deliberately skips nlink-1 reflink restores) or no bypass fired — check stderr for the wrapper's "not a recognized compiler" warning before concluding the spawner bypassed the wrapper entirely (bare cargo without `RUSTC_WRAPPER`); see the never-chmod note in `.env.local.example`. Staying disabled for 14+ idle days lets the entry-time GC prune the unstamped default epoch — re-enabling is then a one-time cold rebuild; mitigation in `.env.local.example`); relocate the store with `RIO_KACHE_CACHE_DIR` and resize it with `RIO_KACHE_MAX_SIZE` (kache's own `KACHE_CACHE_DIR`/`KACHE_MAX_SIZE` are deliberately ignored so foreign kache configs can't steer our store). Debug unexpected misses with `kache why-miss <crate>`.
  - **Out-of-band macro inputs are tracked**: `.sqlx/` and `migrations/` are hashed into rustc env-deps by `rio-buildhash` build scripts, so both cargo and kache re-key when they change without `.rs` edits. If you add a macro that reads files invisible to dep-info, add a tracker fn to `rio-buildhash` mirroring `track_migrations`, then give the consuming crate the two-line `build.rs` + `const _: &str = env!(…)` pattern.
  - **Debugger paths**: cached compiles remap source prefixes to sentinels. Map back with gdb `set substitute-path <WORKSPACE> .` / lldb `settings set target.source-map <WORKSPACE> .`, or force a real-path rebuild with `cargo clean -p <crate> && KACHE_DISABLED=1 cargo build -p <crate>` (the `clean` matters — a warm target recompiles nothing, so the remapped debuginfo would just stay).
...
3. Run `cargo xtask regen cargo-json` — the `fuzz-<target>` run derivations are derived from `fuzz/<ws>/Cargo.json` `crateBin` (no hand list in `nix/fuzz.nix`; target names must be unique across fuzz workspaces)
