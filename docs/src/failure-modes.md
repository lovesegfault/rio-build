...
| **Scheduler** | No new builds accepted; `SubmitBuild` returns `UNAVAILABLE` | Workers' `BuildExecution` streams disconnect; workers go idle. Gateways return errors to clients. | New leader acquires the Kubernetes Lease. **State recovery is Phase 3b-deferred** --- the new leader starts with an empty in-memory DAG; in-flight builds are lost (clients hang). Workers and gateways reconnect streams. |
...
- Optional Phase 3b hardening: add a `scheduler_meta` row with a generation-guard WHERE clause for strict fencing
...
- **Phase 3b deferral:** FUSE read timeout and worker-side circuit breaker on FUSE fetches are not yet implemented. Currently reads block indefinitely on store degradation; builds may stall until the daemon timeout (`daemon_timeout_secs`, default 2h) fires
