...
3. **Cache miss, `size > STREAM_THRESHOLD`**: streaming path (§7). `StatBlob(file_digest, send_chunks=true)` (`r[store.castore.blob-stat]`) returns the `ChunkMeta[]` list plus `first_chunk_skip`/`last_chunk_take` slice offsets (the file's bytes may begin/end mid-chunk when the resolved manifest was legacy whole-NAR-chunked); the fill task sources each chunk from `/var/rio/chunks/` (mountd-owned, RO) first; misses go to `GetChunks` and are written into `.partial` (sliced for first/last) + `staging/chunks/` (whole), with digests batched into `PromoteChunks` (assembly proceeds from own staging; the batch is for other builds). Concurrent builds **share progress at chunk granularity** — no leader/follower, no sentinel. On completion: whole-file verify, rename `.partial → <hex>`, `Promote{digest}`. Next `open` hits (1).
...
`open()` spawns a background fill task and returns `FOPEN_KEEP_CACHE` after the **first chunk** is verified and written (~10 ms). The fill is sequential; `read(off, len)` for a range not yet filled blocks — bounded by the fill's size-aware budget — until the high-water mark covers it (re-prioritizing the requested chunk range to the head of the fetch queue is a possible future optimization, not part of the landed fill); `mmap(MAP_PRIVATE)` page faults route through the same `read` path. `KEEP_CACHE` does not suppress cold-page upcalls, only cross-open invalidation, so no mode-flip is needed within the streaming window. Whole-file digest verification runs at fill-complete and gates the `Promote{digest}` request to mountd.
...
One directory bucket is provisioned **per availability zone**; each store pod is configured with at most the bucket for its own AZ. Helm renders either a single explicit bucket (`store.chunkBackend.expressBucket` — single-AZ or zone-pinned deployments) or, for multi-AZ, the full zone-keyed map (`store.chunkBackend.expressBucketByZone`, from the terraform `express_bucket_by_zone` output) as one JSON env var plus the pod's own zone via the downward API (`RIO_NODE_ZONE` ← the pod's `topology.kubernetes.io/zone` label, copied from the node at binding by KEP-4742's `PodTopologyLabelsAdmission`, beta in k8s 1.35); the store selects `map[zone]` at startup (P0554). A replica with no selected bucket (AZ without Express, zone label absent, empty map) runs with `local=None` and logs which piece was missing. A newly-scaled replica is warm immediately; S3 standard GET is paid once per chunk per AZ. Any single Express bucket (or its AZ) becoming unavailable degrades that AZ's replicas to direct S3-standard reads — slower, not down — and leaves other AZs unaffected. The cache tier is toggled by a single helm value (`store.chunkBackend.kind`), and flipping it back to `s3` is instant and lossless.
...
When enabled, the store writes the NAR — zstd-compressed by default — to S3-standard at `nar/{FileHash}.nar.zst`, where `FileHash = sha256(compressed bytes)`. Who writes it depends on the ingest path: the buffered upload RPCs (`PutPath`/`PutPathBatch`, the gateway surface) compress the verified NAR they already hold in RAM and publish it synchronously after commit; paths committed via `PutPathChunked` (builder uploads) or upstream substitution are **not** written inline — the compat reconciler finds them via `narinfo.compat_file_hash IS NULL`, reassembles the NAR from chunks, and publishes the pair off the upload hot path shortly after.

When enabled, the same writer emits a stock-Nix-format `.narinfo` to S3-standard at `{StorePathHash}.narinfo` with `FileHash`/`FileSize` populated — the compressed artifact exists, so the fields are computable. Signatures are the same ones stored in PG (signed at commit time over the standard fingerprint), copied verbatim.

Inline compat writes happen **after** the PostgreSQL transaction commits, **synchronously** within the upload handler. A crash between PG-commit and compat-write leaves a PG row with no S3 narinfo — recoverable (the reconciler re-emits from chunks, exactly as it does for chunked/substituted paths). A compat-write failure is logged and metered but does **not** roll back the PG commit or fail the upload RPC: the path is durably stored and rio-servable; only the stock-Nix fallback is missing for that path until the reconciler retries it. `write_mode = "async"` (background queue, lower `PutPath` latency, larger reconciler backlog on crash) is a documented future option, not implemented in the first pass.
...
`rio-mountd` holds, per build, the UDS connection, a dup of that build's `/dev/fuse` fd, and the staging dirfds; ~250 LoC. Requests carry a `seq: u32` echoed in replies (so `spawn_blocking` `Promote`/`PromoteChunks` can reply out-of-order); errors are typed (`DigestMismatch`/`NotRegular`/`TooLarge`/`BadBuildId`/`AlreadyMounted`/`Unauthorized` are build-fatal, `Retryable(..)` is infra-retry). Admission is two-mode (§P0559, credential scheme superseded by the [mount-admission credentials ADR](./022-mountd-admission-credentials.md) under §P0590): without any mountd verifier configured the UDS socket is mode 0660 group `rio-builder` and mountd checks `SO_PEERCRED.gid`, rejecting others — the standalone/systemd posture, where the builder service simply joins the `rio-builder` group. Both the socket-inode DAC and `SO_PEERCRED` observe the peer's *init-namespace* gid, and a `hostUsers: false` builder pod (`r[sec.pod.host-users-false]`) has every gid kubelet-remapped into a per-pod range — no static in-pod gid maps to the host's `rio-builder` gid, so the gid gate admits no userns-confined builder. Production k8s therefore configures the asymmetric mountd credential scheme: the scheduler — and only the scheduler — holds the Ed25519 signing key (`RIO_MOUNTD_SIGNING_KEY_PATH`) and at dispatch signs a per-build `rmt2.<claims>.<signature>` token whose claims carry `aud:"rio-mountd"`, the `build_id`, the audit `tenant`, issued/expiry (same TTL as the assignment token, so the reconnect path's mid-build re-`Mount{}` stays within validity), and the kube `spec.nodeName` of the node the build was placed on, resolved from the controller-attested binding (executor/pod-name keyed, with the executor's own register-time `RIO_NODE_NAME` report as fallback; `mountd_node_binding = require|prefer` decides whether an unresolvable placement defers the dispatch or mints unbound). The token rides `WorkAssignment.mountd_token`; the builder presents it opaquely in `Mount{build_id, token}`; every mountd DaemonSet pod holds only the *public* trust roots (`--token-pubkey-path` / `RIO_MOUNTD_PUBKEY_PATH`, multiple lines = rotation overlap) plus its own node name (`--node-name` / `RIO_MOUNTD_NODE_NAME`, downward API) and verifies offline. The never-provisioned symmetric `mountdHmac` arm (`--token-key-path`) remains only as a contingency overlap and is deleted in the ADR's final phase. Either way the broker remains a strictly smaller privileged surface than the pre-ADR-022 model, where the builder pod itself held `CAP_SYS_ADMIN`: the brokered `BACKING_OPEN` registers an fd the builder already holds (conn-scoped, depth-0-only), and `Promote` is the integrity boundary for the shared cache.

r[builder.mountd.token-admission+2]

When any mountd token verifier is configured (token mode), mountd MUST admit a connection's `Mount{}` either when the peer's `SO_PEERCRED.gid` equals the allowed gid or when the request carries a token that verifies — for an `rmt2` token: an Ed25519 signature valid under one of the configured trust roots, unexpired, audience `rio-mountd`, `claims.build_id` equal to the requested `build_id`, and (when mountd is configured with its own node name) `claims.node` equal to that node name; for a legacy two-segment token, only while an HMAC key is still configured: HMAC-valid, unexpired, audience `rio-mountd`, and matching `build_id`; a `Mount{}` satisfying neither arm MUST be rejected (`Unauthorized`) and the connection closed; in token mode the socket inode MUST be world-connectable (mode 0666), and without any configured verifier mountd MUST keep the gid-only admission and the 0660 root:allowedGid socket posture unchanged.

Rationale and consequences: the gid arm keeps the standalone/systemd worker working with zero extra config; the token arm is how a `hostUsers: false` executor pod — which cannot present any host gid — proves it is a scheduler-dispatched build rather than an arbitrary local process. The 0666 mode exists because the socket DAC cannot discriminate the peers token mode is for (every pod uid/gid is kubelet-remapped), so admission moves entirely in-protocol and the daemon keeps the pre-auth surface one frame deep: a provisionally-admitted (non-gid) connection must authenticate with its FIRST frame, anything else — another request, an undecodable frame, a bad token — is answered and closed, and an idle unauthenticated connection is closed after a bounded window; frames stay capped at `MAX_FRAME_BYTES` either way. Binding `claims.build_id` to the `Mount` argument means a leaked token cannot squat another build's id, and the node claim (below) means it cannot squat that id on any other node either. Verification never runs serde on unauthenticated bytes (signature first, claims parse only after a trust root vouched), the wire reply stays one opaque `Unauthorized` (reject reasons — including the new `node-mismatch`/`node-missing` — go to logs and the reject counter only), and the verifying trust-root name is logged on success so rotation is observable via the `rio_mountd_trust_root` gauge. A possible future hardening, deliberately not required here, is an additional `SO_PEERCRED.uid >= <userns floor>` check in production values so low-uid host daemons cannot even reach the token path.

r[builder.mountd.token-key-separate+2]

The mountd Mount-admission credential MUST be its own key family, separate from every other credential in the system: the Ed25519 mountd signing keypair (and the legacy mountd HMAC key while it exists) MUST be distinct from the store-facing assignment-HMAC key, the service-token key, the tenant-JWT keypair, and the narinfo signing keypair; mountd key names MUST carry the `rio-mountd-` prefix and loaders MUST reject names without it; mountd MUST NOT accept assignment, executor, service, or tenant-JWT tokens for Mount admission, and the scheduler MUST NOT mint Mount-admission tokens with any of those other keys.

The trust material is mounted into the rio-mountd DaemonSet on every builder/fetcher node; if it were (or shared bytes with) the assignment key, a single node compromise would yield a credential the store trusts for uploads and tenant-scoped castore reads. Independent defences keep the families apart even under operational mix-ups: separate Secrets, divergent claims shapes (`deny_unknown_fields` on both sides), the explicit `aud: "rio-mountd"` discriminator, the `rmt2` envelope tag covered by the signature, and the `rio-mountd-` key-name prefix that stops a narinfo keypair (same `name:base64` file encoding) from being cross-wired into the mountd trust chain.

r[builder.mountd.token-no-node-mint]

Material resident on builder/fetcher nodes MUST NOT be able to mint Mount-admission credentials: the `rmt2` signing key is held only by the control plane (the scheduler), and the only mountd-side trust material is Ed25519 public keys.

This is the structural property the §P0590 redesign exists for — under the superseded symmetric scheme every node's verifier *was* a signer, so rooting one node minted Mount admission for any build on every node. With public-only trust roots, node compromise yields nothing to rotate: the attacker gains the trust roots (worthless for minting) and the expiring tokens of builds already placed on that node, and no credential any other node or service accepts. The legacy HMAC arm violates this property by construction, which is why production never configures it and the final ADR phase deletes it.

r[builder.mountd.token-node-scoped]

A scheduler-minted `rmt2` Mount-admission token MUST name the node it is valid on (`claims.node`, the kube `spec.nodeName` resolved from controller-attested placement at dispatch), except when minted unbound under the `mountd_node_binding = prefer` escape hatch for an unresolvable placement — in which case node-checking mountds MUST reject it (`node-missing`); a mountd configured with its own node name MUST reject a token that names a different node (`node-mismatch`) or carries no node claim (`node-missing`); a mountd with no node name configured MUST skip the node check and admission MUST otherwise be unchanged.

This closes the cross-node squat: a token stolen on node A carries `node = A` under the scheduler's signature, so node B's mountd rejects it, and the thief cannot rewrite the claim without breaking the signature. The scheduler resolves the node at the existing mint site as controller-attested binding (pod-name keyed) → intent-keyed binding → the executor's own register-time report (which can only mis-scope that executor's own token — self-defeating), and the `mountd_node_binding = require|prefer` knob (default `require`) decides whether an unresolvable placement defers the dispatch one pass or mints an unbound token that strict mountds reject. The remaining residual is a token stolen from node A presented on a future node that reuses A's exact name within the TTL — strictly narrower than the any-node squat, closable later via the reserved `node_fp` claim. The standalone/gid-990 path is unaffected (no node name configured ⇒ no node check).

The per-build hardening invariants:
...
| rio-mountd crash | existing mounts unaffected; new build-starts block on UDS connect | DaemonSet restarts; start-up orphan scan reaps stale staging trees (plus pre-cutover `/var/rio/castore` leftovers) but skips dirs whose builder still holds the flock'd `.rio-live` sentinel (taken at Mount time; the kernel drops the lock with the process) — live builder-ns mounts need no recovery. In-flight builds whose mountd RPCs hit the dead connection re-dial and re-`Mount{build_id}` against the new daemon (`r[builder.fs.mountd-reconnect]`); a whole-file miss whose Promote still cannot land is served from its verified staged copy instead of EIO (`r[builder.fs.promote-degrade-staged]`) |
...
| castore-FUSE | `builder.fs.digest-fuse-open` · `builder.fs.passthrough-on-hit` · `builder.fs.passthrough-stack-depth` · `builder.fs.file-digest-integrity` · `builder.fs.fetch-circuit` · `builder.fs.shared-backing-cache` · `builder.fs.node-digest-cache` · `builder.fs.node-chunk-cache` · `builder.fs.streaming-open` · `builder.fs.streaming-open-threshold` · `builder.fs.mountd-reconnect` · `builder.fs.promote-degrade-staged` |
| Privilege | `builder.mountd.fuse-handoff` · `builder.mountd.backing-broker` · `builder.mountd.promote-verified` · `builder.mountd.promote-bounded-copy` · `builder.mountd.orphan-scan` · `builder.mountd.concurrency` · `builder.mountd.build-id-validated` · `builder.mountd.build-id-unique` · `builder.mountd.uid-bound` · `builder.mountd.one-mount` · `builder.mountd.staging-quota` · `builder.mountd.token-admission` · `builder.mountd.token-key-separate` · `builder.mountd.token-no-node-mint` · `builder.mountd.token-node-scoped` · `sec.boundary.mountd` |
