...
One directory bucket is provisioned **per availability zone**; each store pod is configured with at most the bucket for its own AZ. Helm currently renders a single explicit bucket (`store.chunkBackend.expressBucket`), which is correct for single-AZ or zone-pinned deployments; multi-AZ deployments carry the full per-AZ map (`store.chunkBackend.expressBucketByAzId`, from the terraform output) and the per-pod startup resolution that selects from it is a pending main-line item (see the implementation plan's P0554 handoff note) — until it lands, an unselected replica runs with `local=None`. A newly-scaled replica is warm immediately; S3 standard GET is paid once per chunk per AZ. Any single Express bucket (or its AZ) becoming unavailable degrades that AZ's replicas to direct S3-standard reads — slower, not down — and leaves other AZs unaffected. The cache tier is toggled by a single helm value (`store.chunkBackend.kind`), and flipping it back to `s3` is instant and lossless.
