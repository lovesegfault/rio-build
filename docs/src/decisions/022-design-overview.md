...
A scheduler-minted `rmt2` Mount-admission token MUST name the node it is valid on (`claims.node`, the kube `spec.nodeName` resolved from controller-attested placement at dispatch), and a mountd configured with its own node name MUST reject a token that names a different node (`node-mismatch`) or carries no node claim (`node-missing`); a mountd with no node name configured MUST skip the node check and admission MUST otherwise be unchanged.
