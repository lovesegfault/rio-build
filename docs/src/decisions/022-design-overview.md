...
| **Structural per-file and per-subtree dedup** | Inode numbers are allocated sequentially at mount, deduped by content class (file: digest+exec bit, symlink: target). Two store paths containing byte-identical files share one FUSE inode, one fetch, one SSD copy, one page-cache copy. Two paths sharing a subtree share one dcache subtree. No explicit dedup pass. |
...
r[store.index.authoritative]

r[store.index.nar-ls-streaming]

The index is computed during ingest from the NAR stream (`nar_ls` + per-file blake3 in a single forward `Read` pass — no `Seek`, bounded memory regardless of NAR size) and MUST be persisted in the same transaction that makes the manifest `complete`, so a path is never visible without its index. The index is **authoritative from that moment**: the NAR byte stream is not persisted (§6 stores per-file content chunks and regenerates the framing from the Directory DAG on read), so the index cannot be recomputed after the fact and a `complete` path with a missing index row is data loss, not a cache miss. There is no deferred-indexing path and no background indexer.
...
The castore-FUSE handler serves the full tree from the in-heap Directory DAG (cold `lookup`/`readdir`/`readlink`, §4) and brokers data on `open()`. Its state is a `HashMap<u64, Node>` keyed by inode (sequentially allocated at mount; deduped by content for files/symlinks, unique per path for directories), populated at mount from one `GetDirectory(recursive=true)` call seeded with all closure `dir_digest` roots (`r[builder.fs.castore-dag-source]`). `lookup(parent_ino, name)` probes the parent's precomputed child index and returns the child's inode. Any name outside the prefetched DAG → `ENOENT` (declared-input allowlist).
...
Real-world builds touch 0.3–33% of giant `.so`/`.a` files in scattered or bimodal patterns; streaming open means the untouched 67–99.7% is fetched in the background (or not at all if the build finishes first), and subsequent opens of the same file on that node are passthrough once the fill-window handles have drained (`builder.fs.open-iomode-compatible` — the kernel refuses to mix caching and passthrough opens on one inode, so until then they are served as userspace reads of the completed cache entry).
...
There is deliberately NO per-uid connection binding. An earlier design (`builder.mountd.uid-bound`, since removed) allowed one live connection per `SO_PEERCRED.uid`, on the premise that k8s userns gives each pod a distinct host-uid range and the uid therefore identifies a build. That premise does not hold in production: builder pods run `hostUsers: true` (required for FUSE passthrough), so every executor presents host uid 0 and the gate degraded to one-build-per-node — under a saturated scheduler packing several builder pods per node, concurrent builds structurally collided on the uid claim and exhausted their infra retries fleet-wide (observed live in a 16-client load run). More fundamentally, the gate policed the wrong layer: the build sandbox (pivot_root + seccomp + namespaces around the build process) is the actual security boundary, and the mountd UDS is connected to by the *trusted executor*, not by build code. At all-uid-0 the check contributed nothing against a sandbox escape either — an escapee is uid 0 like every executor. mountd's remaining defenses exist to bound damage IF the sandbox fails — `build-id-unique`, `one-mount`, validated `build_id` paths, kernel-enforced staging quotas, and the socket's gid DAC — and none of them keys on the peer uid.

r[builder.mountd.teardown-fast-release]

Connection teardown MUST release the `build_id` claim after metadata-only work (lazy unmount, mountpoint rmdir, quota release, staging quarantine-rename) and MUST NOT hold it across the recursive staging delete. The rename happens before the `build_id` claim is released, so a same-`build_id` remount cannot lose its fresh staging tree to the pending delete; the deep delete runs detached, with the sweep's quarantine arm as the retry backstop. A re-dispatch of the same drv reconnects within ~100 ms of EOF under a saturated scheduler, while the delete is O(staging size) — holding the claim across it is what made those re-Mounts collide.

r[builder.mountd.build-id-unique]

One live `build_id` per process: a process-wide set rejects `Mount{}` for a `build_id` another connection already owns, so a compromised builder cannot use a Mount to claim a victim's `build_id`.
...
| `rio_store_tiered_get_duration_seconds` (histogram) | chunk GET latency labeled `{tier="express"\|"standard"}`, each arm timing only its own tier's read — the direct Express-vs-S3-standard TTFB measurement |
...
| `rio_mountd_promote_reject_total` | rejected promotes, labeled `{reason="mismatch"\|"not-regular"\|"too-large"\|"race-timeout"\|"backlog"\|"other"}` — `backlog` is the per-connection in-flight cap turning away a pipelined flood |
...
| `rio_mountd_cache_evicted_bytes_total` | bytes unlinked by the LRU sweep, labeled `{dir="cache"\|"chunks"}` — a sustained nonzero rate means the node SSD is too small for its build mix |
...
- **Kernel ≥ 6.14** — `FUSE_OVER_IO_URING`, the castore-FUSE's only wire transport (§4); this floor also covers `FUSE_PASSTHROUGH` ([`7dc4e97a4f9a`](https://git.kernel.org/linus/7dc4e97a4f9a), 6.9+). `nix/nixos-node/kernel.nix` asserts at module eval; the node boots with `fuse.enable_uring=1` (`nix/nixos-node/hardening.nix`).
...
| NAR index | `store.index.file-digest` · `store.index.nar-ls-offset` · `store.index.nar-ls-streaming` · `store.index.table-cascade` · `store.index.authoritative` · `store.index.putpath-eager` · `store.index.rpc` |
