...
`rio-mountd` holds, per build, the UDS connection, a dup of that build's `/dev/fuse` fd, and the staging dirfds; ~250 LoC. Requests carry a `seq: u32` echoed in replies (so `spawn_blocking` `Promote`/`PromoteChunks` can reply out-of-order); errors are typed (`DigestMismatch`/`NotRegular`/`TooLarge`/`BadBuildId`/`AlreadyMounted`/`Unauthorized` are build-fatal, `Retryable(..)` is infra-retry). Admission is two-mode (§P0559): without a mountd token key the UDS socket is mode 0660 group `rio-builder` and mountd checks `SO_PEERCRED.gid`, rejecting others — the standalone/systemd posture, where the builder service simply joins the `rio-builder` group. Both the socket-inode DAC and `SO_PEERCRED` observe the peer's *init-namespace* gid, and a `hostUsers: false` builder pod (`r[sec.pod.host-users-false]`) has every gid kubelet-remapped into a per-pod range — no static in-pod gid maps to the host's `rio-builder` gid, so the gid gate admits no userns-confined builder. Production k8s therefore configures the dedicated mountd HMAC key (`mountdHmac.secretName` → `RIO_MOUNTD_HMAC_KEY_PATH` on the DaemonSet and the scheduler): the scheduler mints a per-build `MountdClaims{aud:"rio-mountd", build_id, tenant, issued, expiry}` token into `WorkAssignment.mountd_token` (same TTL as the assignment token, so the reconnect path's mid-build re-`Mount{}` stays within validity), the builder presents it in `Mount{build_id, token}`, and mountd verifies it offline. Either way the broker remains a strictly smaller privileged surface than the pre-ADR-022 model, where the builder pod itself held `CAP_SYS_ADMIN`: the brokered `BACKING_OPEN` registers an fd the builder already holds (conn-scoped, depth-0-only), and `Promote` is the integrity boundary for the shared cache.

r[builder.mountd.token-admission]

When a mountd token key is configured (token mode), mountd MUST admit a connection's `Mount{}` either when the peer's `SO_PEERCRED.gid` equals the allowed gid or when the request carries a token that verifies under that key — HMAC-valid, unexpired, audience `rio-mountd`, and `claims.build_id` equal to the requested `build_id`; a `Mount{}` satisfying neither MUST be rejected (`Unauthorized`) and the connection closed; in token mode the socket inode MUST be world-connectable (mode 0666), and without a configured key mountd MUST keep the gid-only admission and the 0660 root:allowedGid socket posture unchanged.

Rationale and consequences: the gid arm keeps the standalone/systemd worker working with zero extra config; the token arm is how a `hostUsers: false` executor pod — which cannot present any host gid — proves it is a scheduler-dispatched build rather than an arbitrary local process. The 0666 mode exists because the socket DAC cannot discriminate the peers token mode is for (every pod uid/gid is kubelet-remapped), so admission moves entirely in-protocol and the daemon keeps the pre-auth surface one frame deep: a provisionally-admitted (non-gid) connection must authenticate with its FIRST frame, anything else — another request, an undecodable frame, a bad token — is answered and closed, and an idle unauthenticated connection is closed after a bounded window; frames stay capped at `MAX_FRAME_BYTES` either way. Binding `claims.build_id` to the `Mount` argument means a leaked token cannot squat another build's id. A possible future hardening, deliberately not required here, is an additional `SO_PEERCRED.uid >= <userns floor>` check in production values so low-uid host daemons cannot even reach the token path.

r[builder.mountd.token-key-separate]

The mountd token key MUST be distinct from the store-facing assignment-HMAC key (and from the service-token key); mountd MUST NOT accept assignment or executor tokens for Mount admission, and the scheduler MUST NOT mint mountd tokens with the assignment key.

The verification key is mounted into the rio-mountd DaemonSet on every builder/fetcher node; if it were the assignment key, a single node compromise would yield a credential the store trusts for uploads and tenant-scoped castore reads. Three independent defences keep the families apart even under operational mix-ups: separate Secrets (`rio-mountd-hmac` vs `rio-hmac`), divergent claims shapes (`deny_unknown_fields` on both sides), and the explicit `aud: "rio-mountd"` discriminator.
...
| Privilege | `builder.mountd.fuse-handoff` · `builder.mountd.backing-broker` · `builder.mountd.promote-verified` · `builder.mountd.promote-bounded-copy` · `builder.mountd.orphan-scan` · `builder.mountd.concurrency` · `builder.mountd.build-id-validated` · `builder.mountd.build-id-unique` · `builder.mountd.uid-bound` · `builder.mountd.one-mount` · `builder.mountd.staging-quota` · `builder.mountd.token-admission` · `builder.mountd.token-key-separate` · `sec.boundary.mountd` |
