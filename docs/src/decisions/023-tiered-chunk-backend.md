...
Each rio-store pod is configured with at most one Express bucket — the one for its own AZ. Two ways to get there: an explicit bucket (`store.chunkBackend.expressBucket` → `RIO_CHUNK_BACKEND__EXPRESS_BUCKET`), correct when every store pod runs in that bucket's AZ (single-AZ or zone-pinned deployments, VM tests), or per-pod selection (P0554): helm hands every replica the zone-keyed map (`store.chunkBackend.expressBucketByZone`, from the terraform `express_bucket_by_zone` output, rendered as one JSON env var) plus the pod's own zone (`RIO_NODE_ZONE`, a downward-API read of the pod's `topology.kubernetes.io/zone` label — KEP-4742's `PodTopologyLabelsAdmission` plugin, beta in Kubernetes 1.35, copies it from the node at binding), and the store picks `map[zone]` at startup before the backend is built. An explicit bucket wins over the map. A pod with no selected bucket (its AZ has no Express bucket, the zone label is absent because the admission plugin is disabled, or the map is empty) runs with `local = None` and logs which piece was missing; the IMDS and node-label-lookup alternatives that were considered — and why they lost (hop-limit/IPv6/netpol friction vs. RBAC/SA-token posture) — are recorded in the [implementation plan's P0554 section](./022-implementation-plan.md).
