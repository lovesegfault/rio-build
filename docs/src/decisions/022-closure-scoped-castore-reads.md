...
Config `[castore_read_scope] mode = "off" | "log" | "enforce"` (+ cache size, idle TTL), modeled on `[assignment_revocation]`. **The shipped default is `enforce`** (decision 9); `log` is the rollback value, `off` exists for dev parity.
...
| Surface | Addition |
|---|---|
| store config | `[castore_read_scope] mode = "off" \| "log" \| "enforce"` (default `enforce`), plus cache capacity and idle TTL; modeled on `[assignment_revocation]` |
