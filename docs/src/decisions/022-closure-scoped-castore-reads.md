...
Config `[castore_read_scope] mode = "off" | "log" | "enforce"` (+ cache size, idle TTL), modeled on `[assignment_revocation]`. **The shipped default is `enforce`** (decision 9); `log` is the rollback value, `off` exists for dev parity. *Phase staging note:* Phase 1 shipped with `mode = log` (the builder did not present yet, so enforce would have leaned every read on the derivation fallback alone); the `enforce` default flipped when the Phase-2 builder presentation landed — `log` remains the rollback value, and decision 9 (enforce as the end-state default, no soak phase once both halves exist) is unchanged.
...
| Surface | Addition |
|---|---|
| store config | `[castore_read_scope] mode = "off" \| "log" \| "enforce"` (default `enforce` since the Phase-2 builder presentation landed; the store-only Phase 1 interim shipped `log` — see §3.4), plus cache capacity and idle TTL; modeled on `[assignment_revocation]` |
