...
6. **Seccomp profiles single-sourced at `nix/nixos-node/seccomp/`.** The AMI bakes them via `systemd.tmpfiles` (`hardening.nix`) — no bootstrap container, no SPO DaemonSet, no chart-side copies. k3s VM tests use the same `systemd.tmpfiles` delivery (`nix/tests/fixtures/k3s-full.nix`), so the JSON here is the only copy.
7. **Three AMI variants — UEFI/UKI for virtualized + arm64 .metal; legacy-bios/grub for x86_64 .metal.** AWS x86_64 bare-metal SKUs reject UEFI AMIs with `InvalidParameterValue` — `aws ec2 describe-instance-types --filters Name=bare-metal,Values=true Name=processor-info.supported-architecture,Values=x86_64 --query 'InstanceTypes[].SupportedBootModes'` returns `["legacy-bios"]` for every entry through gen 8. A UEFI-only x86 AMI made x86 metal nodes unschedulable (I-205). `.#node-ami-x86_64-bios` builds the same module tree with `ec2.efi=false` (`legacy+gpt` partition table, grub-bios, `bios-boot.nix` instead of `uki-boot.nix`); `xtask ami push` registers it alongside the UEFI variants and tags all three with `rio.build/boot={uefi,legacy-bios}`. A second EC2NodeClass `rio-metal` selects `(amd64, legacy-bios)` and `(arm64, uefi)`; only §13b metal NodeClaims reference it (§13c: hwClasses with `nodeClass: rio-metal`). Virtualized x86 + all arm64 keep the loaderless UKI image. Cost: the bios variant drops the perlless `forbiddenDependenciesRegexes` check (grub's installer is Perl); the perlless behaviors (overlay-/etc, userborn, systemd-initrd) are restated in `minimal.nix` and remain.
8. **`/dev/fuse` + `/dev/kvm` via containerd `base_runtime_spec`, not a device plugin.** runc `mknod`s the device nodes inside the container's `/dev` (container-namespace uid/gid — no `hostUsers:false` idmap-mount rejection). Every pod gets `/dev/fuse`; `/dev/kvm` is host-conditional — the AMI bakes both spec variants and containerd's `ExecStartPre` symlinks `/run/base-runtime-spec.json` to the `withKvm` variant iff `test -c /dev/kvm` succeeds on the host, so non-`.metal` pods don't see a dead `mknod` that fools `test -c /dev/kvm` probes then ENXIOs on open. kvm pods route to §13b metal NodeClaims via per-intent `nodeAffinity` (`r[ctrl.pool.node-affinity-from-intent]`) plus the pool-static toleration the controller derives from `features:[kvm]` (`r[ctrl.pool.kvm-device+2]`) — never a pool-static `nodeSelector`, which would deadlock against the affinity on shared features (r33 bug_002). No extended resource is requested — Karpenter NodeOverlay capacity is simulation-only (nothing writes `Node.status.capacity`), so kube-scheduler can't bind pods that request it. Eliminates the I-184 goroutine-orphan failure mode and the watchdog it required. The k3s VM-test fixture uses the same boot-time selection via a `k3s.service` `ExecStartPre` (k3s embeds containerd) and the same `base_runtime_spec` path in `services.k3s.containerdConfigTemplate` (a full `config.toml.tmpl` — containerd's `imports` merge replaces entire plugin sections, so a drop-in can't add a single key to runc).
