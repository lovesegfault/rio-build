...
7. **`/dev/fuse` + `/dev/kvm` via containerd `base_runtime_spec`, not a device plugin.** runc `mknod`s the device nodes inside the container's `/dev` (container-namespace uid/gid — no `hostUsers:false` idmap-mount rejection). Every pod gets both; opening `/dev/kvm` ENXIOs on non-`.metal`. The Karpenter NodeOverlay still declares synthetic `rio.build/{fuse,kvm}` capacity (cold-start bin-packing happens before any node exists); with no on-node device plugin, kubelet leaves that extended resource alone. Eliminates the I-184 goroutine-orphan failure mode and the watchdog it required. The k3s VM-test fixture uses the same `base_runtime_spec` JSON via `services.k3s.containerdConfigTemplate` (a full `config.toml.tmpl` — containerd's `imports` merge replaces entire plugin sections, so a drop-in can't add a single key to runc), plus a `kubectl patch node --subresource=status` for the capacity.
