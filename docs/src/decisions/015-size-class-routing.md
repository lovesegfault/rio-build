...
rio-build already has the building blocks: per-derivation EMA duration estimation via `build_history`, closure-size-based fallbacks for unknown derivations, and an executor scoring algorithm that could be extended with size-class filtering.
...
2. **Passive misclassification (not TAGS kill/restart):** When a build exceeds 2x its class cutoff, it is marked as misclassified and the EMA estimate is updated with a penalty. The build continues on its current executor because Nix builds are non-preemptible --- killing and restarting wastes all prior work and produces an identical result (deterministic builds).

3. **Resource-aware class bumping:** Beyond duration, the scheduler tracks peak memory, CPU, and output size per `(pname, system)`. A derivation that fits the "small" duration class but historically OOM-kills on small executors is bumped to "medium."

## Alternatives Considered

- **Uniform pools (current design):** Simple but wasteful. Over-provisioning every executor for the worst case means 90% of builds use 10% of their allocated resources. Under-provisioning risks OOM kills for the largest builds.
...
- **Per-derivation resource requests (Kubernetes VPA-style):** Each derivation would carry its own resource request, and the scheduler would bin-pack onto executors. More granular than size classes but much more complex (requires per-derivation resource prediction, dynamic pod resizing, and tighter scheduler/K8s integration). Size classes are a simpler 80/20 solution.

## Consequences

- **Positive:** Resource utilization improves dramatically. Small executors with high concurrency (8 builds/pod) handle the bulk of trivial derivations at low cost. Large executors with dedicated resources handle the long tail without OOM risk.
...
- **Negative:** Size-class boundaries create potential for queue imbalance: if all ready derivations are "medium" but only "small" executors are idle, the derivations wait even though resources exist. Mitigation: allow overflow routing (small class can spill to medium executors when small queue is empty).
...
- **Static size-class routing:** Executors declare a `size_class` (CRD field + `RIO_SIZE_CLASS` env); the scheduler classifies derivations via `build_history` EMA duration and routes accordingly.
...
- **Overflow routing:** Smaller classes spill upward when no matching-class executor is idle (up-only overflow).
