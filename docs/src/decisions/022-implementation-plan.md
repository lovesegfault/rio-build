...
| Device exposure: **no smarter-device-manager** | part of cutover | `/dev/fuse` → containerd `base_runtime_spec` injection (ADR-021 §7) — load-bearing post-P0560: the builder opens AND mounts the device itself; `rio-mountd` only receives a dup for `BACKING_OPEN` brokering (the kernel requires opener-userns == mounter-userns, so an fd-handoff *to* the builder cannot work). `/dev/kvm` → `hostPath{type:CharDevice}` + `nodeSelector{rio.build/kvm}` on kvm-pool pods + `extra-sandbox-paths=["/dev/kvm"]` in builder nix.conf. `nix/nixos-node/static-pods.nix` drops the device-plugin pod. |
...
| `docs/src/security.md` | `r[sec.pod.fuse-device-plugin]` stays as written — `base_runtime_spec` `/dev/fuse` injection remains and is load-bearing for builds (the builder must open the device it mounts; mountd only receives a dup); bump `r[common.hmac.claims]` (add `tenant`+`role`+`input_closure_digest`); add §Boundary-4 `r[sec.boundary.mountd]` (mountd threat surface: build_id traversal, disk-fill, cross-build interference, fd smuggling — and mitigations); rewrite §Known-Limitations #2/#3 (executors no longer hold `CAP_SYS_ADMIN`); update §Read-authorization (castore surface IS tenant-scoped); add `HasChunks` to §Cross-Tenant Chunk Probing |
...
>
> **Round 2 (§A deletion + flip consequences, same date).** `rio-builder/src/fuse/` (~5,070 LoC) is deleted along with `prefetch_manifests`, the warm-gate size cap, the store_fetch retry/jitter helpers, the `fuse_{mount_point,cache_dir,passthrough,fetch_timeout}` config fields (+ CLI flags), and the old-FUSE metric registrations; `fuse_threads` survives as the castore serve-thread knob. The `builder.fuse.*` rules, `builder.warmgate.{filter,manifest-prime}`, and `builder.result.input-enoent-is-infra+2` are retired from the spec (castore replacements live in this ADR set); `builder.warmgate.handshake+2` now specifies the ACK-immediately contract; `builder.mountd.fuse-handoff+2` / `builder.fs.fd-handoff-ordering+2` reflect the builder-opens-the-device protocol; `builder.typ`'s FUSE sections describe the castore stack. Helm: mountd-ds drops its `/dev/fuse` hostPath + the resolved mount-propagation TODO; the builder netpol drops the `builderS3Cidr` carve-out; the §A table's NodeOverlay / `fuseCacheSize` / device-plugin rows were already absent from the chart. mountd hardening: `Mount{}`'s fd is fstat-gated to the fuse character device and a missing/bogus fd is the new build-fatal `BadFuseFd`. Moved to round 3 (or later): the `RIO_BUILDER_JIT_FETCH` escape hatch named in the table does not exist anywhere (nothing to delete); the controller/CRD `fuse-cache` complex (the real-world successor of the table's `fuseCacheSize` row — `PoolSpec.{fusePassthrough,fuseCacheBytes}`, `[nodeclaim_pool].fuse_cache_bytes`, the `fuse-cache` emptyDir + `RIO_FUSE_*` pod env in `pool/pod.rs`, and the FFD/disk-budget addend that single-sources from it) is left in place because removing it changes the ephemeral-storage accounting model and its CEL/degrade tests, which need the round-3 k3s scenarios to validate; `nix/modules/builder.nix`'s `fuseStorePath`/`fuseCacheDir` options and the now-ignored `RIO_FUSE_*` env they set follow the same round-3 fixture rework.
>
> **Round 3a (existing-suite cutover, 2026-05-26).** The deferred "fuse-cache complex" and the fixture/scenario sweep landed; the new `vm-castore-e2e` scenario stays round 3b. **Controller/CRD:** `PoolSpec.{fusePassthrough,fuseCacheBytes}` + their CEL rules and degrade checks are gone (`fuseThreads` survives as the castore serve-thread knob); `[nodeclaim_pool].fuse_cache_bytes`, `BUILDER_FUSE_CACHE{,_BYTES}`, and the fuse addend in `pod_ephemeral_request`/`intent_pod_footprint`/`SizingCfg`/`ffd::simulate` are deleted — per-pod ephemeral is now `disk×headroom + LOG_BUDGET` only (the input closure lives in the node-level `/var/rio` caches, accounted by `varRioSize`); helm/14 and the values.yaml `dataVolumeSize` math drop the fuse term (need 441→391 Gi at unchanged 610 Gi), helm/16 drops the fuseCacheBytes axis (dataVolumeSize added in its place), `controller.toml` no longer renders `fuse_cache_bytes`. Executor pods lose the `fuse-cache` emptyDir, the `fuse-store` read-only-rootfs emptyDir, and the `RIO_FUSE_{MOUNT_POINT,CACHE_DIR,PASSTHROUGH}` env; they gain the `/run/rio-mountd` RO hostPath plus explicit `RIO_MOUNTD_SOCKET`/`RIO_CASTORE_{CACHE,CHUNKS,STAGING}_DIR` env, and the vestigial `/var/rio/castore` hostPath is dropped (pod.rs, mountd-ds.yaml, eks-node tmpfiles, vm-nixos-node assert). mountd's `--castore-dir` is now optional — the startup orphan scan skips a missing dir instead of creating it. **Fixtures:** `k3s-full.nix` imports `nixos-node/kernel.nix`, labels both nodes `rio.build/node-role=builder` (mountd-DS affinity), creates `/var/rio/{cache,chunks,staging}` + the gid-990 group; vmtest-full.yaml sets `mountd.{allowedGid:0, stagingQuotaBytes:0}` because vmtest executor pods run as root in the host userns and the k3s nodes have no XFS-prjquota loopback (production posture stays 990/10 GiB, asserted by helm/25 + vm-nixos-node; the pod-side gid story is still §P0559's open row). `nix/modules/builder.nix` (used by every standalone worker fixture) drops the old fuse options/env and now also runs `rio-mountd` (allowed-gid 990, staging quota 0 by default) with the builder service joining the `rio-builder` group so SO_PEERCRED matches. **Scenario sweep:** `scheduling/fuse-direct.nix` + `fuse-listxattr.nix` are removed with their wiring (they asserted the old persistent `/var/rio/fuse-store` mount; no markers were attached — castore equivalents are vm-castore-e2e's `stat-dcache-absorbed`/`shutil-copy2`, round 3b); `sigint-graceful` keeps its exit-code + profraw assertions and drops the FUSE-mountpoint baseline/teardown checks; stale fanout/overlay-readdir/assertions.py comments updated. Spec: controller.typ/sla-sizing.typ drop the per-pod fuse-cache budget language (the `crd-field("Pool","fuseCacheBytes")` reference would now fail docs eval); the three remaining `/var/rio/fuse-store` mentions in builder.typ §overlay/§ns-order/§store-model prose are pre-existing round-2 leftovers left for the P0562 audit (they sit inside rule bodies and need a deliberate `tracey bump` review), as is the `docs-lint` redness from lazy-store.typ/builder.typ `refs.gh` links into the deleted `rio-builder/src/fuse/` tree. **Blocker found (needs a §A fix round before the suite can go green):** the castore read path never authenticates to rio-store — `StoreClients` is built on a bare channel and neither `build_tree`'s `GetDirectory` nor the open()/stream fetches attach the per-build assignment token, while `DirectoryService` enforces `r[store.castore.tenant-scope]` ("send a JWT or an HMAC assignment token", no anonymous fallback). vm-scheduling-core-standalone now fails every build with `castore mount failed: castore DAG prefetch failed: GetDirectory: … DirectoryService requires a tenant`, and the k3s scenarios will fail identically (their assignment tokens exist but are never attached). The fix is builder-side (thread `assignment.assignment_token` into the castore-FUSE store clients per build), plus a decision for the no-HMAC standalone fixtures (enable `withHmac` so assignment tokens carry tenants, or give the store an explicit dev-mode tenant); the round-3a fixture/scenario work above is independent of that and stays. *(Resolved 2026-05-26 — owner decision: **option B, real tenancy everywhere**; `store.castore.tenant-scope` stays exactly as written, and "a build that is not attributable to a tenant cannot perform castore reads — it fails loudly at mount time" is now an explicit, accepted property of the design, not an oversight. The builder-side token threading landed (`5e13a90ae`).)*
>
> **Round 3a follow-up (tenancy in the test fleet, 2026-05-26).** The canonical "this fixture runs as tenant T" recipe is the one the k3s lifecycle splits already carried (lifecycle.nix prelude + `jwtEnabled` module wiring): provision the tenant row up front, give the client SSH key a comment naming it (`r[gw.auth.tenant-from-key-comment]`), run with HMAC so dispatched assignment tokens verify and carry the tenant claim, push the seed only after the key swap, and have any gateway-bypassing `SubmitBuild` set `tenant_name` (or attach the tenant JWT in `jwtEnabled` mode). For the standalone fixtures this landed as shared helpers — `mkBootstrap` gained a `tenant` argument that provisions the tenant via AdminService `CreateTenant` (`rio-cli create-tenant` with the fixture's service-HMAC key — the same RPC the k3s bootstrap flow and `xtask k8s cli create-tenant` use, not a psql poke) and switches the key setup to the new `sshKeySetupFor`; the scheduling fixture flips `withHmac = true` and its gRPC-direct submit helper injects `tenantName`. The k3s prod-parity lifecycle split needs nothing extra (same prelude; the scheduler's `tenant_name` body fallback resolves the same row when `jwtEnabled=false`). With the recipe in place the UNAUTHENTICATED mount failures are gone end-to-end (verified on vm-scheduling-core-standalone), **but the suite still cannot go green: gateway-pushed paths never get `path_tenants` rows.** `PutPath`/`PutPathBatch` use the session JWT's tenant only for narinfo signing, `PutPathChunked` writes the junction only from the HMAC (builder) tenant, and the scheduler's upserts cover derivation *outputs* only — so client-pushed **sources** (the seeded busybox closure in the VM tests; any locally-pushed source in production) are invisible to tenant-scoped castore reads, and every build whose closure includes one dies at mount with `store returned no Directory body for digest … (referenced by …busybox…)` until `max_infra_retries` poisons it (reproduced on vm-scheduling-core-standalone with the full recipe and on vm-lifecycle-core-k3s, which already had it). Candidate fixes, each small but with different fixture implications, owned by the next fix round: **(a)** store-side — write `path_tenants` for JWT-attributed uploads (the gateway already attaches the session JWT on every push), making the JWT-attributed gateway push the source-attribution mechanism (fixtures then need `jwtEnabled`/a gateway signing key wherever sources are pushed); **(b)** scheduler-side — upsert `path_tenants` for the submitted build's input closure (it already computes the closure and knows the tenant), which keeps the no-JWT standalone recipe sufficient and also covers pre-existing paths a new tenant starts referencing. Fanning the tenancy recipe out to the remaining standalone scenarios waits on that decision so the fixtures aren't churned twice. *(Resolved 2026-05-26 — option (a) landed in rio-store: `PutPath`/`PutPathBatch` now upsert `path_tenants` for the session-JWT tenant in the same transaction that completes the manifest (`r[store.put.tenant-attribution]`), so gateway-pushed sources are attributed to the pushing tenant at upload time; tenant-less dev-mode pushes still write no row, and the substituter/scheduler attribution paths are unchanged.)*
>
> **Round 3a follow-up 2 (JWT-attributed seed + next gap, 2026-05-26).** With `store.put.tenant-attribution` in, the standalone tenancy recipe gained its missing half: the standalone fixture now has a `withJwt` flag (gateway gets the lib/jwt-keys.nix signing seed, the **store only** gets the pubkey — the scheduler deliberately stays out of JWT mode so the scenarios' tokenless grpcurl SubmitBuild/CancelBuild helpers keep working through the `tenant_name` dual-mode fallback), the scheduling fixture turns it on, and `mkBootstrap`'s tenant flow now asserts right after the seed push that `path_tenants` has rows for the tenant (the assert pins the JWT-attributed-push half of the recipe and names the missing piece when it fails). Verified on vm-scheduling-core-standalone: `seed attributed to vm-sched: 1 path_tenants rows`, no UNAUTHENTICATED, no missing-Directory mount failures — the per-build castore mount now works end-to-end in the VM suite. **Next castore-stack gap (new code finding, needs a rio-builder fix round):** the build itself now fails with nix-daemon reporting `store path '<drv_path>' does not exist` (`MiscFailure`, all four fanout leaves identically). The executor drives the daemon with `wopBuildDerivation` (drv path + full `BasicDerivation`), but the daemon still wants the `.drv` to be a valid, readable store path inside the per-build chroot store — under the old whole-path FUSE any path (including the `.drv`) was materialized JIT on lookup, while the castore lower serves only `WorkAssignment.input_roots`, and nothing materializes or registers the `.drv` itself (the executor already holds `assignment.drv_content`). Fix direction: write/register the `.drv` (from `drv_content`, falling back to a store fetch) into the per-build store before the daemon call — or include `.drv` paths in `input_roots` — then re-run the matrix; the tenancy recipe and fixtures above are complete and stay as-is. Fan-out of the recipe to the remaining standalone scenarios and the not-yet-tenanted k3s scenarios (security-nonpriv, fetcher-split, le-build, dashboard, …) stays parked behind that fix so red runs aren't burned on a known-blocked path (the k3s lifecycle splits already carry the full recipe via `jwtEnabled` and will be the first re-run targets).
...
| `infra/helm/rio-build/templates/karpenter.yaml` `rio-builder-{fuse,kvm}` NodeOverlays | **DROPPED** (already absent from the current chart) — both existed to advertise `smarter-devices/*` capacity. fuse: containerd `base_runtime_spec` injects `/dev/fuse` into every executor pod, so no extended resource is needed (the builder opens the device itself; mountd only brokers ioctls). kvm: hostPath + `nodeSelector{rio.build/kvm}` (the metal NodePool already labels+taints; capacity is unbounded so no overlay needed). | helm |
