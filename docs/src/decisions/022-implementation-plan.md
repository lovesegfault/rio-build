...
| `rio-store/tests/migrations.rs` | **do NOT pin yet** — `054_nar_index.sql` is appended to by P0572/P0581/P0583/P0586; pinning here would fail `migration_checksums_frozen` on each subsequent plan. The `(54, "<sha384>")` PINNED entry is added in P0586's exit (last toucher). The `unpinned migration NNN` panic is expected and ignored for 054 until then. |
...
| `docs/src/components/builder.md` | `r[builder.mountd.{build-id-validated,uid-bound,staging-quota,promote-bounded-copy,one-mount}]` col-0 spec text + `r[builder.fs.listxattr-size-branch]` col-0 spec text. (`fuse-handoff`/`backing-broker`/`concurrency` already col-0-defined in design-overview §11; `promote-verified`/`orphan-scan` already col-0-defined in ADR §2.5/§2.6 — do NOT duplicate.) |
...
| `rio-store/src/grpc/put_path_chunked/validate.rs` | `validate_begin(&Begin, &AssignmentClaims) -> Result<ValidatedBegin>`: token role accepted per `r[store.put.builder-chunked-only]`; `hash_part(deriver)==claims.drv_hash`; `blake3(sorted(input_closure))==claims.input_closure_digest`; `len(outputs) ≤ MAX_BATCH_OUTPUTS` and `outputs[*].store_path` pairwise-distinct; `output_paths = {outputs[*].store_path}`; per-output `store_path ∈ claims.expected_outputs` (non-CA), `nar_size ≤ MAX_NAR_SIZE`, `len(refs) ≤ MAX_REFERENCES`, `refs ⊆ input_closure ∪ output_paths`, `len(chunk_manifest) ≤ MAX_CHUNKS`; `len(directories) ≤ MAX_DIR_NODES`; for each `d ∈ directories` run `castore::Directory::validate(d)` (snix's checks — entry names single-component, sorted, no dups, child sizes consistent) then recompute `blake3(canonical-encode(d))`, build `HashMap<dir_digest, &Directory>`, walk each `root_node` asserting every reachable `dir_digest` present and per regular file the contiguous `chunk_manifest` `len` run sums to `FileEntry.size`; build `manifest_len: HashMap<Digest, u32>` and `first_site: HashMap<Digest,(out_idx,pos)>` over all outputs in order; `novel ⊆ manifest_len.keys()` no-dups; acquire `nar_bytes_budget` for `Σ manifest_len[d]` over `d ∉ novel` (lengths attested via `FileEntry.size`). All violations → `INVALID_ARGUMENT`. `// r[impl store.put.chunked-bounds]` |
| `rio-store/src/grpc/put_path_chunked/mod.rs` | new — on validated `Begin`: arm `PlaceholderGuard` (`r[store.put.drop-cleanup+2]`, `r[store.gc.orphan-heartbeat]`); per non-CA output, if a `'complete'` manifest exists mark idempotent-skip else insert `'uploading'` placeholder with `references` (`r[store.put.wal-manifest]`, `r[store.put.placeholder-refs]`); build `(tx_map, rx_map): HashMap<Digest, oneshot::{Sender,Receiver}<Bytes>>` over `Begin.novel`; `verify_sem = Semaphore::new(VERIFY_WINDOW)`; `cancel = CancellationToken::new()`; `received: HashSet<Digest>`; spawn `verify_driver` per non-skipped output. **Receive loop** per `Chunk`: assert `digest ∈ novel ∧ digest ∉ received ∧ len(bytes)==manifest_len[digest] ∧ blake3(bytes)==digest` else `INVALID_ARGUMENT`; `received.insert(digest)`; `verify_sem.acquire()`; `cas::put` (`r[store.cas.upload-bounded]`); `tx_map.remove(digest).map(|tx| tx.send(bytes))`. On client stream end OR `cancel`: `drop(tx_map)`. **verify_driver(out_idx, output, dir_map, rx_map, first_site, sem, cancel, candidate_set)**: walks `output.chunk_manifest` in order interleaving `nar::Encoder` framing from `dir_map`; framing AND body bytes → per-output SHA-256 AND `RefScanSink(candidate_set)`; per entry: `(out_idx,pos)==first_site[digest]` → `rx_map.remove(digest).await` (Canceled ⇒ `Verdict::Incomplete`), then `sem.add_permits(1)`; else → `cas::get(digest)` (Err ⇒ `Verdict::Unavailable`), prefetch ≤32, no permit release. `candidate_set = input_closure ∪ output_paths`. Returns `Verdict::{Match, Mismatch{computed_nar_hash, scanned_refs}}` where Match requires `sha256==nar_hash && sorted(scanned)==refs`; any non-Match fires `cancel`. **Join all drivers**: any `Mismatch` → `FAILED_PRECONDITION`, `rio_store_{narhash,refs}_mismatch_total++` (per failing condition), structured-log `{store_path, drv_path, builder_pod, claimed, computed}`; any `Incomplete` → `FAILED_PRECONDITION`, `rio_store_putpath_incomplete_total++`; any `Unavailable` (no Mismatch) → `UNAVAILABLE`, `rio_store_putpath_verify_unavailable_total++`. **Commit txn** (all Match, non-skipped outputs): for `is_ca` outputs recompute `make_fixed_output(name, computed_nar_hash, true, refs)` and assert `== store_path` else `PERMISSION_DENIED`; then per output INSERT `manifest_data`, `nar_index.root_node`; UPSERT `directories`/`file_blobs` refcount += 1 per output-tree occurrence; INSERT `directory_tenants`/`file_blob_tenants`/`path_tenants` for `claims.tenant_id` `ON CONFLICT DO NOTHING`; chunk refcount UPSERT; `UPDATE chunks SET durable=TRUE WHERE blake3_hash = ANY($sorted) AND NOT durable` (lock-order per `r[store.chunk.lock-order]`); flip `uploading→complete` (or `INSERT … ON CONFLICT (store_path_hash) WHERE status='complete' DO NOTHING` for CA); enqueue compat writes if enabled (`r[store.compat.nar-on-put]`). The legacy `grpc/put_path.rs` complete-txn gets the same `durable=TRUE` UPDATE. Idempotent re-drive (`r[store.put.idempotent]`). `// r[impl store.chunk.self-verify]` `// r[impl store.put.narhash-sync]` `// r[impl store.put.refs-sync]` `// r[impl store.put.chunked-ca]` `// r[impl store.integrity.verify-on-put]` `// r[impl store.atomic.multi-output]` `// r[impl store.castore.tenant-scope]` |
| `rio-builder/src/upload.rs` | rewrite `upload_all_outputs`: per output, single canonical-NAR-order walk (`r[builder.nar.entry-name-safety]`); the walk feeds the **full NAR byte sequence** (framing, entry names, symlink targets, file bytes) into per-output SHA-256 AND Boyer-Moore refscan (`r[builder.upload.references-scanned]`); per regular file, the same disk read additionally drives FastCDC (16/64/256 KiB, `r[store.cas.fastcdc]`) emitting `(offset,len,blake3)` + whole-file blake3 → `file_digest`; `Directory` body construction. Optional input-reuse shortcut: size match against the in-memory `(size, file_digest)` map of declared inputs (P0559's `tree::InoMap`) → `cmp` against the castore-FUSE lower → reuse input's `file_digest` (still re-FastCDC + SHA-256 + refscan over the `cmp`-read bytes; chunk-list reuse unsound for legacy whole-NAR-chunked inputs). End-of-walk over all outputs: dedupe `directories` by digest; batch `HasChunks` over `∪ chunk_manifest.digest` → `novel`; client-stream `Begin{deriver, outputs[], directories[], novel[], input_closure: WorkAssignment.input_closure}` then each `novel` chunk. `// r[impl builder.upload.fused-walk]` `// r[impl builder.upload.chunked-manifest]` `// r[impl builder.upload.batch+2]` |
...
**Exit:** `/nixbuild --checks` green; `nix build .#checks.x86_64-linux.vm-put-path-chunked` green; append `(54, "<sha384>")` to `PINNED` in `rio-store/tests/migrations.rs` (P0586 is the last plan to touch `054_nar_index.sql`).
...
{"plan":589,"title":"AssignmentClaims.{role,tenant_id,input_closure_digest} + dispatch populate","deps":[544,588],"crate":"rio-auth,rio-scheduler","priority":92,"status":"UNIMPL","complexity":"LOW","note":"sequenced before P0573/P0586/P0584 — all three read these fields; closure already in hand at dispatch via P0588"}
...
{"plan":573,"title":"DirectoryService RPC: GetDirectory(recursive=true server-BFS stream) / HasDirectories / HasBlobs (batch bitmap; I-110 lesson)","deps":[572,589],"crate":"rio-proto,rio-store","priority":90,"status":"UNIMPL","complexity":"MED","note":"snix-wire-compatible; recursive=true is the P0559 mount-time prefetch path; tenant-scoping reads claims.tenant_id (P0589)"}
...
| `builder.mountd.staging-quota` | components/builder.md | bin/rio-mountd.rs (P0567) | composefs-spike-priv `staging-quota` (P0578) |
| `builder.mountd.promote-bounded-copy` | components/builder.md | bin/rio-mountd.rs (P0567) | composefs-spike-priv `promote-bounded-copy` (P0578) |
| `builder.mountd.one-mount` | components/builder.md | bin/rio-mountd.rs (P0567) | composefs-spike-priv `one-mount` (P0578) |
...
| `migrations/054_nar_index.sql` | P0551, P0572, P0581, P0583, P0586 | P0551 → P0572 → P0581 → P0583 → P0586; same migration file (greenfield); **NOT added to `PINNED` until P0586** (`migration_checksums_frozen` would fail every intermediate plan otherwise) |
