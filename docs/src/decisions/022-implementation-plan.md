...
| `rio-store/src/grpc/put_path_chunked.rs` | new — token verify (accepts `Builder`/`Gateway`/`Admin` per `r[store.put.builder-chunked-only]`); insert placeholder manifest (`r[store.put.wal-manifest]`). On `Begin`, spawn `verify_driver(chunk_manifest, dir_tree, rx_map)`: walks manifest in NAR order, per entry awaits `rx_map[digest]` (novel) or `cas::get(digest)` (deduped/durable, prefetch window ≤32), feeds each body interleaved with `nar::Encoder` framing into a SHA-256 accumulator. Receive loop per `Chunk`: assert `blake3(bytes)==digest` AND digest ∈ `Begin.chunk_manifest` else `INVALID_ARGUMENT`, then `cas::put` (×32 bounded, `r[store.cas.upload-bounded]`), then `tx_map[digest].send(bytes)`. On stream end + verify-driver join: if `sha256 != Begin.nar_hash` → delete placeholder, roll back refcounts, `rio_store_narhash_mismatch_total{}` inc, structured-log `{store_path, drv_path, builder_pod, claimed, computed}`, return `FAILED_PRECONDITION`; else in one txn flip `uploading→complete` **and `UPDATE chunks SET durable=TRUE WHERE blake3_hash = ANY($sorted_manifest_digests) AND NOT durable`** (lock-order per `r[store.chunk.lock-order]`). The legacy `grpc/put_path.rs` complete-txn gets the same `durable=TRUE` UPDATE. Idempotent re-drive (`r[store.put.idempotent]`). `// r[impl store.chunk.self-verify]` `// r[impl store.put.narhash-sync]` `// r[impl store.integrity.verify-on-put]` |
...
| `nix/tests/scenarios/put-path-chunked.nix` | new — builder→`PutPathChunked` of a multi-file output; `GetPath` round-trip NAR sha256 == claimed; tampered `Chunk` body → `INVALID_ARGUMENT`; deliberately wrong `Begin.nar_hash` → `FAILED_PRECONDITION` reject + `rio_store_narhash_mismatch_total` increment + manifest row absent; high-dedup output (most chunks pre-seeded) → commit succeeds with verify-driver having fetched deduped chunks; `HasChunks` reports false for refcount≥1-but-uploading chunk |
...
{"plan":586,"title":"PutPathChunked: builder-side fused walk + HasChunks + put_path_chunked handler + pipelined sync narhash verify","deps":[551,572,573,577],"crate":"rio-store,rio-builder,rio-proto","priority":85,"status":"UNIMPL","complexity":"MED","note":"closes TODO(P0433/P0434); store working set drops to one chunk; nar_hash recomputed before commit (verify driver pipelined with upload), mismatch → FAILED_PRECONDITION reject"}
...
| `store.put.narhash-sync` | decisions/022 §6.3 | grpc/put_path_chunked.rs (P0586) | vm-put-path-chunked (P0586) |
