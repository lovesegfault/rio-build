...
**Crate:** `infra, xtask` · **Deps:** P0548, P0553 · **Complexity:** LOW · **Status: PARTIAL 2026-05-25** (helm half done in the adr-022-express worktree; per-pod AZ→bucket selection + xtask `--set-json` plumbing remain — see the handoff note)
`store.chunkBackend.kind={s3|tiered}` helm value (default `s3`): tiered reuses `bucket`/`prefix` for the authoritative tier and adds `expressBucket` (single explicit Express bucket → `RIO_CHUNK_BACKEND__EXPRESS_BUCKET`, rendered only when `kind=tiered` and non-empty; empty → env omitted → `express_bucket=None` → `local=None`) and `expressBucketByAzId` (the terraform-output map; values-only, never rendered into the pod). Locked by `nix/tests/helm/26-tiered-chunk-backend.sh`. No zonal-endpoint config needed: the aws-sdk detects the `--x-s3` bucket-name suffix and routes to `https://s3express-{az_id}.{region}.amazonaws.com` + the CreateSession auth flow on its own (`rio_common::s3::default_client` already relies on this). **Exit:** `helm template --set store.chunkBackend.kind=tiered` renders (met); `/nixbuild --checks` green. ★ FIRST SHIPPED VALUE (U2)

**Remaining (handoff to the main line — requires rio-store/xtask changes, out of scope for the infra-only worktree):** the original sketch ("Store Deployment exposes node zone via downward-API env from `topology.kubernetes.io/zone`") does not work — the downward API exposes *pod* fields, not node labels, and every zero-Rust workaround is structurally broken: an initContainer cannot inject env into a sibling container and the store image has no shell to source a file (`r[sec.image.control-plane-minimal]` forbids adding one); per-AZ zone-pinned Deployments would break the ComponentScaler's single-`deployments/scale` patch target. So multi-AZ per-pod bucket selection needs code:
1. `rio-store`: at startup, when `chunk_backend.kind=tiered` and `express_bucket` is unset but a (new) `express_bucket_by_az_id` map field is non-empty, resolve own placement and select; no match → `local=None`. Three candidate resolution mechanisms (no winner picked — whoever implements this decides):
   - **IMDS** `placement/availability-zone-id` → keys the AZ-ID map (`express_bucket_by_az_id` output) directly. Verify hop-limit/IPv6-IMDS reachability from a v6-only pod first.
   - **Node-label lookup**: `GET /api/v1/nodes/{spec.nodeName}` → `topology.kubernetes.io/zone` against the zone-name-keyed map (`express_bucket_by_zone` output). Needs a `get nodes` ClusterRole.
   - **KEP-4742 PodTopologyLabelsAdmission** (beta in Kubernetes 1.35 — the cluster's pinned version): the admission plugin copies `topology.kubernetes.io/zone` from the node onto the *pod's own* labels at binding, so a plain downward-API env (`fieldRef: metadata.labels['topology.kubernetes.io/zone']`) hands the store its zone name with zero IMDS and zero extra RBAC; pairs with the zone-name-keyed `express_bucket_by_zone` output. Needs verification that EKS 1.35 actually enables the beta admission plugin. (Still not zero-Rust: reading that env and selecting from the map is store-side code either way.)

   Whichever mechanism: also verify `AWS_USE_DUALSTACK_ENDPOINT=true` (set pod-wide in store.yaml) doesn't break the SDK's directory-bucket endpoint resolution before enabling in prod.
2. `xtask/src/k8s/eks/deploy.rs`: `--set store.chunkBackend.kind=tiered` + `--set-json store.chunkBackend.expressBucketByAzId=$(tofu output -json express_bucket_by_az_id)` (or the `express_bucket_by_zone` output if the selection keys on zone names).
Until then a multi-AZ `kind=tiered` deployment runs every replica with `local=None` (functionally identical to `kind=s3`); a zone-pinned or single-AZ deployment can set `expressBucket` explicitly and gets the full cache tier today.
