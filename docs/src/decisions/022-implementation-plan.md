...
1. `rio-store`: at startup, when `chunk_backend.kind=tiered` and `express_bucket` is unset but a (new) `express_bucket_by_az_id` map field is non-empty, resolve own placement — IMDS `placement/availability-zone-id` (verify hop-limit/IPv6-IMDS reachability from a v6-only pod first) or `GET /api/v1/nodes/{spec.nodeName}` → `topology.kubernetes.io/zone` label against the zone-name-keyed map (needs a `get nodes` ClusterRole) — and select; no match → `local=None`. Verify `AWS_USE_DUALSTACK_ENDPOINT=true` (set pod-wide in store.yaml) doesn't break the SDK's directory-bucket endpoint resolution before enabling in prod.
2. `xtask/src/k8s/eks/deploy.rs`: `--set store.chunkBackend.kind=tiered` + `--set-json store.chunkBackend.expressBucketByAzId=$(tofu output -json express_bucket_by_az_id)`.
