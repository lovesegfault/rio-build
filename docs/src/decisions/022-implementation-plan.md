...
| File | Change |
|---|---|
| `rio-proto/proto/build_types.proto` | un-reserve field 3 in `WorkAssignment`: `repeated InputRoot input_roots = 3;` where `message InputRoot { string store_path = 1; oneof node { bytes dir_digest = 2; FileEntry file = 3; SymlinkEntry symlink = 4; } }` (FileEntry/SymlinkEntry from `castore.proto`). Add `repeated string input_closure = 4;` — the **transitive runtime closure of the build's inputs** (BFS over `references`, equivalent to Nix `computeFSClosure(inputs)`; NOT `approx_input_closure` which omits ad-hoc additions), sorted, exactly the value the scheduler hashes into `claims.input_closure_digest` (P0589). `// r[impl sched.dispatch.input-roots]` |
...
{"plan":588,"title":"WorkAssignment.{input_roots,input_closure}: scheduler→builder root_node + sorted-closure transport (proto fields 3,4 + dispatch.rs populate)","deps":[572],"crate":"rio-proto,rio-scheduler","priority":85,"status":"UNIMPL","complexity":"LOW","note":"~50 LoC; r[sched.dispatch.input-roots]; input_closure is exactly what P0589 hashes into claims.input_closure_digest"}
