...
**Crate:** `spike, nix` · **Deps:** P0541 · **Complexity:** LOW · **Status:** PARTIAL 2026-05-19 (kernel mechanisms i–iv, viii, ix; mountd protocol v–vii, x–xv deferred to P0567)
...
{"plan":567,"title":"rio-mountd DaemonSet (fuse-fd-handoff + BACKING_OPEN broker + Promote/PromoteChunks verify-copy + cache+chunks ownership + build_id validation + per-uid conn binding + staging quota + metrics)","deps":[576,578],"crate":"rio-builder,infra","priority":80,"status":"PARTIAL","complexity":"MED","note":"daemon + SOCK_SEQPACKET/postcard wire protocol + unit tests landed; helm DS, eks-node pquota assert, and the VM test (P0578-deferred subtests) pending; tokio async per-conn; Promote+PromoteChunks both on spawn_blocking+Semaphore; build_id ^[A-Za-z0-9_-]{1,64}$; openat(base_dirfd) not string-concat; one conn per peer_uid; kernel project-quota staging enforcement"}
...
| `builder.mountd.fuse-handoff` | decisions/022-design-overview §11 | castore_fuse/mountd.rs (P0567) | vm-castore-e2e `cold-read` (P0560§B) |
| `builder.mountd.backing-broker` | decisions/022-design-overview §11 | castore_fuse/{mountd,mountd_proto}.rs (P0567) | composefs-spike-priv `passthrough-under-overlay` (P0578) |
| `builder.mountd.promote-verified` | decisions/022 §2.6 | castore_fuse/mountd.rs (P0567) | composefs-spike-priv `passthrough-under-overlay` (P0578) + vm-castore-e2e `integrity-fail` (P0560§B) |
| `builder.mountd.orphan-scan` | decisions/022 §2.5 | castore_fuse/mountd.rs (P0567) | vm-castore-e2e `mountd-restart` (P0560§B) |
| `builder.mountd.concurrency` | decisions/022-design-overview §11 | castore_fuse/mountd.rs (P0567) | composefs-spike-priv (vi) (P0578) |
...
| `builder.mountd.build-id-validated` | decisions/022-design-overview §11 | castore_fuse/mountd.rs (P0567) | composefs-spike-priv `build-id-traversal-reject` (P0578) |
| `builder.mountd.uid-bound` | decisions/022-design-overview §11 | castore_fuse/mountd.rs (P0567) | composefs-spike-priv `second-conn-same-uid` (P0578) |
| `builder.mountd.build-id-unique` | decisions/022-design-overview §11 | castore_fuse/mountd.rs (P0567) | composefs-spike-priv xv (P0578) |
| `builder.mountd.staging-quota` | decisions/022-design-overview §11 | castore_fuse/mountd.rs (P0567) | composefs-spike-priv `staging-quota` (P0578) |
| `builder.mountd.promote-bounded-copy` | decisions/022-design-overview §11 | castore_fuse/mountd.rs (P0567) | composefs-spike-priv `promote-bounded-copy` (P0578) |
| `builder.mountd.one-mount` | decisions/022-design-overview §11 | castore_fuse/mountd.rs (P0567) | composefs-spike-priv `one-mount` (P0578) |
