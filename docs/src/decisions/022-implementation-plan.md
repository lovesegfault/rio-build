...
>
> **Reconciliation (2026-05-24, deployment tail).** `mountd-ds.yaml` + the eks-node `/var/rio` work landed with four notes. (a) **The pquota assert needed a filesystem to assert on**: the AMI is single-root ext4, so `/var/rio/staging` had nothing satisfying "XFS with `pquota`". `eks-node.nix` now declares `fileSystems."/var/rio"` as a sparse XFS loopback (`/var/rio.img`, `services.rio.eksNode.varRioSize`, default 100G) mounted `prjquota` — works identically on EBS-only and NVMe NodeClasses, and keeps mountd's monotonic-from-1 projids out of kubelet's ephemeral-storage projid space on the instance-store RAID0. The module-eval assertion checks the *covering* `fileSystems` entry for `/var/rio/staging` (longest mount-point prefix) is `xfs` + `prjquota`/`pquota`, so deleting the loopback entry fails `node-ami-eval`; `vm-nixos-node` asserts the booted mount. (b) **No dedicated image**: `rio-mountd` is a `[[bin]]` of the rio-builder crate, so the DaemonSet runs `image: rio-builder` with `command: ["/bin/rio-mountd"]` — that image is already prebaked into the AMI (`executorSeed`), so mountd needs zero ECR pull on node boot. (c) **`fsGroup: rio-builder` on builder pods is deferred to P0559** (a precise TODO sits at the `PodSecurityContext` in `pool/pod.rs`): `fsGroup` only grants a supplementary group — enough for the socket-inode DAC but not for the daemon's `SO_PEERCRED.gid` check, which reads the egid (`runAsGroup`); and under the production `hostUsers: false` no static in-pod gid maps to the host's `rio-builder` gid at all. The gid itself is pinned to **990** at three sites (`users.groups.rio-builder.gid` in eks-node.nix, `mountd.allowedGid` in values.yaml, `--allowed-gid` in the DS args; helm/25 + `vm-nixos-node` cross-check two of the three pairs). (d) **`privileged: false` + `capabilities.add: [SYS_ADMIN]` + seccomp `RuntimeDefault` shipped as specced**, which means the castore mounts the daemon creates do NOT propagate to the host mount namespace — k8s only admits `mountPropagation: Bidirectional` on `privileged: true` containers. Nothing consumes the mountpoints until P0560; the resolution (flip to privileged, or move the `mount(2)` into the builder's own userns since it already holds the `/dev/fuse` fd) is recorded as a TODO in `mountd-ds.yaml` and belongs to P0560.
>
