...
Instead, the builder stages fetched files in a **per-build staging dir** (`/var/rio/staging/{build_id}/`, builder-writable, mode 0700 owned by the build's uid, created by mountd at `Mount` time) and mountd verify-copies into the cache on `Promote`. mountd opens the staging file `O_RDONLY|O_NOFOLLOW`, rejects non-regular or oversized, **and rejects when the connection's running `staged_bytes` total would exceed `staging_quota_bytes`** (`r[builder.mountd.staging-quota]` — the quota is enforced mountd-side, not just advisory in the builder; a compromised builder ignoring its self-tracking cannot fill the node-shared partition). It creates `cache/ab/<digest>.promoting` (mountd-owned, `O_EXCL|O_WRONLY`, 0444), **stream-copies while hashing**, verifies `blake3 == digest`, renames `.promoting → final`. The copy is the integrity boundary: the cache file is a distinct inode mountd created and hashed.
...
After §6.2 validation passes, the handler precomputes `first_site: HashMap<Digest, (output_idx, manifest_pos)>` — for each `d ∈ Begin.novel`, the *globally first* position across all outputs' `chunk_manifest`s in `(output_idx, manifest_pos)` order — and spawns one verify driver per non-skipped output, concurrent with the receive loop. Each driver walks its output's `chunk_manifest` in NAR order, emitting `nar::Encoder` framing from the §6.2-validated `Directory` tree (the §6.1 invariant that `chunk_manifest` order equals canonical-NAR-walk order over `root_node`, with per-file `len` runs summing to `FileEntry.size`, is what makes the interleave well-defined). For each manifest entry it obtains the chunk body, feeds framing + body into the per-output SHA-256 and `RefScanSink`, and drops the body:

| `digest` is… | source | `VERIFY_WINDOW` |
|---|---|---|
| in `Begin.novel` and `(output_idx, pos) == first_site[digest]` | `rx_map[digest].await` — oneshot signalled by the receive loop after `cas::put` | release 1 permit on consume |
| in `Begin.novel` otherwise (intra- or cross-output repeat) | `cas::get(digest)` — the first site's `cas::put` has completed, so the chunk is in S3 (and typically the replica's moka tier) | — |
