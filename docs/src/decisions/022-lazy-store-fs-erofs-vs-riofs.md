...
At mount time the builder holds the closure's set of input store paths and each path's `root_digest` (from the scheduler-supplied `BuildAssignment`). It prefetches the full Directory DAG via **`GetDirectory(root_digest, recursive=true) → stream<Directory>`** (`r[store.castore.directory-rpc]`) — one server-side BFS-walked stream per store path, deduped on `dir_digest` so a subtree shared across N paths is sent once. Chromium-scale (8 221 dirs, 23 218 files) is on the order of the EROFS image's 5.3 MiB encoded; held in heap rather than mmapped via loop device.
...
`NarIndex` is no longer fetched at mount — the Directory DAG carries everything `lookup`/`getattr`/`readdir`/`readlink` need (`{name, digest, size, executable}` per `FileNode`; `{name, target}` per `SymlinkNode`). `NarIndex` remains the store-side per-NAR index that maps `file_digest → (nar_hash, nar_offset, chunk-range)` for `ReadBlob` and the `DigestResolver`; the builder consults it via rio-store at `open()` time, not mount time.

### 2.3 Inode model — content-addressed

r[builder.fs.castore-inode-digest]

Inode numbers are derived from content, not path: `ino(FileNode) = h(file_digest)`; `ino(DirectoryNode) = h(dir_digest)`; `ino(SymlinkNode) = h("l" ‖ target)`, where `h` is the low 63 bits of blake3 with bit 63 set (so `ino ≥ 2`, never colliding with `FUSE_ROOT_ID = 1`). Two regular files anywhere in the closure with the same bytes get the **same FUSE inode** — the kernel's icache holds one `struct inode`, one `open()` upcall fetches it once, and one `BACKING_OPEN` binds it to one page-cache. Two directories with identical `dir_digest` get the same inode and the **same dcache subtree** — `lookup(ino, name)` is answered from one `Directory` body regardless of which store path led there.
