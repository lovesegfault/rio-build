...
r[store.chunk.has-chunks-authenticated]

`HasChunks` MUST require an authenticated caller (a JWT or a verified HMAC assignment token). The retrieval RPCs (`GetChunk`, `GetChunks`) stay unauthenticated — knowing a BLAKE3 digest already proves the caller has (or had) the bytes, so the response discloses nothing new — but a presence probe's response is one *new* bit ("someone, in some tenant, has uploaded content with exactly these bytes"), and a file under `FASTCDC_MIN_BYTES` is exactly one chunk, so an anonymous probe is a whole-file content-existence oracle over offline candidate guesses. The *answer* is deliberately not tenant-scoped — global chunk dedup is the point, and that cross-tenant presence disclosure to authenticated builders is an accepted trade-off — but the probe must not be anonymous.
...
r[store.put.verify-file-digest]

The verify task MUST also recompute each regular file's whole-file BLAKE3 over the chunk bodies it splices into that file's content range and compare it against the `FileEntry.digest` the builder claimed in the Directory body; a mismatch is a **mismatch** verdict for that output. The bytes are already in hand for the SHA-256, so the cost is one extra hasher per file. Without this check the NAR-level SHA-256 still passes (it hashes the same bytes), but the *Directory body* — which is committed keyed by `FileEntry.digest` and served cross-tenant by `GetDirectory`/`ReadBlob`/`StatBlob` — would map a file digest to content that does not hash to it, poisoning the global `file_digest → content` dedup namespace for every tenant whose castore-FUSE later opens a file with that digest.
