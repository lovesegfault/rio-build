...
Device exposure: containerd's `base_runtime_spec` injects `/dev/fuse` into every executor pod — the builder opens and `mount(2)`s it itself and hands `rio-mountd` a dup only for ioctl brokering (§2.5). `/dev/kvm` rides the same mechanism: on kvm-capable (.metal) hosts the boot-time `pickExecStartPre` symlinks the `withKvm` spec variant (`nix/base-runtime-spec.nix`), so every pod on that node gets the device, and the per-build nix-daemon needs no rio-side config — stock Nix auto-adds the `kvm` system feature when `/dev/kvm` is accessible and bind-mounts it into sandboxes that have the feature (the `kvm-hostpath-spike` validated the sandbox-visibility mechanism with explicit `extra-sandbox-paths`; the shipped path gets the same effect from Nix's defaults). No device-plugin DaemonSet, no hostPath device volumes.
...
a′. `mount("fuse.rio-castore", castore_mnt, …, "fd=N,rootmode=40555,user_id=<own euid>,group_id=<own egid>,default_permissions")` on a builder-owned per-build mountpoint (under the build's working area) — userns-root may mount FUSE from an fd it holds; no `allow_other` (everything reads through the overlay, which carries the overlay mounter's credentials into the lower)
a. `fuser::Session::from_fd(fuse_fd)` → spawn `castore_fuse::serve` (§2.6) — `from_fd` answers the `FUSE_INIT` queued by (a′) and **must be answering before (b)**
...
Teardown — builder unmounts (or its pod exits), then closes the UDS:
...
**Ordering is load-bearing:** the builder MUST `mount(2)` its fuse fd (a′) and the castore-FUSE server MUST be answering on it (a) before builder step (b). overlayfs probes the lower's root at `mount(2)`; with no one serving the fuse connection, that probe deadlocks. (`Session::from_fd` answers the `FUSE_INIT` queued by (a′), so completing (a) is the "serving" barrier.)
...
Instead, the builder stages fetched files in a **per-build staging dir** (`/var/rio/staging/{build_id}/`, builder-writable, mode 0700 owned by the build's uid, created by mountd at `Mount` time) and mountd verify-copies into the cache on `Promote`. The disk-fill defense is an **XFS project quota** mountd sets on the staging dir at `mkdirat` (`r[builder.mountd.staging-quota]`): a compromised builder writing past `staging_quota_bytes` gets `ENOSPC` from `write(2)` directly — mountd does not need to observe the writes. mountd opens the staging file `O_RDONLY|O_NOFOLLOW`, rejects non-regular or per-file oversized, creates `cache/ab/<digest>.promoting` (mountd-owned, `O_EXCL|O_WRONLY`, 0444), **stream-copies at most `fstat(src).st_size` bytes** while hashing (`r[builder.mountd.promote-bounded-copy]` — the builder owns the source inode and can append to it concurrently; copying past `st_size` would let it fill the mountd-owned cache before the digest reject), verifies `blake3 == digest`, renames `.promoting → final`. The copy is the integrity boundary: the cache file is a distinct inode mountd created and hashed.
...
r[builder.fs.mountd-reconnect]

When a `Promote` or `PromoteChunks` RPC fails because the connection to `rio-mountd` is gone (daemon restart, socket reset), the builder MUST attempt to re-establish the session — re-dial the UDS, re-issue `Mount{build_id}` as the new connection's first request, and retry the failed RPC — with bounded attempts and jittered backoff inside the existing per-request budgets, except while the post-exhaustion cooldown is active; requests whose callers already degrade cheaply (`BackingOpen`/`BackingClose` → keep-cache reads / a deferred close) and test-injected socketpair connections fail fast instead. The (restarted) daemon MUST accept that re-`Mount` for a surviving live build's staging dir without disturbing its contents.

The daemon's per-connection state (claims, kept fuse-fd dup, staging dirfds) dies with the old process, but the staging dir and the builder's flock'd `.rio-live` sentinel survive on disk: the staging setup is EEXIST-tolerant, re-chowns to the same `SO_PEERCRED` uid (the gid/uid gates are unchanged), and re-applies the staging quota to the project id the dir already carries — re-tagging it with a fresh id would hand the build a second, empty quota bucket per restart, orphan the old id's limit record, and hide it from the next incarnation's seed-above-survivors scan; only an untagged dir draws a fresh id from the monotonic counter — so re-adoption needs nothing beyond the filesystem state the daemon already trusts. Client-side, re-dial attempts are bounded with jittered backoff (≈5.5 s of sleeps expected; the re-`Mount` inside each attempt is capped at 5 s, so even a daemon that accepts connections but never replies bounds the cycle at ≈30 s — about one `mountd_request_timeout`) and an exhausted cycle starts a cooldown so a long outage degrades to the pre-reconnect fail-fast behavior instead of stalling every FUSE callback; a successful Promote-driven reconnect swaps the shared connection, so cache-hit opens regain passthrough without ever paying the cycle themselves. Re-establishment attempts are counted by `rio_builder_castore_fuse_mountd_reconnect_total{outcome}`.

r[builder.fs.promote-degrade-staged]

When a whole-file fill has fetched and digest-verified its bytes into the build's staging dir but `Promote` still cannot publish them into the shared cache (rio-mountd unreachable even after the bounded reconnect attempts, or another infrastructure-class promote failure), the builder MUST serve the open from the verified staged copy (`FOPEN_KEEP_CACHE`, userspace reads) instead of failing it with `EIO`; only a daemon-side rejection of the bytes themselves (digest mismatch, not-regular, too-large) remains fatal to the open.

This mirrors the trust the streaming fill already places in its own staging fd: the bytes were verified against `file_digest` by the builder before the rename, the handle is opened before the staging file can be unlinked, and the `.rio-live` flock keeps reapers away from the dir for the build's lifetime. Only the node-cache publication is lost — later opens of the digest re-fetch and re-attempt the promote — and each degraded open is counted by `rio_builder_castore_fuse_degraded_serve_total`.

r[builder.fs.streaming-open-threshold]

**Streaming open is the during-fill mode for large files.** The 1000 largest files in nixpkgs are *all* >64 MiB (median 179 MiB, 7 files >1 GiB; `top1000.csv`), and access-pattern measurement (`42aa81b2`) shows consumers touch 0.3-33% of them — whole-file fetch over-fetches 64-99.7%. For files > `STREAM_THRESHOLD` (default 8 MiB) on cache miss, `open()` cannot reply passthrough (no complete backing fd yet) so it replies `FOPEN_KEEP_CACHE`, spawns a background fill task, and returns after the first chunk (~10 ms). `read(off,len)` upcalls once per uncached page during fill, blocking (bounded) on the sequential fill's high-water mark until the range arrives (priority-bumping the requested range is a possible future optimization); `mmap(MAP_PRIVATE)` page-faults route through the same `read` path, covering linkers. ~80 LoC; spike-proven (`15a9db79`) — `KEEP_CACHE` does not suppress cold-page upcalls, only invalidation, so no mode-flip is needed within the streaming window. Once the fill completes and renames into `/var/rio/cache/`, the next `open()` of that digest takes the passthrough path (§2.6 case 1) — the streaming mode is one-shot per file per node.
...
The handler runs **one sequential verify task** (no per-output drivers, no oneshot map, no permit semaphore). The task holds, per output, a SHA-256 accumulator and a `RefScanSink`; tracks `seen: HashSet<Digest>` and `next_novel: usize` (cursor into `Begin.novel`); and walks the global `(output_idx, manifest_pos)` sequence in order — `outputs[0].chunk_manifest` start to end, then `outputs[1]`, and so on. At each position it emits `nar::Encoder` framing from the §6.2-validated `Directory` tree into that output's accumulator+scanner (no-op for idempotent-skipped outputs), then obtains the chunk body and feeds it likewise:

| at `(out_idx, pos)` with `d = chunk_manifest[out_idx][pos].digest` | body source |
|---|---|
| `d ∈ novel` and `d ∉ seen` | `stream.next().await` → assert `frame.digest == d == novel[next_novel]`, `len(bytes) == manifest_len[d]`, `blake3(bytes) == d` else `INVALID_ARGUMENT`; `cas::put(d, bytes)` (`r[store.cas.upload-bounded]`; transient error → `UNAVAILABLE`); `seen.insert(d)`; `next_novel += 1` |
| otherwise (deduped, or any repeat) | `cas::get(d)` → assert `len() == manifest_len[d]` else `Mismatch` |

A single bounded prefetch task (window ≤32, shared across the whole walk) issues `cas::get` for upcoming non-novel/repeat positions into a small LRU so S3 reads overlap with `stream.next()` waits. Because the walk is sequential and `novel` is ordered by global first occurrence, the next `Chunk` frame the builder MUST send is always exactly `novel[next_novel]`; the assertion is the wire-order enforcement. There is no separate receive loop, no `rx_map`/`tx_map`, no `VERIFY_WINDOW` semaphore, no cross-driver synchronization, and no `first_site` lookup — the §6.2 ordering invariant on `novel` makes those structurally unnecessary. Per-stream working set is one chunk in hand plus the ≤32-entry prefetch LRU. Upload and verify overlap: deduped-chunk fetches are prefetched while the task awaits `stream.next()`, so wall-clock added is approximately `max(0, deduped_fetch_time − upload_time)` rather than `Σ nar_size / sha256_throughput`.

After the global walk: if `stream.next()` returns another frame, `INVALID_ARGUMENT` (extra `Chunk` after `novel` exhausted); if `next_novel < len(novel)`, the stream ended early — `Incomplete`.

r[store.put.refs-sync]

The verify driver feeds the **same NAR byte sequence** as the SHA-256 — framing, entry names, symlink targets, and chunk bodies — through a per-output `RefScanSink`, so the scanner sees exactly what §6.1's builder-side walk and Nix's `scanForReferences` see. The candidate set is `Begin.input_closure ∪ {outputs[*].store_path}` — the `r[builder.upload.references-scanned]` definition, with the input-closure half attested by the scheduler via `claims.input_closure_digest` and the output-paths half being the set this RPC is committing (for non-CA, `{outputs[*].store_path} == claims.expected_outputs` by the §6.2 membership check; for CA, `expected_outputs` is empty and the actual `store_path` values are used). At verdict time, per output, `sorted(scanned_refs) == outputs[i].refs` is part of the match condition. The scanner is the existing `rio_nix::refscan` Boyer-Moore implementation; on bytes already in hand for SHA-256 it adds approximately memcpy-rate CPU.

Per non-skipped output, the verdict is **match** (`sha256 == outputs[i].nar_hash` AND `sorted(scanned_refs) == outputs[i].refs`), **mismatch** (either condition fails, or a `cas::get` body length differed from `manifest_len`), **incomplete** (stream ended before `novel` exhausted), or **unavailable** (a transient `cas::get` or `cas::put` error — S3 fault or a deduped chunk GC'd between the builder's `HasChunks` and now).
