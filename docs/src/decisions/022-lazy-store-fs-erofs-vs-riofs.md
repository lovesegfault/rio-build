...
The builder is untrusted (`r[builder.upload.references-scanned]` context: outputs are adversary-controlled bytes); rio-store signs `narinfo`. The `nar_hash` in `Begin` is therefore **claimed**, not attested. rio-store independently recomputes it before commit — the same `r[store.integrity.verify-on-put]` contract `PutPath` already honors — without reintroducing the whole-NAR buffer §6.2 eliminates.

On `Begin` arrival the handler spawns a verify driver that walks `chunk_manifest` in NAR order alongside the receive loop. For each entry: deduped chunks (already `durable` per `r[store.chunk.has-chunks-durable]`) are fetched via `cas::get` from the tiered backend; novel chunks are awaited on per-digest oneshot channels signalled by the receive loop after blake3-verify and `cas::put`. Each chunk body — interleaved with NAR framing emitted from the `Directory` tree (`nar::Encoder` headers/padding, exactly as §6.1's builder-side SHA-256 sink) — is fed into a SHA-256 accumulator and dropped. Working set stays one chunk plus a bounded prefetch window for deduped fetches; the §6.2 single-chunk-in-flight property holds. Upload and verify overlap: deduped-chunk fetches run while novel chunks are still streaming, so wall-clock added is approximately `max(0, deduped_fetch_time − upload_time)` rather than `nar_size / sha256_throughput`.

On stream end, the SHA-256 verdict gates commit. **Match:** the `uploading→complete` transaction proceeds (manifest flip + `chunks.durable=TRUE`). **Mismatch:** the placeholder is deleted, refcounts rolled back (same path as `PutPath` step-2 reject), the stream returns `FAILED_PRECONDITION`, `rio_store_narhash_mismatch_total` increments, and a structured-log line carries `{store_path, drv_path, builder_pod, claimed_nar_hash, computed_nar_hash}`. Novel chunks already in S3-standard are left for `r[store.chunk.grace-ttl]` orphan sweep — they are content-addressed (cannot poison), not yet `durable` (cannot cause `HasChunks` skips), and the same orphan path already covers builder-crash-mid-stream. A mismatch means a builder NAR-framing bug or a compromised builder; both are alert-worthy and approximately never.
