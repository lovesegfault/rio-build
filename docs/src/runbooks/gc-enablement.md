...
1. **Worker version**: All workers running a version with the NAR reference scanner (commit <TODO: fill in after PR2 lands> or later). Check: `kubectl get pods -l app=rio-worker -o jsonpath='{.items[*].spec.containers[*].image}'`
