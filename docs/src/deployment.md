...
- **Pod spec:** `hostUsers: false` is incompatible with `/dev/fuse` hostPath volumes (kernel rejects idmap mounts on device nodes). The NixOS AMI injects `/dev/{fuse,kvm}` via containerd `base_runtime_spec` (OCI `linux.devices` — runc `mknod`s inside the container's `/dev`); on k3s/kind a `smarter-device-manager` DaemonSet provides them as a device plugin.
- **`/dev/fuse` access:** Worker pods need access to `/dev/fuse`. A `hostPath` volume with `privileged: true` works for development but production should use `base_runtime_spec` device injection or a device plugin to avoid granting full privileges. `CAP_SYS_ADMIN` alone is not sufficient for `/dev/fuse` access — the container's device cgroup must also allow the FUSE character device.
