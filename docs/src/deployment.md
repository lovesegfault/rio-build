...
- **Instance type:** Compute-optimized (e.g., `c8a.xlarge` on AWS). For IO-heavy builds the `rio-builder-nvme-{x86,aarch64}` NodePools select instance-store-NVMe families via `karpenter.k8s.aws/instance-local-nvme > 0`; the `rio-nvme` EC2NodeClass sets `instanceStorePolicy: RAID0` and the AMI's `rio-nvme-mount.service` mounts the resulting `/dev/md0` at `/var/lib/kubelet` with `prjquota` so kubelet enforces per-pod ephemeral-storage via XFS project quotas.
