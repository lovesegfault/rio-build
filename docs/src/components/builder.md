...
2. Chunk and upload to rio-store (CAS) via `PutPathChunked` — client-side FastCDC, see [ADR-022 §6](../decisions/022-lazy-store-fs-erofs-vs-riofs.md). Each request carries the scheduler-issued HMAC assignment token in the `x-rio-assignment-token` gRPC metadata header; the store verifies the token and rejects uploads for paths not in `claims.expected_outputs` (see [Security: assignment tokens](../security.md#boundary-2-gatewayexecutor--internal-services-grpc)). Builder-role tokens are **not** accepted by `PutPath`/`PutPathBatch` (`r[store.put.builder-chunked-only]`).
...
For **single-output derivations**, the builder uses `PutPathChunked` with a single output (atomicity is vacuous for one output).
