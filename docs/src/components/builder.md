...
r[builder.input.warm-bounded+3]
The FUSE input-warm phase (`warm_inputs_in_fuse`) MUST materialize each input by calling the FUSE cache's fetch path directly (`prefetch_path_blocking`), NOT by `stat()`ing through the builder's own FUSE mount. Each input's prefetch timeout MUST be at least `nar_size / WARM_MIN_THROUGHPUT_BPS` (size-scaled, floored at `fuse_fetch_timeout`); the phase MUST be bounded by an overall deadline of at least `Σnar_size / (WARM_MIN_THROUGHPUT_BPS × WARM_FUSE_CONCURRENCY)` (floored at `WARM_OVERALL_DEADLINE`). On expiry the build proceeds with a partial warm and increments `rio_builder_input_warm_timeout_total`. Un-warmed inputs are subject to the I-043 overlay negative-dentry race (rare, recoverable via build retry). I-178: a flat per-path timeout (60 s) aborted a 1.9 GB input mid-fetch; the daemon's subsequent overlay→FUSE `lstat` saw ENOENT → `MiscFailure` → `PermanentFailure` poison. The FUSE-callback path (`ensure_cached`'s `self.fetch_timeout`) stays at the flat `fuse_fetch_timeout` --- only the warm path is size-scaled. I-165c: a stat-through-own-FUSE warm parks the `spawn_blocking` thread in kernel D-state (`request_wait_answer`) when the store stalls; the deadline detaches that thread but cannot wake it. If the build later dies by SIGKILL (OOM), `FuseMount::Drop` never runs --- no fusectl abort --- and the D-state threads pin the process forever (pod shows `Running` with the OOMKilled container un-reaped). Calling the cache directly keeps any detached thread in userspace `block_on(gRPC)` (bounded by the per-path timeout, dies on SIGKILL). After the cache confirms a path is on local disk, ONE fast `stat()` through the FUSE mount populates the kernel's FUSE-layer dentry for I-043 --- that stat hits the FUSE `lookup()` local-disk fast path and returns in microseconds, never D-states.

r[builder.result.input-enoent-is-infra]
When the nix-daemon returns `MiscFailure` with an error message indicating a missing input path (`getting attributes of path '<p>'`) and `<p>` is in the build's computed input closure, the builder MUST report `BuildResultStatus::InfrastructureFailure` (not `PermanentFailure`). The input was verified present in rio-store by `compute_input_closure`; its absence at sandbox-setup time is a worker-local materialization failure (warm timeout, FUSE EIO, overlay negative-dentry race), not a build defect.
