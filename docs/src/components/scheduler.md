...
A transient failure that promotes `size_class_floor` (`r[sched.builder.size-class-reactive]` / `r[sched.fod.size-class-reactive]`) MUST NOT increment `retry_count`. Promotion is bounded by the size-class ladder length (`next_*_class` returns `None` at the largest class); `max_retries` becomes the budget for retries at the SAME size class. I-213: with promotion consuming the budget, `max_retries=2` capped the climb at three rungs of a five-rung ladder regardless of `r[sched.sizing.soft-feature-floor]`.
