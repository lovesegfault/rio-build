...
| Node pool | `rio.build/builder` taint | Dedicated `rio.build/fetcher` taint (`r[fetcher.node.dedicated+4]`) |
...
- `r[fetcher.node.dedicated+4]` — dedicated nodes carrying the `rio.build/fetcher` taint+label, minted per-intent through the NodeClaim shim (§13e)
