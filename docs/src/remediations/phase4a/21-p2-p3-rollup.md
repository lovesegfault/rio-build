...
| (no ID — tracey gap) passthrough `r[verify]` | `fuse/mod.rs:8` + new test | `r[impl builder.fuse.passthrough]` exists; no `r[verify ...]` anywhere. `tracey query untested` flags it. The passthrough mode is the perf-critical path (kernel handles reads directly, no userspace copy) — it deserves an assertion. Minimum viable: unit test that mounts with `passthrough=true`, opens a file, asserts `passthrough_failures` atomic stays 0. Full verification needs a VM test (passthrough requires `CAP_SYS_ADMIN` + modern kernel) — defer to VM-test batch, but land the unit-test stub with `#[ignore]` + `r[verify builder.fuse.passthrough]` now so tracey stops flagging it. | ~25 |
