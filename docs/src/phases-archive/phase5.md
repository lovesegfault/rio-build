...
- [x] Activate CA early cutoff in rio-scheduler
...
- [x] NAR chunk transfer optimization: only transfer missing chunks when populating worker stores
...
- [x] Live preemption / migration — mid-build `ResourceUsage` streaming via `ProgressUpdate`, worker-side checkpoint, scheduler detects "about to OOM, move it"
- [x] Atomic multi-output registration — all-or-nothing semantics across a derivation's outputs; currently partial registration is possible on upload failure
- [x] Chaos testing harness (toxiproxy or equivalent for network fault injection)
