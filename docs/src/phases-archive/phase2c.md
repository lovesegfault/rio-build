...
> **Deferrals (status as of Phase 3a completion):**
> - ~~`rio-store/src/main.rs` ChunkBackend construction from config~~ — **done in 3a** (`ChunkBackendKind` config → shared `Arc<ChunkCache>` + NixOS `extraConfig`).
> - ~~`ema_peak_cpu_cores` EMA write~~ — **done in 3a** via cgroup v2 `cpu.stat` 1Hz polling.
> - `WorkerPoolSet` CRD + `CutoffRebalancer` — **deferred to Phase 4** (adaptive size-class cutoffs from `misclassification_count`). Still using operator-configured static cutoffs.
> - Closure-size-as-proxy fallback in `Estimator` — still deferred; needs scheduler-side nar_size tracking.
