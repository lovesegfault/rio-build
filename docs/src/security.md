...
> **Service-token bypass (`r[sec.authz.service-token]`):** PutPath skips assignment-token verification when the caller presents a valid `x-rio-service-token` — an HMAC-signed `ServiceClaims { caller, expiry_unix }` keyed with `RIO_SERVICE_HMAC_KEY_PATH` (a separate secret from the assignment key). The gateway mints one per upload with a 60s expiry. The same mechanism gates controller-only mutating RPCs on the scheduler's `AdminService` (`AppendInterruptSample`, `DrainExecutor`): builders share port 9001 with `AdminService` (CCNP allows scheduler:9001 at L4 only), so without the gate a compromised builder could poison λ\[h\] or drain arbitrary executors. The controller mints `caller="rio-controller"` per request via an interceptor; the scheduler verifies and checks against a per-RPC allowlist. This replaces the former certificate-CN check and is transport-agnostic. See `rio-auth/src/hmac.rs`, `rio-store/src/grpc/put_path/`, `rio-scheduler/src/admin/mod.rs::ensure_service_caller`.
