...
| TLS certificates | All gRPC components | Via cert-manager auto-renewal (90d certs, renew at 30d) | Implemented — see `infra/k8s/overlays/prod/cert-manager.yaml` |
...
- **Phase**: Implemented (Phase 3b). See `infra/k8s/base/fod-proxy.yaml` (Squid + allowlist) and the worker's `spawn_daemon_in_namespace` (`fod_proxy` param, injects env only when `is_fixed_output`).
