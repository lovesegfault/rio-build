...
+ *Config* — `sla.{tiers, defaultTier, defaultDisk, probe, featureProbes, maxCores, maxMem, maxDisk}` in scheduler config (`headroom` is the §Coupled-model formula, not a key; the FUSE cache budget is controller TOML `[nodeclaim_pool].fuse_cache_bytes` --- the #(refs.crd-field)("Pool", "fuseCacheBytes") field is CEL-rejected per `pool.rs:268-275` --- and the per-build log budget is `const LOG_BUDGET_BYTES`, no CRD/config exposure); plumbed via helm values. Tiers accept envelope form from day one; phase 2 reads only the `p90` entry until phase 12 lands.
