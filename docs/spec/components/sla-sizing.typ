...
#algorithm(
  ...
  + *function* #smallcaps[ReconcilePool]$("intents", "live")$ #rann[`intents = ⊥` on RPC error — distinct from $emptyset$]
    ...
    + *for each* $n in "live"$, $n$ NodeClaim-backed, with kubelet `NotReady` for $> 2 dot.op q_0.9 ("boot"["cell"(n)])$, *or* $n$ marked wedged by the controller's open-attempt clustering (#rref("ctrl.nodeclaim.wedge-cluster"): $>= 2$ distinct derivations whose open attempts ran out their intent deadlines on $n$ inside the 30-minute window, node attribution from the ledger's `source_node` / the spawn-ack binding) — *and* the controller requires kubelet `Ready` for this arm (so an apiserver↔node partition does not evict a healthy node), *and* per-tick fleet-wide reaping is capped at $min(3, ceil(0.05 dot.op |"live"|))$ nodes (so a single-tick blip cannot cascade) — the evidence is ledger facts the controller reads itself; `GetSpawnIntentsResponse.dead_nodes` is always empty since the scheduler-side heartbeat detector retired (field kept until the 1d proto sweep) *do* cordon, evict, delete $n$ #rann[Karpenter disruption gated; rio owns health incl. kubelet-up-runtime-hung]
    + #text(
        size: 0.85em,
