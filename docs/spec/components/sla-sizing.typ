...
#algorithm(
  ...
  + *function* #smallcaps[ReconcilePool]$("intents", "live")$ #rann[`intents = ⊥` on RPC error — distinct from $emptyset$]
    ...
    + *for each* $(h, "cap")$ minus @ice\-masked *do*
      ...
      + $N <- min(1 + ceil(max(0, sum_U i.c^* - "anchor.cores") slash "bulk.cores"), 1 + floor(max(0, "budget" - "anchor.cores") slash "bulk.cores"))$ #rann[$>= 1$; the three-term law --- demand, budget, and the sh-043 inflight-headroom (`max_inflight_unlaunched - |{"live": "Launched" != "True"}|`), no flat per-tick cap (RETIRED, live_049 L1, `ctrl.nodeclaim.mint-deficit-proportional`); mem/disk under-coverage from $sum c^*$-only $N$ self-corrects next tick]
