...
+ *Config* — `sla.{tiers, defaultTier, defaultDisk, probe, featureProbes, maxCores, maxMem, maxDisk}` in scheduler config (`headroom` is the §Coupled-model formula, not a key; `fuseCacheBudget`/`logBudget` are #(refs.crd)("Pool") CRD fields, controller-side); plumbed via helm values. Tiers accept envelope form from day one; phase 2 reads only the `p90` entry until phase 12 lands.
