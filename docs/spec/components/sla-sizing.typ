...
#algorithm(
  ...
  + *function* #smallcaps[ReconcilePool]$("intents", "live")$ #rann[`intents = ⊥` on RPC error — distinct from $emptyset$]
    ...
    + *for each* $(h, "cap")$ minus @ice\-masked *do*
      ...
      + create NodeClaims: $1 times "anchor"$ then $(N - 1) times "bulk"$; `created_this_tick += anchor.cores + (N-1)·bulk.cores` #rann[core-waste $<= "anchor.cores" + "bulk.cores"$; sh-043: the L1-era pricing — `CreateFleet` rate pressure is absorbed by Karpenter batching + cloud-provider backoff — DID NOT account for Karpenter's cross-pool `cluster.Synced()` gate (sh-043: did not account for cluster.Synced); the inflight-headroom term bounds the unlaunched set so `Synced()=false` windows stay $<<$ tick (worst-case pricing at the component recap's config row)]
...
+ #set enum(numbering: n => [13#"abcdefgh".at(n - 1).])
  ...
  + *Forecast provisioning (controller)* — \~3.5–4.5k LoC Rust + \~400 nix all-in, including `rio-lease` extraction.
    ...
    *Config:* `sla.{maxLeadTime, maxConsolidationTime, nodeClassRef}`, `karpenter.nodeclaimPool.maxInflightUnlaunched` (+ the DEPRECATED-IGNORED `maxNodeClaimsPerCellPerTick` row, retained parse-only — live_049 L1), with config-load assertion `maxCores < 1024`. The worst-case create burst is the THREE-TERM bound: `min(⌊remaining-budget/min-chunk⌋, max_inflight_unlaunched − |{live: Launched≠True}|)` creates per tick across all classes (illustration: `⌊30000/191⌋ ≈ 157` at the launchability-grounded anchor chunk would be the budget arm; the headroom arm caps it at ≤50 — ≈2.5-5s of `Synced()=false` per tick at the observed ~10-20/s CreateFleet throughput). The L1-era pricing — absorbed by the controller's kube-client QPS posture and Karpenter's CreateFleet batching + cloud-provider backoff — DID NOT account for Karpenter's cross-pool `cluster.Synced()` gate (sh-043: did not account for cluster.Synced; aws/karpenter-provider-aws#7428): the inflight-headroom term bounds the unlaunched set so `Synced()=false` windows stay ≪ tick.
