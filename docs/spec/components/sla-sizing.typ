...
#r("sched.sla.disk-reaches-ephemeral-storage+3")
...
#memo[Neither the duration-residual $sigma$ range $[0.1, 0.4]$ used throughout this document nor the $sigma_M$ range is *measured*. The duration range is an assumption pending live `build_samples` data; gate (d) in §Implementation Phasing is the empirical check. The 1.25 floor is provisional pending the same data, and the OOM-reactive penalty-bump (§Exploration, last paragraph; spec marker `r[sched.sla.reactive-floor+8]`) is the safety net if it proves too low.]
...
#algorithm(
  ...
  + *function* #smallcaps[ReconcilePool]$("intents", "live")$ #rann[`intents = ⊥` on RPC error — distinct from $emptyset$]
    ...
    + *for each* $(h, "cap")$ minus @ice\-masked *do*
      ...
      + $"headroom" <- "max_inflight_unlaunched" - |{n in "live": "Launched"(n) != "True" and not "Registered"(n) and not "terminating"(n)}| - "created_count_this_tick"$; *if* $"headroom" <= 0$ *then* skip cell #rann[the sh-043 third law term; `created_count_this_tick` is the within-tick claim count (NOT cores) accumulated across earlier cells in this pass — same bug_015-polarity accumulator shape as `created_this_tick`]
      + $N <- min(1 + ceil(max(0, sum_U i.c^* - "anchor.cores") slash "bulk.cores"), 1 + floor(max(0, "budget" - "anchor.cores") slash "bulk.cores"), "headroom")$ #rann[$>= 1$ --- all three min-args are $>= 1$ past the headroom $<= 0$ guard, so the anchor guarantee ($N - 1 >= 0$) holds; the three-term law --- demand, budget, and the sh-043 inflight-headroom, no flat per-tick cap (RETIRED, live_049 L1, `ctrl.nodeclaim.mint-deficit-proportional`); mem/disk under-coverage from $sum c^*$-only $N$ self-corrects next tick]
      + create NodeClaims: $1 times "anchor"$ then $(N - 1) times "bulk"$; `created_this_tick += anchor.cores + (N-1)·bulk.cores`; `created_count_this_tick += N` #rann[core-waste $<= "anchor.cores" + "bulk.cores"$; sh-043: this WAS THE L1-era R17 pricing (`CreateFleet` rate pressure is absorbed by Karpenter batching + cloud-provider backoff) and it DID NOT account for Karpenter's cross-pool `cluster.Synced()` gate; the inflight-headroom term bounds the unlaunched set so `Synced()=false` windows stay $<<$ tick (worst-case pricing at the component recap's config row)]
...
+ *Config* — `sla.{tiers, defaultTier, defaultDisk, probe, featureProbes, maxCores, maxMem, maxDisk}` in scheduler config (`headroom` is the §Coupled-model formula, not a key; the per-build log budget is `const LOG_BUDGET_BYTES`, no CRD/config exposure; there is no FUSE cache budget --- the castore caches are node-level, owned by rio-mountd, outside per-pod ephemeral-storage accounting since ADR-022 P0560); plumbed via helm values. Tiers accept envelope form from day one; phase 2 reads only the `p90` entry until phase 12 lands.
...
+ #set enum(numbering: n => [13#"abcdefgh".at(n - 1).])
  ...
  + *Forecast provisioning (controller)* — \~3.5–4.5k LoC Rust + \~400 nix all-in, including `rio-lease` extraction.
    ...
    *Config:* `sla.{maxLeadTime, maxConsolidationTime, nodeClassRef}`, `karpenter.nodeclaimPool.maxInflightUnlaunched` (+ the DEPRECATED-IGNORED `maxNodeClaimsPerCellPerTick` row, retained parse-only — live_049 L1), with config-load assertion `maxCores < 1024`. The worst-case create burst is the THREE-TERM bound: `min(⌊remaining-budget/min-chunk⌋, max_inflight_unlaunched − |{live: Launched≠True ∧ ¬registered ∧ ¬terminating}| − created_this_tick)` creates per tick across all classes (illustration: `⌊30000/191⌋ ≈ 157` at the launchability-grounded anchor chunk would be the budget arm; the headroom arm caps it at ≤50 — ≈2.5-5s of `Synced()=false` per tick at the observed ~10-20/s CreateFleet throughput). This WAS THE L1-era R17 pricing (absorbed by the controller's kube-client QPS posture and Karpenter's CreateFleet batching + cloud-provider backoff) and it DID NOT account for Karpenter's cross-pool `cluster.Synced()` gate (sh-043; aws/karpenter-provider-aws#7428): the inflight-headroom term bounds the unlaunched set so `Synced()=false` windows stay ≪ tick.
...
#r("sched.sla.reactive-floor+8")[
  ...
  Every NON-SUCCESS close MUST call `observe_peaks` (worker-reported via
  `CompletionReport.final_resources`; controller-witnessed via
  `last_reported_peaks` when a heartbeat landed --- the witnessed lane now
  CARRIES `cpu_seconds`): for each
  axis the report carries, set the dimension to `min(max(floor, peak ×
  headroom(reason, axis)), ceiling)` where `headroom = 2.0` if a corroborated
  hard event fired on that axis (mem $arrow.r$ `CgroupOom`/witnessed-OOM with
  `peak >= assigned/2`; disk $arrow.r$ `DiskFull`/witnessed-emptyDir with
  `peak >= overlay(assigned, H_MIN)/2`; deadline $arrow.r$ `TimedOut` with
  `wall >= assigned_deadline/2`; cores $arrow.r$ `cpu_util >= threshold` jumps
  to `prov_max`), else `1.2` (mem/disk soft) or `1.0` (deadline --- no soft
  headroom). Success skips: `record_build_sample` already feeds the SLA fit's
  `p90 × headroom`, which dominates the soft floor at the solve. The CEILING
  band (`peak <= assigned × TRUST_BAND_MEM`; disk `peak <= overlay(assigned,
  H_MAX) + slack`) refuses forged-HIGH peaks; the FLOOR band degrades a
  forged-LOW hard claim to soft. `last_intent` is the corroboration anchor
