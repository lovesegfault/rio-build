...
// `none` = ref to a future/aspirational location; renders as plain mono.
#let gh-sha = "538458a5b57affcae7f660ef8ce9d3c1c5a0b23c"
#let gh-base = "https://github.com/lovesegfault/rio-build/blob/" + gh-sha + "/"
#let src-map = (
  "translate.rs:452": gh-base + "rio-gateway/src/translate.rs#L452",
  "executor/mod.rs:791": gh-base + "rio-builder/src/executor/mod.rs#L791",
  "values.yaml:506": gh-base + "infra/helm/rio-build/values.yaml#L506",
  "values.yaml:483": gh-base + "infra/helm/rio-build/values.yaml#L483",
  "controller.md:266": gh-base + "docs/src/components/controller.md#L266",
  "runtime/setup.rs:95": gh-base + "rio-builder/src/runtime/setup.rs#L95",
  "pool/pod.rs:75": gh-base + "rio-controller/src/reconcilers/pool/pod.rs#L75",
)
...
#memo[Neither the duration-residual $sigma$ range $[0.1, 0.4]$ used throughout this document nor the $sigma_M$ range is *measured*. The duration range is an assumption pending live `build_samples` data; gate (d) in §Implementation Phasing is the empirical check. The 1.25 floor is provisional pending the same data, and the OOM-reactive penalty-bump (§Exploration, last paragraph; spec marker `r[sched.sla.reactive-floor+2]`) is the safety net if it proves too low.]
...
Karpenter provisions across instance generations by spot cost — #gh("values.yaml:506") admits gen 6/7/8, which spans Graviton2→4 within `aarch64-linux` and Milan/Ice Lake/Genoa/Sapphire Rapids within `x86_64-linux`. Phoronix LLVM-compile benchmarks @phoronix2024 @openbenchmarking2024 show a *#mul(1.89) wall-clock spread* across that Graviton range and #mul(1.31) across same-gen x86 vendors — #mul[2–6] the model's $tilde.op #dur-margin-pct%$ duration margin ($e^(z_0.9 dot.op 0.1) - 1$, the p90-to-median gap at the $sigma = 0.1$ floor of the §Duration distribution range). Pooling samples without correction makes $T(c)$ systematically wrong on the slowest hardware Karpenter picks. Under a global-DB deployment the spread only widens.
...
*`hw_perf_factors{hw_class → (alu, membw, ioseq)}`* is a PG table seeded from OpenBenchmarking and *self-calibrating*. The builder _supervisor_ — not the untrusted build payload — runs three single-threaded probes at init before accepting any assignment, freeing the working set after (#gh("runtime/setup.rs:95")):
...
The controller maintains *two DDSketches @masson2019 per cell*: $z = "boot" - "eta_error"$ for `lead_time`, and `boot` alone for the @ice timeout and the consolidation break-even. DDSketch is relative-error-bounded, mergeable across replicas, handles the spot fleet-retry tail, and is already a transitive dep via `metrics-util`; the `sketches-ddsketch` crate's `negative_store` handles the signed $z$. To track regime shifts — boot distribution changes on AMI rollout, `eta_error` shrinks as fits mature — a *sliding pair* of sketches is kept per cell: the active sketch and a half-life-old shadow. On each half-life boundary, active replaces shadow and a fresh sketch becomes active, so the effective window is one half-life. Sketches serialize to @pg as `BYTEA` via `use_serde` + bincode, prefixed with a `u32` schema-version tag so a sketch-config change deserializes to seed-fallback rather than silently wrong quantiles, alongside `hw_cost_factors`.
...
SLI `rio_controller_forecast_warm_hit_ratio{h, cap}` is the closed-loop check with a Schmitt deadband, the same $0.85 times$/$1.05 times$ as §Tier reassignment. A sustained dip below $0.85 dot.op "target"$ widens the quantile by $Delta q = 0.02$ per Schmitt firing, capped at $q <= 0.99$ and `lead_time` $<=$ `sla.maxLeadTime`. `rio_controller_lead_time_at_cap{h,cap}` distinguishes "still adapting" from "structurally cannot cover": a fat-tailed `eta_error` the model cannot cover would otherwise ratchet $q -> 1$ and idle-cost unbounded. A sustained-high above $1.05 dot.op "target"$ narrows it back, so the estimator does not ratchet.
...
#algorithm(
  ...
  + *function* #smallcaps[ReconcilePool]$("intents", "live")$ #rann[`intents = ⊥` on RPC error — distinct from $emptyset$]
    ...
    + *for each* $(h, "cap")$ minus @ice\-masked *do*
      ...
      + $N <- min(1 + ceil(max(0, sum_U i.c^* - "anchor.cores") slash "bulk.cores"), "sla.maxNodeClaimsPerCellPerTick", 1 + floor(max(0, "budget" - "anchor.cores") slash "bulk.cores"))$ #rann[$>= 1$; mem/disk under-coverage from $sum c^*$-only $N$ self-corrects next tick]
      + create NodeClaims: $1 times "anchor"$ then $(N - 1) times "bulk"$; `created_this_tick += anchor.cores + (N-1)·bulk.cores` #rann[core-waste $<= "anchor.cores" + "bulk.cores"$; per-tick cap defends `CreateFleet` rate-limit cascade]
    ...
    + *for each* $n in "live"$, $n$ NodeClaim-backed, with kubelet `NotReady` for $> 2 dot.op q_0.9 ("boot"["cell"(n)])$, *or* $n in$ `GetSpawnIntentsResponse.dead_nodes` (scheduler-computed: nodes where $>= max(3, ceil(0.5 dot.op "occupancy"(n)))$ executors *across $>= 2$ tenants* had stale heartbeat at removal within the last $5 dot.op "JOB_REQUEUE"$; reset when all live executors on $n$ heartbeat) — *and* the controller requires kubelet `Ready` for this arm (so a scheduler↔node partition does not evict a healthy node), *and* per-tick fleet-wide reaping is capped at $min(3, ceil(0.05 dot.op |"live"|))$ nodes (so a scheduler-side blip cannot cascade) — the scheduler owns both per-pod heartbeat state and the executor-removal tombstone, so the count is observable there; controller acts on the report) *do* cordon, evict, delete $n$ #rann[Karpenter disruption gated; rio owns health incl. kubelet-up-runtime-hung]
    + #text(
        size: 0.85em,
...
- `rio_scheduler_sla_prediction_ratio{dim=wall|mem}` — histogram of `actual / predicted`. Sustained skew off 1.0 is the *model-drift* alert; this is the "silently wrong" signal.
- `rio_scheduler_sla_residual_multimodal_total{tenant}` — incremented when Hartigan's dip test (a unimodality test @hartigan1985) yields $p < 0.05$ on log-residuals (often an upstream cache-hit/miss split, not something the model should fit). Exception-path only; pname identity is in the span.
- `rio_scheduler_sla_resize_retry_total{reason=oom|enospc}` — penalty-bump retries; `sla_prediction_ratio` is blind to censored samples so this is the under-provisioning signal.
...
- `rio_scheduler_sla_infeasible_total{tenant, reason=serial_floor|mem_ceiling|disk_ceiling|core_ceiling|interrupt_runaway|capacity_exhausted}`
- `rio_scheduler_sla_{suspicious_scaling,outlier_rejected,hw_cost_unknown,mem_fit_weak,als_cap_hit}_total{tenant}`; `rio_controller_ddsketch_seed_fallback_total{h,cap}`; `rio_scheduler_sla_hw_ladder_exhausted_total{tenant,exit}`
- `rio_scheduler_sla_prior_divergence{param}` (gauge), `rio_scheduler_sla_hw_cost_stale_seconds` (gauge)
...
- *Positive:* Infeasibility is explicit. `rio_scheduler_sla_infeasible_total{tenant,reason}` surfaces builds the SLA can't cover; `rio-cli sla explain` shows which pname and whether a `--mem` override would help.
...
- *Positive:* Bounded blast radius — a bad fit affects one `(pname, system, tenant)` key. `rio-cli sla reset <pname>` clears it; `BuilderPool.spec.sizing: Static` opts out entirely.
...
+ *Telemetry* — persist `cpu_limit_cores`, `peak_cpu_cores`, `cpu_seconds_total`, `peak_disk_bytes`, `peak_io_pressure_pct`, `version`, `tenant`, `hw_class`, `enable_parallel_building`, `prefer_local_build` in `build_samples`. cgroup already reads `usage_usec`; project-quota read from kubelet's emptyDir quota ID and `io.pressure` parsed alongside; drv attrs at #gh("translate.rs:452"); `hw_class` via the controller's pod annotation read through the downward-API (#gh("controller.md:266"); the builder reports it directly, no cross-process Node-label lookup). *$c^*$ plumbed to `wopSetOptions.buildCores`* so `NIX_BUILD_CORES` $= c^*$ inside the sandbox. *$D$ emitted as `resources.requests.ephemeral-storage`* (closes pre-existing wiring gap at #gh("pool/pod.rs:75")). Migration and bind columns.
...
+ *Config* — `sla.{tiers, defaultTier, defaultDisk, probe, featureProbes, maxCores, maxMem, maxDisk}` in scheduler config (`headroom` is the §Coupled-model formula, not a key; `fuseCacheBudget`/`logBudget` are `BuilderPool` CRD fields, controller-side); plumbed via helm values. Tiers accept envelope form from day one; phase 2 reads only the `p90` entry until phase 12 lands.
...
+ *Surfacing* — `rio_scheduler_sla_infeasible_total{tenant,reason}` and `rio_scheduler_sla_prior_divergence{param}` metrics; `rio-cli sla status [pname]` (learned $S$, $P$, $macron(p)$, $Q$, $M(c)$ params, tier, sample count) and `rio-cli sla defaults` (configured vs fleet-learned vs active prior).
...
+ #set enum(numbering: n => [13#"abcdefgh".at(n - 1).])
  ...
  + *Forecast provisioning (controller)* — \~3.5–4.5k LoC Rust + \~400 nix all-in, including `rio-lease` extraction.
    ...
    *Config:* `sla.{maxLeadTime, maxConsolidationTime, consolidateExploreEpsilon, maxNodeClaimsPerCellPerTick, nodeClassRef}`, with config-load assertion `maxCores < 1024`. The implicit global create rate is `maxNodeClaimsPerCellPerTick × |cells|`; EC2 `RequestLimitExceeded` above that is absorbed by Karpenter's cloud-provider backoff.

    *Controller:*
    - *Lease-elected.* Extract `rio_scheduler::lease` to a shared `rio-lease` crate and reuse; `kube-rs` has no `runtime::leader_election` module @kube-rs-485, and @alg-pool's NodeClaim-create + DDSketch-persist are not idempotent under concurrent execution.
    - `nodeclaim_pool` reconciler: NodeClaim CRD bindings via `kube::CustomResource`; FFD sim; anchor+bulk pack capped at `sla.maxNodeClaimsPerCellPerTick` and standing `sla.maxFleetCores`; per-cell sliding DDSketch pair → @pg via version-tagged `bincode = { version = "2", features = ["serde"] }` (new dep — `sketches-ddsketch`'s `use_serde` provides `serde::{Serialize,Deserialize}`, not bincode-2 native; encode via `bincode::serde::encode_to_vec`); Nelson–Aalen consolidation with $epsilon$-hold-open; $bot$-ticks-then-consolidate-only; unhealthy-node reaping.
    ...
    *Proto + RPC:*
    - `GetSpawnIntentsResponse.dead_nodes` for the runtime-hung-node detector — scheduler-side accumulation from executor-removal tombstones, scheduler→controller on the existing intent-poll reply. `AckSpawnedIntents` is controller→scheduler with `Empty` return.
    - `AckSpawnedIntentsRequest` gains `unfulfillable_cells` (new proto field).

    *Metrics:* `rio_controller_nodeclaim_{created,deleted,ice}_total{h,cap,reason}`, `placement_sim_mismatch_total`, `rio_controller_forecast_warm_hit_ratio{h,cap}`, `rio_controller_lead_time_at_cap{h,cap}`.
