...
+ *Config* — `sla.{tiers, defaultTier, defaultDisk, probe, featureProbes, maxCores, maxMem, maxDisk}` in scheduler config (`headroom` is the §Coupled-model formula, not a key; the node-level castore cache is sized by `services.rio.eksNode.varRioSize`, not per-pod; the per-build log budget is `const LOG_BUDGET_BYTES`, no CRD/config exposure); plumbed via helm values. Tiers accept envelope form from day one; phase 2 reads only the `p90` entry until phase 12 lands.
