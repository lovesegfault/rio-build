...
+ #set enum(numbering: n => [13#"abcdefgh".at(n - 1).])
  ...
  + *Forecast provisioning (controller)* — \~3.5–4.5k LoC Rust + \~400 nix all-in, including `rio-lease` extraction.
    ...
    *Controller:*
    ...
    - `nodeclaim_pool` reconciler: NodeClaim CRD bindings via `kube::CustomResource`; FFD sim; anchor+bulk pack capped at `sla.maxNodeClaimsPerCellPerTick` and standing `sla.maxFleetCores`; per-cell sliding HdrHistogram pair → @pg as version-tagged HdrHistogram-V2 `BYTEA` (`hdrhistogram` crate, `serialization` feature); Nelson–Aalen consolidation with $epsilon$-hold-open; $bot$-ticks-then-consolidate-only; unhealthy-node reaping.
