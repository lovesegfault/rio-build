...
| bug_042 + bug_112 (delete-outcome consumer law, round-11 WO-S4-1) | S + formal | `tombstoneConsequenceApplied`: falsified as-built (`nodeclaimLifecycleTombstoneAsBuilt`, seed 0x8928a5e4724159f3 (20k-sample sim, <1s; re-rolled at the branching bound)) / HOLD (`nodeclaimLifecycleTombstone`) + `canReachReapAmbiguous` + `canReachTombstoneConfirmed` witnesses; Rust reds W11-AF/W11-AG/W11-AH (the lane census + the sweep product census); divergence-header grammar minted at the model (`MODEL-DIVERGENCE(law=ctrl.pool.delete-outcome…)`: the consequence payload — counters, wedge feed, NA samples — is the Rust battery's, not the model's) |
| merged_bug_047 (held demand-visibility, round-11 WO-S4-4) | S + formal | `heldPlacementDemandVisible`: falsified as-built (`spawnCoherenceHeldAsBuilt`, seed 0x98e958c07c2b24ff (20k-sample sim, <1s) — the laundered-absence trace: the fold strips the placed-in-flight held intent, the Complete witness survives, the orphan arm deletes the wanted Job) / HOLD (`spawnCoherenceHeld`) + `canReachHeldKept` witness; Rust reds W11-AK (unit + end-to-end through reap_stale_for_intents + the lossy-narrowing witness-degradation belt); divergence header minted (`MODEL-DIVERGENCE(law=ctrl.pool.window-visibility…)`: the PageNarrowing API and queued arithmetic are the rust battery's) |
| bug_043 (fold-clock denomination, round-11 WO-S4-2) | S + formal | `iceMarkSoundness` widened by the `ourCommit` ground-truth axis: falsified as-built (`nodeclaimLifecycleTombstoneAsBuilt`, seed 0xab34ccedf18c6100 (20k-sample sim, <1s; re-rolled at the branching bound); the deterministic `w11aiFoldlessWindowDropRed` run pins the trace — stamp → 2 foldless ⊥ ticks age the wall TTL past grace → finalizer completes → consolidate-only fold prunes BEFORE the consult and mints the false vanish mark) / HOLD (`nodeclaimLifecycleTombstone` + the `w11aiFoldlessWindowSurvivesGreen` twin run) ; Rust red W11-AI (both skip paths: failed-LIST window + ⊥-tick window, end-to-end through the real tick bodies) |
