...
| `ReportOutcomeRequest.materialization_outcome` | `MaterializationOutcome materialization_outcome = 3` | Set INSTEAD of `report` (message-typed presence; a request carrying both is rejected); oneof `Success{ingested_paths 1, verified_paths 2}` (1) / `Unobtainable{missing_paths 1, verified_paths 2, cause 3, missing_reference_paths 4}` (2) / `InfraFailure{detail 1}` (3) / `RetryLater{detail 1, retry_after_secs 2, class 3}` (4) |
| `Unobtainable.missing_reference_paths` | `repeated string = 4` (A4, bughunt wave) | REFERENCE-closure misses (narinfo extensions), partitioned from the live-wanted `missing_paths` cell (merged_bug_193); old-store reports leave it empty and the consumer's skew partition attributes unattributable entries (outside expected ∪ carried ∪ live-wanted) to it — proto3-additive, scheduler-before-store rollout |
| `MaterializationOutcome.retry_later` | `RetryLater retry_later = 4` (A4, bughunt wave) | Transient substitution failure (raced placeholder slot / upstream 429, `class` names which): the consumption closes the attempt CHARGE-FREE with a view-only deferral that pull admission reads (park/PD-20/stalled never do). An OLD scheduler decoding this sees `outcome = None` → ack-and-ignore → the attempt re-establishes bounded (proto3-additive back-compat) |
| `MaterializationOutcome.aborted` | `Aborted aborted = 5` (B1-s2, bughunt wave; rows completed by A4 per the §4 second-lander rule) | Worker-side abort (SIGTERM during a store rollout/drain): nothing confirmed either way — the consumption closes the attempt CHARGE-FREE (AD5 parity for the materialization kind, owner default Q3 2026-06-03; no MaterializationInfra row, job back to pending claimable) so routine rollouts never burn the park budget. Old scheduler: InvalidArgument → establishment sweep closes the attempt — bounded skew; scheduler-before-store rollout |
| `BuildResult.store_degraded` | `bool store_degraded = 7` (B1-s2, bughunt wave) | Builder-stamped exemption: the FUSE breaker saw the store degraded during the build — route to UNCHARGED paced requeue, never a build charge. proto3 bool: absence ≡ false BY DESIGN (an old builder never claims the exemption — safe default) |
| `PullAssignmentRequest.resume_exec_id` | `string resume_exec_id = 5` (B1-s2, bughunt wave) | The materialization re-delivery resume token: a same-identity re-pull must present the open attempt's exec id to receive DeliverExisting (the kernel's check_kinded_one_winner_arbitration extension). Empty ≡ absent; build-kind pulls ignore it; old schedulers ignore it (delivery stays gated scheduler-side) — scheduler-before-store rollout |
...
#### Addendum extension — bughunt fix wave (RELOCATED)

The rule-4 / rule-4b amendment record and both owner counter-signatures
moved verbatim to the pull-contract amendment anchor in
`docs/spec/components/scheduler.typ` (adjacent to the claim-resume rule);
the wire-delta rows are carried by the `build_types.proto` field comments.
Relocation per owner directive 2026-06-12; the original block is in git
history.
