...
**Status: CAMPAIGN CLOSED (A5/A6, 2026-06-02).** All four phases are
landed and integrated on `formal-sprint` behind green full gates: Phase A
(additive-dormant; dormancy criteria 1–7 HOLD), Phase B (the flag-on
cutover; the six equivalence criteria HOLD; five product bugs + the
finding-11 design correction fixed red-first), Phase C′ (model completion
+ the §9.3 calibration transfer at 100% — the go/no-go, GO), Phase D′
(the deletion — net −16,075 lines; the store-owned materialization job is
the only substitution mechanism; migration 080). The owner counter-signed
the five standing orchestrator rulings on 2026-06-01 (round-2 final
decision gate); the signature lines are applied at their flags in this
map and in `closure-evidence-invariant-map.md`. The campaign close-out
section at the end of this map carries the final acceptance accounting,
the consolidated deployment checklist, the follow-up ledger, and the
round-2 program close-out. The stage records below are the permanent
...
| 2 | **FailoverDuo B9 corner** — the predecessor encoding's post-failover stored-union widening violates B9 at the two-builds+failover scope (full alphabet, reproduced; seed `0xffbfc9ac0c85df5b`; transcript in the T-D7.1 commit body and the module scope note). History — production deleted the lossy fallback at D2.3; the corner documents WHY the 062 semantics had to die | Carried into the `closureEvidence.qnt` archive record (row 3); retroactive validation of the replacement |
| 3 | **`closureEvidence.qnt` archive** — deliberately NOT archived at this close-out: the survivors core (A14/A15/A22/B9/B10/L3 + 2 kept pins) soaks one integration first; the archive then carries the survivors core forward and the B9 corner with its seed | Post-soak step, first maintenance pass after the next integration |
| 4 | **Frozen-contract addendum** — `executor-invariant-map.md`'s frozen pull-protocol contract (:2361–2536) records the materialization PullAssignment kind/`executor_instance` addendum in FINAL form | Rides Item I's integration (the pull-contract addendum integration; D′ handoff item 4) |
| 5 | **Item I — decision-5 store scaling** | COMMISSIONED (owner, 2026-06-01): post-D′-integration parallel infra lane, hard pre-wipe anchor, ~3–4 d-e; scope = the backlog gauge + KEDA ScaledObject (three triggers, `open_attempts` re-key) + ComponentScaler CR removal + the vm-substitute-scale decoupling + the controller-map delta entry (harden-store memo §6.2) |
| 6 | **Item S — store ingest progress/stall hardening** | COMMISSIONED (owner, 2026-06-01): post-D′, store-side only (no scheduler changes, no new wire surface), ~2–4 d-e (harden-store memo §6.2) |
| 7 | **Item T — conversion visibility & strictness** | OWNER-DECIDED OBSERVABILITY-FIRST (2026-06-01): the paged conversion counter + alert (~0.5 d) on the intact PD-20/MD-D1 surface; the strictness knob WAITS on alert evidence per harden-store trigger 1 (memo §6.2) |
