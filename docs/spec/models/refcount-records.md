...
| Retiring the as-built `chunkLiveness.qnt` (model-of-record flip) and re-pointing the three surviving `quint-refcount-calib-*` checks at `chunkCollect.qnt` | **Flip executed** — round-13 WO-S9-7 (this row's owner, previously ownerless); calib re-pointing stays deferred to Phase 2 | The STANDING half landed without retiring the encoding (the deferral's concern — discarding the only model of the still-deployable previous release — is honored): `chunkCollect` is the model of record; the `quint-chunk-liveness-*` checks stay wired as HISTORICAL PINS of the prior release's encoding (the demotion comment in `nix/quint.nix`); every verify marker they carried is consolidated at the `chunkCollect` wirings (zero orphaned); the chunkLiveness modules moved from `quint_policy.py`'s P8 grandfather to the `P8_HISTORICAL_PINS` register (the one sanctioned non-annotation exit — the dead protocol is never annotated). The calib re-pointing (re-encoding the three `quint-refcount-calib-*` defect lanes against `chunkCollect`) remains Phase-2 work per the retry campaign's retirement template (preserve non-vacuity anchors when removing checks). |
