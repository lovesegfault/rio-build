...
(* The PG-side faults (MaxClaimFailures skipped claim writes, MaxRestores  *)
(* point-in-time restores) are each survivable alone -- the Lease's        *)
(* transition count is an independent epoch source -- and only collide in  *)
(* conjunction with a deletion (LeaderElectionPgFaults.cfg).               *)
(*                                                                         *)
