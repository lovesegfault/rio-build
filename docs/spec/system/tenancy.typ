...
Each SSH connection is associated with exactly one tenant. See
#rref("gw.auth.tenant-from-key-comment") for how the gateway extracts the
tenant name from the `authorized_keys` entry comment, and
...
#info(title: [Landed])[
  ...
  revocation + K8s ConfigMap mount + SIGHUP hot-swap. See
  #rref("gw.jwt.issue"), #rref("gw.jwt.verify"), #rref("gw.jwt.dual-mode"),
  #rref("sec.jwt.pubkey-mount").
]

Tenant identity is cryptographically bound using signed JWT tokens rather
than plain gRPC metadata:

+ *Token issuance*: When a client authenticates via SSH, the gateway issues
  a short-lived JWT containing:
  - `sub`: the `tenant_id`
...
+ *Propagation*: The signed JWT propagates through all internal gRPC calls
  ...
  `tenant_id` field.
+ *Verification*: All downstream services (scheduler, store, controller)
  verify the JWT signature and expiry before processing any request. An
  invalid or expired token results in `UNAUTHENTICATED` gRPC status.
+ *Key distribution*: The gateway's public verification key is distributed
...
#table(
  ...
  table.header([Table], [Tenant Column], [Isolation Level]),
  ...
  [N:M junction — path is visible to a tenant iff junction row exists],
  ...
  [— (predates junction-table design; migration 002)],

  [`realisations`], [`tenant_id` (nullable)], [Query-level filtering],

  [`assignments`],
  [_(none)_],
  [Implicit via `derivation_id` FK — no direct `tenant_id` column; tenant
    scope is derived by joining to `derivations`],
)

Query-level filtering ensures tenants can only see their own builds and
...
#info(title: [Implemented])[
  ...
  the mark CTE (#rref("store.gc.tenant-retention")).
]
...
Over-quota tenants receive `STDERR_ERROR` with current usage / limit; the
SSH connection stays open so the user can GC and retry. Enforcement is
eventually-consistent (30s TTL cache on `tenant_store_bytes`); a few MB of
race-window overflow is acceptable.

`NULL` `gc_max_store_bytes` (the default) disables the gate for that tenant.
Single-tenant mode (empty `authorized_keys` comment) never quota-checks.
...
#table(
  ...
  align: (left, left, left),
  table.header([Feature], [Plan], [Status]),
  [`tenant_id` columns + FK], [migration 009], [done],
  [`path_tenants` junction table], [P0206], [done],
  [Per-tenant GC retention], [P0207], [done],
  [Quota reject at SubmitBuild], [P0255], [done],
  [Per-tenant signing keys], [P0256], [done],
  [JWT issuance + verify middleware], [P0257–P0260], [done],
  [Per-tenant narinfo filter], [P0272], [done],
  [`chunk_tenants` per-tenant scope], [migration 018 + P0264], [done],
)

#warning[
  Multi-tenant safety depends on the JWT verify interceptor being LIVE
  (pubkey mounted and non-`None` in scheduler/store `main.rs`). With the
  interceptor inert, downstream services do not validate tenant identity and
  query-level filtering is bypassable. Do not expose a deployment with inert
  JWT verification to untrusted tenants.
]
