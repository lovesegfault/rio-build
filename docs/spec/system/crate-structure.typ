...
#let _crate-prose = (
  ...
  "rio-retry-kernel": [
    Dependency-free home of the scheduler's retry/poison decision kernels: the reference fold over the observed failure history, `decide()`/`classify()`/`placeable()`, their kani `ensures` contracts and the six CBMC proof harnesses. Extracted from `rio-scheduler/src/retry_policy.rs` (which remains as the projection shim and re-export surface) so the harnesses' goto model closes over this crate alone; the CBMC verification runs as the merge-gated `kani-rio-retry-kernel` check in `nix/kani.nix` (seven harnesses over a bounded `cfg(kani)` set representation; the harness inventory and its rationale live in `nix/kani.nix`). The executor-identity type is a parameter: production instantiates `String`/`ExecutorId`, the harnesses a small copy type. Must not grow dependencies; it is excluded from hakari's workspace-hack unification.
  ],
  "rio-log-kernel": [
    Dependency-free home of the store's log chunk-range/dedup/gate decision kernels: the read-path overlap-dedup walk (`visit_chunk`), the ingest accept verdict (`accept_verdict`), the completeness fold (`manifest_covers_contiguously`), the cutter's contiguous-prefix rule (`contiguous_prefix_len`), their kani contracts and the five CBMC proof harnesses. Extracted from `rio-store/src/logs/kernel.rs` (the `logs` module re-exports it as `rio_store::logs::kernel`, so store-side imports are unchanged) so the harnesses' goto model closes over this crate alone instead of rio-store's full artifact context; the CBMC verification runs as the merge-gated `kani-rio-log-kernel` check in `nix/kani.nix`. Each kernel parallels a `pure def` in `docs/spec/models/logService.qnt`. Must not grow dependencies; it is excluded from hakari's workspace-hack unification.
  ],
  "rio-test-support": [
