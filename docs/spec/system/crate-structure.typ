...
#let _crate-prose = (
  ...
  "rio-retry-kernel": [
    Dependency-free home of the scheduler's retry/poison decision kernels: the reference fold over the observed failure history, `decide()`/`classify()`/`placeable()`, their kani `ensures` contracts and the six CBMC proof harnesses. Extracted from `rio-scheduler/src/retry_policy.rs` (which remains as the projection shim and re-export surface) so the harnesses' goto model closes over this crate alone; the CBMC verification itself is the manual `kani-rio-retry-kernel` target in `nix/kani.nix` (not yet merge-gated — see the entry's note and the retry invariant map's assurance layer for what still blocks that). The executor-identity type is a parameter: production instantiates `String`/`ExecutorId`, the harnesses a small copy type. Must not grow dependencies; it is excluded from hakari's workspace-hack unification.
  ],
  "rio-test-support": [
