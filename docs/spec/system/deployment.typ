...
- *Recovery procedure:* Restore PostgreSQL from backup, verify S3 bucket accessibility, restart all components. Executors reconnect and re-register.
  ...
  #info[
    *State recovery (Phase 3b):* On `LeaderAcquired` (lease acquisition), the scheduler calls `recover_from_pg` which rebuilds the in-memory @dag from PostgreSQL: loads non-terminal builds + derivations + edges + build_derivations, reconstructs `DerivationState` via `from_recovery_row`, recomputes critical-path priorities, repopulates the ready queue. The lease loop fire-and-forgets `LeaderAcquired` (non-blocking — keeps renewing during recovery); `recovery_complete` flag gates dispatch. If the DAG load fails but the PG floor is still readable, the term continues with an empty DAG yet is still floored, claimed, and (when required) confirmed before dispatch is ungated; only in the floor-unreadable fallback (PG cannot answer even the floor query) does the scheduler complete at the recovery-entry generation (degrade to pre-recovery behavior, don't block). The recovery generation target starts from the generation at recovery entry — the Lease-derived generation (`leaseTransitions + 1`) unless an earlier recovery's PG-floor seed already raised it — and is raised to one past the durable PG floor `GREATEST(MAX(assignments.generation), MAX(leader_generation_claims.generation))` only when that floor exceeds the entry generation, or ties it without this holder's own claim row in the ledger (a same-epoch re-acquire retains its generation); the target is durably claimed in the `leader_generation_claims` ledger (#rref("sched.lease.generation-claim")) and applied via `fetch_max` (#rref("sched.recovery.fetch-max-seed")) before dispatch is ungated. A PostgreSQL restore from backup regresses both arms of that floor; the Lease's transition count is the redundant epoch source in that case, and only the conjunction of a PG fault (skipped claim write or restore) with a Lease deletion — both epoch sources destroyed — reaches the documented residual collision. See `rio-scheduler/src/actor/recovery.rs`.
  ]
