...
- HMAC signing key for assignment tokens (scheduler, store) --- set via `RIO_HMAC_KEY_PATH` on both. The scheduler signs Claims{executor_id, drv_hash, expected_outputs, is_ca, is_fixed_output, tenant, expiry_unix} at dispatch (the optional fields use serde defaults, and `is_fixed_output` is only emitted when `scheduler.signFodClaims` is armed); the store verifies on `PutPath`. Same key file both sides (shared secret). Generate: `openssl rand -out /path/to/key 32`.
...
- *Assignment-claims fields (`sign_fod_claims` two-phase rollout):* `AssignmentClaims` is `deny_unknown_fields`, so a scheduler must never emit a claims field the store fleet cannot parse. New fields ship reader-first (serde defaults) and writer-second. For `is_fixed_output` the writer is gated by the scheduler config knob `sign_fod_claims` (helm `scheduler.signFodClaims`, default off): *Phase 1* deploys the whole release with the knob off — tokens keep the pre-field wire shape, and descriptor-less fixed-output uploads remain membership-checked as before. *Phase 2* arms the knob (or flips the chart default) only once (a) every rio-store replica runs this release and (b) every builder/fetcher worker image — including warm Pool pods — records `fixed:` content-address descriptors on fixed-output uploads, because the store rejects descriptor-less uploads under a FOD-flagged token (#(refs.metric)("rio_store_hmac_rejected_total") with `reason="fod_descriptor_missing"`). Workers never parse claims, so their axis is the descriptor capability, not the token format. Fresh installs and wipe deploys may arm the knob immediately; the VM-test deployments do.
