...
#r("infra.node.kubelet-prjquota")[
  Every builder-hosting node pool MUST provision XFS project quotas at the kubelet root before kubelet starts, INCLUDING the kubelet half of the chain: `/var/lib/kubelet` mounted `-o prjquota` (instance-store NVMe via `rio-nvme-mount`; EBS-only classes via `rio-ebs-quota-mount` over the dedicated quota volume the `EC2NodeClass` attaches as its second mapping), the `LocalStorageCapacityIsolationFSQuotaMonitoring` feature gate enabled, the `/etc/projects` + `/etc/projid` registry files present — kubelet locks both and silently reports quotas unsupported when either is missing — and the fixed-path quota commands kubelet execs reachable (`/sbin/xfs_quota`, `/usr/bin/lsattr`; FHS shims on NixOS — the witness-derived third silent-decline mode). A builder node without the full chain has a dead `peak_disk_bytes` producer (live_060: 2022/2022 completions `None` on the EBS-only fleet), so the EBS mount unit is a hard kubelet requirement: missing quota volume means NotReady, never Ready-with-a-dead-producer.
]
...
- *Instance type:* Compute-optimized (e.g., `c8a.xlarge` on AWS). For IO-heavy builds the `rio-builder-nvme-{x86,aarch64}` NodePools select instance-store-NVMe families via `karpenter.k8s.aws/instance-local-nvme > 0`; the AMI's `rio-nvme-mount.service` (early boot, before tmpfiles and nodeadm) stripes the instance-store devices into `/dev/md0`, formats XFS, and mounts at `/var/lib/kubelet` with `prjquota` so kubelet enforces per-pod ephemeral-storage via XFS project quotas; EBS-only classes (`rio-default`/`rio-metal`) get the same kubelet-root contract from `rio-ebs-quota-mount.service` over the dedicated quota volume (live_060 — see the prjquota rule above). The `rio-nvme` EC2NodeClass sets `instanceStorePolicy: RAID0` so @karpenter's bin-pack simulation counts NVMe capacity toward ephemeral-storage; nodeadm does not act on it (the AMI's `nodeadm init --skip run` never executes the local-disk aspect), so `rio-nvme-mount.service` still owns the mdadm→mkfs.xfs+prjquota chain uncontested.
