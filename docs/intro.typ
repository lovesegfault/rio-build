...
- *Hydra* is a monolithic Perl/C++ application with a single-machine evaluator,
  round-robin scheduling, and aging Perl web UI. It doesn't scale horizontally
  and has poor observability.
- *Remote builders* (`nix.buildMachines`) use SSH-based 1:1 delegation with no
  global scheduling, no work-stealing, no awareness of the full DAG, and no
...
(`ssh-ng://` remote store and `--builders` remote builder) as a
Kubernetes-native distributed build backend. From Nix's perspective, rio-build
_is_ a Nix store and builder. From Kubernetes' perspective, it's an operator
that schedules build DAGs across builder pods with intelligent caching.

= Key Innovations

+ *Protocol-native* --- speaks Nix's own wire protocol, zero custom client
  tooling needed
...
rio-build is a *build execution backend*, not a complete CI/CD system. The
following are explicitly out of scope:

- *Nix evaluation*: rio-build receives derivations via the protocol. It does
  not evaluate flakes, channels, or Nix expressions. Evaluation is performed by
  the client (e.g., `nix build`) or an external eval orchestrator (e.g.,
  `nix-eval-jobs`).
- *VCS integration*: No GitHub/GitLab webhook receiver, no PR status reporting,
  no commit status updates. Use an external CI system (GitHub Actions,
  Buildkite, etc.) to trigger builds via `nix build --store ssh-ng://rio`.
- *Notification hooks*: No email, Slack, or webhook notifications on build
...
- *Eval scheduling / jobsets*: No equivalent of Hydra's jobset model. For
  ...
  #link("./integration.md")[Integration] for patterns.
...
- *Recursive Nix*: Derivations that invoke Nix internally (`__recursive` /
  ...
  #link("./components/builder.md#builder-nix-configuration")[builder configuration].
...
- You have a *single project with moderate build volume* --- use Cachix +
  GitHub Actions
...
See #link("./multi-tenancy.md")[Multi-Tenancy] for tenant isolation details:
...
#table(
  columns: 3,
  table.header([Tool], [Category], [Relationship to rio-build]),
  [*Hydra*],
  [Eval + build + cache],
  [rio-build replaces the build execution and binary cache layers; an external
    eval orchestrator replaces the eval layer],

  [*Tvix*],
  [Nix reimplementation],
  [Influenced rio-store's CAS design (MIT protobuf refs). Different goals: tvix
    is a Nix replacement, rio-build is a build backend],

  [*Attic*],
  [Binary cache],
  [rio-store's chunked CAS overlaps Attic's storage design. rio does not serve
    the HTTP binary-cache protocol; if you only need a substituter, use Attic],

  [*Cachix / Garnix*],
  [Managed CI/cache],
  [SaaS alternatives. rio-build is for self-hosted deployments],

  [*GitHub Actions + Nix*],
  [CI with Nix],
  [The "wrap existing CI" approach. rio-build offers better scheduling but
    higher operational cost],

  [*ofborg*],
  [Nixpkgs CI],
  [Specialized for nixpkgs PR review. rio-build could serve as ofborg's build
    execution backend],
)
