...
| Client upload, streamed | Embedded and relayed paths ≥ `large_nar_threshold_mib`. Relay entries are admitted only past a plan-time plausibility screen on the peer-declared `NarSize` (64 GiB cap): an over-cap entry is skipped per path with a reason naming the cap, so a fabricated size can neither steer lane routing nor parameterize the wire op. | One `client_add_to_store_nar` per path with `NarPayload::Reader` streaming straight from the archive or the relay fetch; deadline = `LARGE_UPLOAD_TIMEOUT` (600 s) base plus payload-proportional headroom at a 1 MiB/s floor, clamped at a 4 h ceiling because the relay arm's payload length is the cache's own declared size; fresh channel after any error. | Constant memory per transfer. |
