...
- **Relay failures degrade per path** (`failed`), and dependents that cannot proceed take `supply-failed`. A circuit breaker (threshold `max(2 × upload_workers, 6)` consecutive transport failures — failed channel opens and mid-upload wire deaths alike; a clean refusal resets the count, because the completed wire op proves the gateway answered — latched) stops dialing a gone gateway and fails remaining planned uploads fast; the watchdog's pause/backpressure machinery then takes over.
