...
- **An SSH transport** (`replay/client.rs`): an in-process russh connection pool with the gateway's 4-channels-per-connection budget, fail-closed host-key policy, lazy re-dial, and per-op deadlines.
...
- **N7.** Archive management tooling beyond what campaigns need (no prune/garbage-collection helpers in this design).
...
| **Replayer** / **replay engine** | The single in-cluster program (today the `rio-parity` run subsystem) that executes campaigns over archives (§6 The replayer). |
...
Naming: the subsystem as a whole is called **build replay**. The term "parity" survives in two places: as the name of the parity report policy (the headline-percentage aggregation), and in legacy implementation and deployment identifiers that are deliberately retained — the `rio-parity` crate and module paths (§6.2), the `rio-parity` namespace, Secret and mount names (§10.2), the `parity` S3 root and `parity-*` tenant names — until Phase 5, whose operator-convergence cutover renames them all to the neutral name set defined in §11.7 (crate `rio-replay`, S3 root `replay/`, namespace `rio-replay`, …). It no longer names the archive format, the comparison vocabulary, the operator command family, or any concept a recorder or report consumer sees; after Phase 5 it names only the report policy.
...
- `<root>` is the deployment's configured S3 prefix. It is configuration, not format; deployments keep `parity` (the existing IRSA grant `parity/*`) until Phase 5, when the cutover in §11.7 renames the root to `replay` and updates the IAM/tofu grant alongside the other identifier renames. The campaign-state prefix `<root>/campaigns/<campaign-id>/` keeps its shape under either root.
...
Today's eval-set machinery (`rio-parity eval`, the xtask `parity eval` Job) becomes the first-party recorder for nixpkgs/Hydra-derived archives. Its job description changes from "produce an eval set the campaign engine knows how to interpret" to "produce a v1 replay archive with truth baked in".
...
|---|---|---|
| k8s Job runtime, pod sizing, labels/annotations, ServiceAccount/IRSA | `xtask/src/parity/jobs.rs`, `infra/eks/parity.tf` | Unchanged. The campaign Job remains `backoffLimit: 6`, `restartPolicy: OnFailure`, no `activeDeadlineSeconds`. |
| Spec ConfigMap input (`<campaign-id>-spec`, mounted at `/etc/rio/parity/spec.json`) | `xtask/src/parity/launch.rs`, `rio-parity/src/run/spec.rs` | `CampaignSpec` gains an `archive` reference, a `scheduling` block, and a `supply` block; the `hydra` block leaves the campaign spec (§6.8, §7.4). |
| S3 state/artifact sync, append-only JSONL state dir, atomic JSON rewrites, `markers/<stage>.done` | `rio-parity/src/run/state.rs`, `artifact.rs` | Unchanged mechanism. New artifacts: `supply.jsonl`, `dispatch.jsonl` (timed mode), `gate.json`. |
| Resume across pod restarts (`download_state_if_missing`, terminal-record skip, `collected.json`) | `rio-parity/src/run/mod.rs` | Extended for in-band collection and timed mode (§6.7). |
| Watchdog, suspension components (pause/idle/ice/dispatch), stall escalation | `rio-parity/src/run/watchdog.rs` | Unchanged in timeless mode; constrained in timed mode (§9.4). |
| `PAUSE` file, backpressure pause, deadline-partial behavior, exit-code contract (0 on drain and on deadline-partial) | `rio-parity/src/run/mod.rs` | Unchanged in timeless mode; in timed mode PAUSE/backpressure become advisory (recorded as suspension windows and lateness, never a dispatch gate — §9.4). Deadline behavior and the exit-code contract are unchanged in both modes; the regression gate never changes the engine exit code (§7.3). |
| `progress.json` + `report/summary.md` + per-class JSONL report files | `rio-parity/src/run/report.rs` | Field renames per §7.4; structure unchanged. |
| Plan stage: filters, closure-overlap analysis, validity snapshot, `campaign.json` with comparability block | `rio-parity/src/run/plan.rs`, `spec.rs` | Reads closures and expected outcomes from the archive instead of `dep-closure.jsonl` + a campaign-time narinfo sweep. |
| Greedy dual-cap batching, submit loop, requeue/cooldown, fail-fast singleton escalation | `rio-parity/src/run/batch.rs`, `submit.rs` | Becomes the **timeless** scheduling mode (§9.1). |
| Two-signal infra attribution, evidence capture (log tails, NAR identity via `BatchQueryPathInfo`) | `rio-parity/src/run/collect.rs`, `grpc.rs` | Signal sources shift with the transport (§6.6); the rule itself is unchanged. |
| The `Submitter` seam and `FakeSubmitter`/`FakeReader` test scaffolding | `rio-parity/src/run/submitter.rs`, `test_support` | The seam is the pivot point of the transport swap (§6.4). |
| Operator workflow: `xtask parity launch` / `status` / `report`, pre-flight, tenant provisioning, SSH/HMAC secret plumbing | `xtask/src/parity/` | §10 (Operator surface). The QueryDerivationStatuses pre-flight probe and `--require-qds` are removed (§6.5). |

### 6.2 What is absorbed from the xtask-replay branch, and where it lands

The xtask-replay branch contributes the transport, the supply planner, the timed scheduler, and the verdict ideas. Its laptop-side orchestration retires. Mechanism is per row: **cherry-pick** (code applies as-is on its current path), **re-home** (code moves into rio-parity, with the replay-specific assumptions removed), **absorb** (the ideas merge into an existing engine module; the source file is not kept), **drop** (not carried forward).
...
| `xtask/src/k8s/replay/archive.rs` (`ReplayArchive`, Dir/DwarFS backends, NAR dump) | `rio-parity/src/archive/` | re-home | Becomes the v1 archive reader plus a writer used by the recorders (§4, §5). Loses nothing; gains `format_version`/capability handling. |
| `xtask/src/k8s/replay/client.rs` (`GatewayPool`, `DaemonChannel`, `HostKeyPolicy`, `ReplayClientError`) | `rio-parity/src/transport/` | re-home | The kubectl port-forward default goes away; in-cluster the engine dials `cluster.gateway_store_url`'s host:port directly. Host-key policy: `Pinned` when the spec carries `cluster.gateway_host_key`; `launch` populates that field at pre-flight from the gateway host-key Secret named by the chart's `gateway.ssh.hostKeySecret` value (the same Secret the gateway mounts), so pinning is the norm on any deployment with a persistent host key. Accept-and-record (the observed fingerprint is written into `campaign.json` for audit) remains only for deployments running on the chart's auto-generated emptyDir host key — dev/k3s shapes where the key regenerates on pod reschedule and pinning is impossible — and is no worse than today's `StrictHostKeyChecking=no`, but visible. |
| `xtask/src/k8s/replay/substituter.rs` (narinfo probe, streaming NAR fetch + decompression) | `rio-parity/src/substituter.rs` | re-home | Merges with the existing `nixcache.rs` narinfo client; one HTTP/S3 binary-cache client for both recorders and the supply planner. |
| `xtask/src/k8s/replay/supply.rs` + `prewarm.rs` (`workload_set`, `walk_closure`, `resolve_source`, `plan_uploads`, `UploadClaims`, topo levels, batch splitting, circuit breaker) | `rio-parity/src/run/supply.rs` | re-home | Becomes the single supply planner (§8). The existing `warm.rs` is folded in as the scheduler-side prefetch arm; `warm.jsonl` is superseded by `supply.jsonl`. |
| `xtask/src/k8s/replay/timeline.rs` (`ScheduledRequest`, `build_schedule`, FIFO admission, disconnect anchoring, confirmation retries, `InFlightTracker`) | `rio-parity/src/run/timeline.rs` | re-home | Becomes the timed scheduling mode (§9.2). |
| `xtask/src/k8s/replay/compare.rs` (verdict taxonomy, `classify` precedence, `DivergenceLog`) | `rio-parity/src/run/classify.rs` + `model.rs` | absorb | The verdict ideas merge into the layered comparison model (§7); `divergences.jsonl` survives as a per-unit divergence stream next to `results.jsonl`. |
...
| `xtask/src/k8s/replay/mod.rs` (`run_live` laptop orchestration, tunnel handling, `--watch`) | — | drop | The subcommand's roles move to the thin launcher (`launch --archive`) plus a local/dev mode (§10). The 30 s scheduler-metrics `--watch` line is already covered by `xtask parity status --watch`. |
| `xtask/tests/fixtures/replay/basic/` + `basic.dwarfs` | `rio-parity/tests/fixtures/archive/` | re-home | Converted to v1; a v0 copy is retained as the input of the v0-handling test described in §4. The offline dry-run CI test moves with it (§10). |

Dependency consequences: rio-parity gains `russh` (already a workspace dependency), `dwarfs`, `async-compression`, and `tokio-util`; xtask loses its replay-only dependencies once the launcher lands. Phase 2 removes the nix-CLI submission/collection path outright (§11.4); the warm-stage prefetch submission keeps its current shell-out form until it moves onto the client-ops pools with the Phase 4 supply planner, after which the engine image no longer needs the `nix` binary or an `ssh` client for the build path. GNU tar + zstd remain only for the legacy eval-set input (`drvs.tar.zst`) until that path is retired in Phase 5 — v1 archives are read directly from the DwarFS image and need neither.
...
- One submission = one `DaemonChannel` acquired from the `GatewayPool` (4 channels per SSH connection, mirroring the gateway's cap).
...
|---|---|---|---|
| Scheduler-side prefetch ("delegate") | Paths covered by the target's own substituters, when the supply policy wants them present before measurement | The producing derivations of the covered paths are submitted as ordinary roots under the prefetch tenant (`parity-warm`) ahead of execution; the scheduler's existing bulk-substitution path fetches the NARs directly from the upstream into rio-store, attributed to `path_tenants` so they survive GC. This is today's warm stage, unchanged in mechanism. | Bytes never transit the engine pod. Estimated to handle the 1e5-path scale envisioned for platform slices (campaign-design sizing: 1–3 h on 1–2 store replicas); to be confirmed by the M2/M3 warm-stage wall-clock measurements. |
...
Tenant plumbing under the client-ops transport: the engine holds one `GatewayPool` per tenant role — the build tenant for submissions and client uploads, the prefetch tenant (`parity-warm`) for the delegate arm's prefetch submissions — each dialed at `cluster.gateway_store_url` with that tenant's mounted key file (tenant selection is by SSH key, not by URL). This replaces today's separate `cluster.warm_store_url`, which is retired when the prefetch arm moves onto these pools in Phase 4 (build submission itself is already client-ops-only from Phase 2 onwards, §11.4; until Phase 4 the prefetch arm keeps its current nix shell-out submission).
...
The mode/tenant coupling carries over from the parity design: `parity-leaf` pairs with `dependencies: substituters`, `parity-selfhosted` with `dependencies: none`; the prefetch tenant is only provisioned when the policy includes a prefetch arm.
...
- **Interruption replay.** When `replay_interruptions` is true and the archive records a cancellation or client disconnect for a unit of the request, the engine arms an abandon deadline at `dispatch + (recorded stop offset − recorded start offset) / speedup` (default 60 s/`speedup` when no stop offset is recorded, floored at 1 s), anchored at dispatch so supply time does not shift it. The build is always actually submitted (minimum 1 s of build time), then raced against the deadline; if the deadline wins the channel is abandoned — the gateway observes a client disconnect and cancels that session's builds — and the unit classifies `interruption-replayed`. Both recorded cancellations and recorded disconnects are reproduced this way; over ssh-ng they are the same observable client behavior.
...
Mode availability is gated by the archive's declared capabilities (§4), checked at bootstrap and at `xtask parity launch` time:
...
The operator surface stays where it is today: a single `cargo xtask` command family that creates Kubernetes objects and reads S3 artifacts, with all measurement work running in-cluster. Nothing the operator runs locally is ever part of the data plane, with the single, explicitly dev-only exception described in §10.4. Throughout the migration (§11 Migration & sequencing), the existing `cargo xtask parity {eval,launch,status,report}` commands keep their names and behavior unchanged; the surface described here is the converged end state that lands in Phase 5.

### 10.1 Command family

The unified subsystem keeps one xtask family. The family name follows the subsystem name (§3.3); the rename from `parity` to `replay` is part of the Phase 5 cutover (§11.7). This section writes the converged form as `cargo xtask replay`, with today's command shown alongside. After Phase 5 the `parity` word survives only inside the report-policy vocabulary (§7 Comparison model), not in command names.
...
**`record` (today `eval`).** Flag surface unchanged: `--eval <u64>`, `--system` (repeatable, default `x86_64-linux`), `--scope constituents:<agg>|jobs:<list>|jobs-file:<path>|full`, `--jobset`, `--full-scale`, `--force`, `--log-level`. Job naming (`parity-eval-<eval>-<8 hex>`), sizing (scoped 16 cpu/64Gi; `--full-scale` 160 cpu/1200Gi), `ttlSecondsAfterFinished=86400`, the ECR image assertion, and the collide-and-instruct 409 behavior on `--force` are all retained. The only change is the Job's output: instead of the six loose eval-set artifacts it packages a v1 archive with expected outcomes baked in (§4 Archive format v1, §5 Recorders) and uploads it under the archive S3 layout with the completion marker last. The politeness budget, recipe reproduction, and fidelity gate are unchanged and their results land in the archive's provenance block.

**`launch`.** Existing flags survive: `--eval`, `--eval-digest`, `--mode leaf|self-hosted`, `--campaign-id`, `--limit`, `--engine-arg`, `--allow-version-skew`, `--restart-gateway`, `--skip-preflight`, `--log-level`. Existing responsibilities survive in the same order: campaign-id validation, tofu output resolution, ECR image assertion, archive resolution in S3 (the completion marker is the existence test, exactly as `evalset.json` is today), namespace/ServiceAccount ensure, tenant provisioning via rio-cli, per-tenant SSH keys, HMAC Secret copy, pre-flight, spec build + validation, `guard_existing_campaign`, ConfigMap apply, Job apply.
...
**`repro`.** The per-job `repro_command` recorded with every failure-class verdict is engine-native: `cargo xtask replay repro <campaign-id> <drv>` resolves the campaign's pinned archive identity from `campaign.json` (or the campaign's S3 prefix), fetches that archive, and replays the single named unit over the same client-ops transport and supply policy the campaign used, printing the resulting verdict and evidence. The report text may additionally print the equivalent `nix build --store <ssh-ng url> '<drv>^*'` line as a human convenience, but the recorded field is the engine-native invocation — it requires no local archive copy and no local Nix store. (Today's `repro` stub gains this behavior at convergence.)
...
The transport between xtask and the engine is unchanged: a ConfigMap named `<campaign-id>-spec` with key `spec.json`, mounted read-only at `/etc/rio/parity/spec.json` (`SPEC_MOUNT_PATH`), plus the `rio-parity-ssh` Secret (one key file per tenant under `/etc/rio/parity-ssh/<tenant>`) and the copied `rio-service-hmac` Secret at `/etc/rio/hmac/service-hmac.key`. Pod env stays `RUST_LOG`, `AWS_REGION`, `AWS_USE_DUALSTACK_ENDPOINT`, `RIO_PARITY_S3_BUCKET`, `HOME=/work`, `XDG_CACHE_HOME`, `TMPDIR`. Mount paths, Secret names, namespace (`rio-parity`), ServiceAccount, and IRSA wiring keep their current values until Phase 5, when the operator-convergence cutover renames them to the neutral subsystem name set (§11.7) together with the S3 root, crate, and command-family renames.
...
1. **`dev` mode.** `cargo xtask replay dev --archive <path> [--store ssh-ng://…|--provider k3s] [--limit N] [--schedule …]` runs the engine binary locally (no Job, no ConfigMap, `--no-s3`, state dir under the repo's scratch directory) against either an explicit `--store` endpoint or a provider port-forward to `svc/rio-gateway`, reusing the host-key policy ladder from the absorbed transport (pinned key > loopback accept > known_hosts). This is the surviving remnant of the xtask-replay run loop, scoped to fixture- and smoke-sized archives for engine development and k3s cluster bring-up. It is not a measurement surface: no comparability block from a `dev` run is publishable, and the command says so in its output.
...
k3s campaign launches (a real Job on a dev cluster) also remain possible through the normal `launch` path with `--skip-preflight` and the empty-HMAC-Secret behavior that already exists; nothing in the converged design is EKS-only except the IRSA/S3 plumbing that is already optional today (`s3.bucket = None` disables sync).
...
**R4 — Gateway channel-budget coupling.** The client transport mirrors the gateway's per-connection channel cap as a hard-coded constant (4) and sizes connections as ceil(max_sessions/4). There is no wire-level discovery of the cap, and gateway-side per-tenant connection policy can change independently. A silent mismatch degrades into channel-open failures that look like infra noise. The constant is acceptable: per the deployment policy (§2.5) the engine and gateway are deployed together onto dev clusters by full wipe, so a mismatch is a configuration error within one deployment, not a cross-version compatibility scenario, and a wire-advertised value is explicitly not planned. Mitigation: carry the assumed cap in the spec's schedule/transport block, log it at startup, and treat a burst of channel-open refusals as a configuration error rather than per-unit infra verdicts.
...
| tenant | The isolation unit: each tenant has its own substituter ("upstream") list, build policy, GC retention, and path ownership. The campaigns use dedicated tenants (`parity-leaf`, `parity-selfhosted`, `parity-warm` today) so measurement traffic never mixes with other workloads. |
...
| the engine | The `rio-parity` binary (the replayer) running inside the campaign Job: planning, supply, submission, collection, classification, reporting, resume. |
