...
- Integrity at use time is layered: the metadata members are checked against `files`, the narinfo sidecar listing against `content_digests.narinfo`, and the embedded-derivation listing against `content_digests.drvs`, when the archive is opened (sidecars and `.drv` members are small text files; hashing the listings is cheap, and the recomputation also catches a member removed or added after finalize); embedded non-drv paths are checked against their sidecar `NarHash`/`NarSize` when the engine serializes them for upload (and the daemon verifies again on ingest). The `.drv` recomputation at open is load-bearing: although an input-addressed `.drv` store path commits to its ATerm contents, no import-time check recomputes that commitment — the import path reads the embedded bytes raw and derives the upload path-info from them under the recorded store path — so without the open-time check a corrupted-but-parseable ATerm would replay a semantically different derivation with the divergence charged to the wrong side.
...
The session-aware lookup above serves consumers acting for a specific recorded request (the timed scheduler). The timeless engine resolves truth per workload *unit* — it has no request identity to probe with — through one canonical collapse over sessions: the session-less record when one exists (it explicitly applies to any request of the unit), otherwise the scoped record of the highest **informativeness rank**, with ties *within* a rank class resolved by record content (the consumed-truth order: outcome, then recorded output hashes — arbitrary but total) and the highest-numbered session deciding only between records whose consumed truth is identical. Session ids never decide consumed truth — between rank classes or within one: they are opaque grouping keys whose allocation order carries no truth ordering (recorders may allocate them out of capture order, and for overlapping sessions even capture order says nothing — the earlier session's build can complete after the later session's disconnect), so a collapse keyed on them would let a concurrent disconnect discard a recorded build — or let two `built` records with conflicting hashes flip the unit's NAR-comparison baseline — with the choice flipping under session relabeling.
...
Scoped records that disagree with the chosen record's consumed truth — the outcome, or the recorded output hashes of outcome-equal `built` records — are logged, and the per-archive count of conflict-resolved units is surfaced in the report's comparability block (`truthCollapseConflicts`): the collapse is losing information the timeless engine cannot represent, and a campaign whose truth was partly chosen by rank or content order is comparably different from one whose truth was plainly recorded.
...
| S3 state/artifact sync, append-only JSONL state dir, atomic JSON rewrites, `markers/<stage>.done` | `rio-replay/src/run/state.rs`, `artifact.rs` | Unchanged mechanism. New artifacts: `supply.jsonl`, `dispatch.jsonl` (timed mode), `requeues.jsonl` (the engine-resubmission journal the resume path folds the retry-budget counters from), `gate.json`. |
...
- Build: one `client_build_paths_with_results` call with the submission's roots as wire derived paths (`"<drv>!*"`, or `"<drv>!out1,out2"` for explicit outputs; the `^` spelling survives only in the human-convenience nix command line the report may print — the recorded per-job `repro_command` itself becomes engine-native, `cargo xtask replay repro <campaign-id> <drv>`, §10.1). Each timed root's output list is the request's RECORDED selection, threaded from the archive to the submission chokepoint (`BatchRoot::derived_path`); workload roots (timeless waves, canaries) carry no recorded selection and demand `!*`. The recorded list is archive-controlled and unvalidated, and the gateway parser rejects shapes a raw join could emit (duplicate names, empty names, >256 names) while reading a `*` mixed among names as a literal output name — so the formatter normalizes: duplicates dedupe to first occurrence, a `*` member saturates to `!*`, and a wire-inexpressible member (empty, or containing `,`) or an over-cap list collapses the whole selection to `!*` — the widest demand, never an under-ask and never a wire rejection.
...
- **Failure causes:** `FailureKind { Genuine, Infra, Timeout, ResourceCeiling, SourceRot }` is kept as the per-failure cause attribute. SourceRot now requires `is_fixed_output = true` derived from the archive's ATerm plus a fetch-error needle in `error_msg`/log tail — the first time this path can actually trigger (see §6.5). The needle bar has ONE owner (`resolve_failure_kind`) and applies to every door, the DAG-fallback blanket recovery included: a recovered sticky-poison trigger satisfies the bar through its own trigger-keyed log tail, a needle-free tail (a worker-executed fixed-output failure such as a hash mismatch) keeps the genuine charge, and an unfetchable tail keeps it too — the same resolution the evidenced cascade door reaches, so one root cause never classifies opposite ways by batch timing. The blanket recovery additionally REFUSES outright (the genuine charge stands) when the poison snapshot also names an executed non-fixed-output member of the same job's closure: that is a real, retry-exhausted dependency regression blocking the job, so §7.1's "failed only because a fixed-output input could not be fetched" cannot hold.
...
`BuildStatus` values add corroborating texture (`TransientFailure`, `TimedOut`) but never override the two-signal rule; the rule remains the only thing that can move a *failure* out of the workload-charged verdicts. One row class is not a failure report and is classified before the rule runs: the gateway's evidence-loss row (`BuildResult::lost_terminal_unverified` — a root whose own terminal was lost under a *completed* DAG with store presence unverifiable). The DAG completed, so nothing is known to have failed on a worker — there is no failure to excuse, stale poison rows from earlier attempts cannot contradict an evidence-loss report, and the row routes to the auto-retry budget and then `infra-indeterminate` (the "evidence loss" leg of §7.2's definition). Producer and detector share the message-prefix constant, so the match is producer-exact.

The lost-terminal cell with store presence *confirmed* is the same evidence loss wearing a success word: the gateway keeps `Substituted` on the wire (presence is real, and stock clients must keep seeing a plain success — the common event-loss sub-case is an executed root whose outputs uploaded before the loss), which makes the row in-band-indistinguishable from a genuine substitution terminal. The disambiguator is a stderr side channel: the gateway relays the lost-terminal marker line (`rio: terminal lost for '<drv>'`, the shared rio-nix formatter/detector pair — spec rule `gw.opcode.lost-terminal-relay`) on the same relayed-stderr channel the engine already captures for the `rio: build <uuid>` announcement; the capture records marked roots on the batch record (`lostTerminals`), and collect routes a `Substituted` row carrying the marker through the same evidence-loss leg as the unverifiable cell — auto-retry, then `infra-indeterminate` (§7.2) — never to the `target-substituted` disposition, which under a force-build measurement tenant is definitionally impossible and would record a false policy violation (and spuriously fail the smoke-scale "zero `target-substituted` under the leaf tenant" exit criterion, §11.1). An *unmarked* `Substituted` row keeps the substitution-event leg unchanged, so genuine target substitutions stay measurable. The marker is not completed-DAG-only: a *failed keep-going* DAG settles only once every derivation resolved, so a terminal-less confirmed-present root there is the same loss shape (or a merge-seeded node that emitted no event — marking either is the conservative polarity: the marker can only flip that drv's row onto the evidence-loss leg, a bounded gate-visible cost — auto-retry burn, then `infra-indeterminate`, a regression-gate trip — never a success or a violation), and a *gateway-synthesized* failure word (reconnect exhausted) is by construction a statement about the dead evidence channel, not the build; both rescue cells relay the marker. Only the fail-fast scheduler-settled rescue stays unmarked — the scheduler stops early, so a terminal-less root was plausibly never attempted. Batch records written before the marker existed default to no marked roots and are not re-classified on resume; across the deploy that introduces the marker, campaign reports show the one-time shift of event-loss rows from `target-substituted` to auto-retry/`infra-indeterminate`. Marker authorship is producer-owned: `rio: ` at byte 0 of an `STDERR_NEXT` line is the gateway's reserved grammar, and the gateway quotes (`> ` prepend, the shared rio-nix `quote_reserved_lines` owner) every relayed worker-controlled line that begins with it — at the no-activity log-relay fallback and per-line on the failure relay's message half (spec rule `gw.stderr.relay-quote-reserved`) — so the engine's byte-0-anchored capture matches gateway-authored lines only and a worker forgery of the marker arrives quoted and inert. The residual is the mixed-fleet skew window: an engine submitting through a gateway deployed from before the relay sanitization retains the prior consumer-priced exposure — a spoofed marker flips only that drv's same-batch `Substituted` row onto the evidence-loss leg, a bounded gate-visible cost (auto-retry burn, then `infra-indeterminate`, a regression-gate trip; timed campaigns terminalize immediately), never a success or a hidden violation — until the gateway is upgraded; the opposite skew (older engine, sanitizing gateway) is safe, since the unchanged byte-0 detector rejects quoted lines.
...
- **Supply state.** Client uploads are not separately journaled as durable state; on resume the planner re-probes target validity (`client_query_valid_paths` / `BatchQueryPathInfo`) and re-plans. Uploads are idempotent (`AddMultipleToStore` of an already-valid path is skipped by the probe), so a crash mid-prewarm costs re-probing, not correctness. The same rule covers a resume past a *completed* supply stage: the engine re-runs the stage's planning and probing (closure walk, validity probe, coverage probes, relay admission — never its deliveries) to rebuild the per-submission top-up context, so the wired-hook decision — and with it the supply attempt floor that gates permanent supply retirements — is identical in every process over the same spec; a pod reschedule can change when probing happens, never which units stay attemptable. One deliberate carve-out: when every in-scope job already has a terminal record, the rebuild (and its probe traffic) is skipped — the hook can never be invoked and the floor gates nothing in that state, so a report-only resume regenerates its artifacts without re-probing a cluster that may no longer exist. `supply.jsonl` is never re-trusted for path validity, but its deferral and settlement rows are deliberate decision inputs on every start: the supply-retirement rollup re-derives upload-rejected / supply-failed retirements from settled refusal and failure rows (under the attempt floor above), and — under the lone wiring with no per-submission delivery path (prewarm with dependencies "none") — a resume reads the journal's outstanding inline-deferral rows to refuse loudly rather than submit jobs whose deferred uploads nothing can deliver.
- **Retry-budget state.** Every engine-initiated resubmission (collect re-offer, stall auto-retry) is journaled to `requeues.jsonl` before the in-memory counter moves, and resume rebuilds the resubmission counters — the infra auto-retry budget, fail-fast singleton isolation, the stall auto-retry gate, `attempts` accounting — as a pure fold of that journal. A restart therefore keeps consumed budget consumed; it never grants a fresh retry allowance. Post-settlement cool-downs and watchdog clocks are deliberately process-local and start fresh.
...
- **Timed mode** resumes with degraded timing fidelity, never silently: outcomes already terminal are kept; requests dispatched but not terminal at the crash are re-dispatched immediately (their `attempts` and `dispatch_lateness` reflect it); requests not yet due are re-anchored so that the first pending request fires immediately and subsequent ones preserve their recorded relative spacing. `progress.json` and the report carry `resume_count` and a `timing_degraded: true` flag when a timed run resumed *with requests still pending* — the flag tracks actual re-anchoring, per its own contract. A resume that finds the schedule fully drained (the post-drain tail: a crash during collect/backfill/report, after the dispatcher already persisted its statistics) re-anchors nothing and dispatches nothing, so it preserves the prior run's `timed-stats.json` artifact verbatim and stays unflagged — the resuming process's zeroed per-run statistics never displace the faithful campaign numbers. The parity report policy ignores the flag; the timed-fidelity numbers (lateness distribution, interruption reproduction) are flagged low-confidence when it is set.
...
| `supply-failed` | supply/execute | The engine could not obtain or deliver required supply for reasons not attributable to the target (relay miss or fetch failure, archive corruption, claim timeout). Counted against run confidence via the `supply-failed-units` low-confidence flag (§7.3); it never trips the regression gate and never enters the gate's `checked` witness — it is evidence about the engine's supply, not about the target. |
...
- The comparability block keeps its fields (eval set → archive identity + digest, manifest sha256 → archive content digest, mode, build tenant, filters, engine version, signature table version, in-scope/attemptable/attempted, per-class excluded counts, completeness, low-confidence flags). Low-confidence flags now derive from `infra_low_confidence_pct` (infra-indeterminate rate), `no_truth_threshold_pct` (no-truth rate), below-pause-threshold prefetch shortfall (§8.4), and any-nonzero `supply-failed` retirements (`supply-failed-units` — the run-confidence accounting of the §7.2 supply-failed row), plus the existing tenant-verification flag (`tenant-upstreams-unverified`). Exclusion records from `exclusions.jsonl` enter the completeness accounting when the member is present (mirroring today's eval-error accounting); when it is absent the block notes that fact and applies no penalty (§4.7).
...
The gate result is data, not an exit code: the engine writes `report/gate.json` (`{ "policy": "regression-gate", "fail_on": "...", "tripped": bool, "checked": n, "counts": {...} }`) and mirrors it in `progress.json`. `checked` is the gate's coverage witness: how many evidence-bearing classified units the trip sets were evaluated over — every verdict, plus the `upload-rejected` and `target-substituted` dispositions; plan-time exclusions, `supply-failed`, and the `not-attempted` deadline backfill are excluded, so an attempted-nothing campaign cannot mint coverage out of backfill records. An untripped gate with `checked: 0` passed vacuously — it observed nothing that could have tripped it. The engine's own exit-code contract is unchanged (0 on full drain and on deadline-partial); encoding the gate in the pod exit code would make the k8s Job retry the whole campaign (`backoffLimit: 6`). `xtask replay report --check` reads `gate.json`, prints `fail_on`/`tripped`/`checked`, and exits non-zero when tripped — it is the single CI consumption point for the gate, and it is where the old `--fail-on` exit behavior now lives (§10). The consumption point also demands pass-meaningfulness, a conjunction over the document's own fields: an untripped pass asserts something only when the gate was evaluated over real units (`checked > 0`) AND could have tripped at all (`fail_on` is not `none` — under `none` the trip predicate is the constant false, so `tripped: false` is structural and a healthy non-zero `checked` masks it). Either vacuous shape (`checked: 0`, or `fail_on: "none"`) prints a loud warning by default — empty-scope campaigns legitimately record a clean gate, recurring smoke pipelines run such campaigns, and acknowledged accounting-only gates exist — and exits non-zero under `--require-coverage`, the opt-in for pipelines whose pass must mean "verified something". Launch closes the producer side of the `fail_on` shape: `--report-policy regression-gate` refuses an omitted `--fail-on` (the old clap default silently selected `none`), so an accounting-only gate is only ever launched through an explicit `--fail-on none` acknowledgment; the engine's spec validation warns on the combination but keeps accepting it, because specs persisted by pre-acknowledgment launches re-validate on resume. Flipping the default from warn to fail is deferred until those smoke pipelines assert a non-empty scope themselves or adopt the flag.
...
| Client upload, streamed | Embedded and relayed paths ≥ `large_nar_threshold_mib` — routed at two seams sharing the one threshold: relay paths at plan time (after the screen below, because their size is peer-declared), embedded paths by the executor within each topological level (their size is locally derived; `UploadPlan::batch` is reference-order truth, not the wire batch lane). Relay entries are admitted only past a plan-time plausibility screen on the peer-declared `NarSize` (64 GiB cap): an over-cap entry is skipped per path with a reason naming the cap, so a fabricated size can neither steer lane routing nor parameterize the wire op. | One `client_add_to_store_nar` per path — relay payloads as `NarPayload::Reader` streaming straight from the relay fetch; embedded payloads are materialized from the archive as full in-memory bytes today (`dump_nar` buffers the NAR; a Reader-from-the-archive variant is the streaming TODO recorded at `dump_nar`, and the buffering is bounded by the archive reader's 16 GiB embedded-tree budget — a decompression-bomb defense on the DwarFS image arm only; the plain-directory arm pays the same ceiling with no amplification threat behind it); deadline = `LARGE_UPLOAD_TIMEOUT` (600 s) base plus payload-proportional headroom at a 1 MiB/s floor, clamped at a 4 h ceiling because the relay arm's payload length is the cache's own declared size; fresh channel after any error. | Constant memory per transfer for relay payloads; an embedded transfer briefly holds its full NAR in memory. |
...
- **Upload refusals retry once on a fresh channel** with re-materialized payloads; a second refusal marks the affected paths `refused`, and dependent units take the `upload-rejected` disposition. A wire error during an upload is undetermined — it may be a refusal racing session teardown, or the transport dying mid-payload — so it gets the same single fresh-channel retry (a genuine refusal re-presents there as a clean daemon error), but `refused` settles only when a CLEAN daemon refusal was observed on one of the attempts: wire-death-only exhaustion settles `failed` and dependents take `supply-failed` (§7.2 — not attributable to the target), consistent with §6.6's rule that mid-payload wire errors are never attributed to the workload. A synthesized refusal would let a dead data path charge the target and trip the regression gate.
- **Relay failures degrade per path** (`failed`), and dependents that cannot proceed take `supply-failed`. This includes a relay byte stream dying INSIDE the wire op (the streamed lane hands the relay's live body reader into the upload, so a mid-body reset or short delivery surfaces there): rio-nix types those reader-side failures `PayloadSource`, distinct from channel wire deaths, and they settle the path `failed` with no retry — the wire-op retry would re-fetch the same failing relay. A circuit breaker (threshold `max(2 × upload_workers, 6)` consecutive transport failures — failed channel opens and mid-upload CHANNEL wire deaths alike, but never payload-source reads, which are the relay's failure and would otherwise trip a "gateway unreachable" collapse against a healthy gateway; a clean refusal resets the count, because the completed wire op proves the gateway answered — latched) stops dialing a gone gateway and fails remaining planned uploads fast; the watchdog's pause/backpressure machinery then takes over.
...
- `--fail-on <policy-specific value>` — recorded into the regression-gate policy block; REQUIRED whenever `--report-policy regression-gate` is requested (`none` must be an explicit acknowledgment of an accounting-only gate, never an omission's silent default — §7.3). It is **not** an engine exit-code knob (the engine keeps its exit-0-on-drain contract), it is evaluated by `report --check`.
...
**R3 — Fat-archive size envelopes.** A fat archive at platform-slice scale would reach hundreds of GiB before compression. The first hard ceiling is the S3 single-PUT cap: publication uploads the packed image as exactly one `PutObject` (the write-once self-write attribution relies on single-part uploads and their non-composite checksums), so the **packed image must stay ≤ 5 GiB** until a multipart path with FULL_OBJECT checksums and reworked attribution exists — the packer and `publish` both enforce this (`ensure_single_put_size`), refusing at staging time rather than failing opaquely at the final upload. Below that cap, the binding constraints are the recorder Job's scratch (200–400 Gi today), the engine pod's ephemeral storage (100 Gi today), and whole-archive download on campaign start. The format work in §4 sizes the envelopes; the operational consequence is that fat archives above the engine's local headroom either need ranged/streamed reads from S3 (DwarFS is seekable; plain directories are not, as S3 prefixes they are) or are simply refused by `launch` with a clear error. Until a streaming reader exists, `launch` must enforce a size ceiling against the pod's ephemeral allocation rather than letting the kubelet evict the pod mid-campaign.

**R4 — Gateway channel-budget coupling.** The client transport multiplexes a fixed 4 channels per SSH connection and sizes connections as ceil(max_sessions/4). The 4 is a client-side fan-out choice (it bounds the blast radius of one dropped connection), not a mirror of any gateway constant: the gateway's per-connection bound is two orders of magnitude higher (512, operator-configurable, and exceeding it terminates the whole connection rather than rejecting the open), and its real exec admission is a global session cap. The residual coupling is one-directional — the client fan-out must stay below the deployed gateway's per-connection bound, since a gateway configured below it would terminate campaign connections and kill sibling in-flight builds. There is no wire-level discovery of the gateway's bound and none is planned. Mitigations: a cross-crate test in rio-gateway (`replay_fan_out_headroom`; rio-replay is a dev-dependency there) compares the gateway's default bound against the engine's real fan-out constant and fails when either side drifts past the 2× headroom — note it pins the *default* only, an operator-lowered deployed bound is not machine-checked; the engine logs its fan-out and connection count at startup; and a burst of exec refusals (the gateway shedding load at its global session cap) is treated as a capacity/configuration problem rather than per-unit infra verdicts.
