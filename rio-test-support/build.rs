...
//! hand-written in grpc/admin.rs. New default-stub RPCs need zero Rust
...
/// generated — they stay hand-written in `src/grpc/admin.rs`. Streaming
    ...
    for m in &service.method {
        ...
        assert!(
            ...
            "streaming RPC {proto_name} must be in MANUAL_METHODS \
             (add it there + write the associated type + stream body in grpc/admin.rs)"
        );
    ...
    std::fs::write(
        ...
        format!(
            "// @generated by rio-test-support/build.rs from admin.proto\n\
             // include!()-ed at module level in src/grpc/admin.rs (before the\n\
...
/// Map a descriptor type reference to the Rust path seen from `src/grpc/admin.rs`.
///
