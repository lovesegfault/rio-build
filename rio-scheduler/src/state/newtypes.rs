//! Scheduler identifier newtypes: [`DrvHash`] and [`ExecutorId`].
//!
...
//! gRPC ingress (`grpc/scheduler_service.rs`, `grpc/executor_service.rs`).

/// Define an `Arc<str>`-backed newtype with the ergonomic trait surface
/// for HashMap keys, tracing fields, and test assertions.
///
/// Clone is an atomic refcount increment — no allocation, no memcpy.
///
...
/// identical to a `String`-backed newtype.
///
