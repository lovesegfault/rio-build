...
//!
//! sh-002 Stage C (the §lifecycle strike-3 close: bug_118 → bug_023 →
//! sh-002C):
//!
...
//!   Pre-split, `run_lease_loop` was a `spawn_monitored` task on the
//!   SHARED `#[tokio::main]` runtime: a 16.35s `17-ready-cache-sweep`
//!   Tick under admitted load starved the renew tick past
//!   `SELF_FENCE_AFTER=11s` and the leader self-fenced against a lease
//!   the standby never stole (sh-002 advisory Stage C; phantom 1.84s
//!   recovery). Same failure shape as the controller's live_054 — same
//!   port (`rio-controller/src/guard.rs`), adapted: the scheduler keeps
//!   the gRPC health reporter on the MAIN gRPC server (it is the
//!   `BalancedChannel` leader-probe surface; moving it would break
//!   leader discovery), so the guard's `/healthz`+`/readyz` is an axum
//!   server on `:9194`.
...
//!   runtime, so a browned-out scheduler sheds (Endpoints removal)
//!   instead of being killed. The chart's `startupProbe` STAYS
//!   `tcpSocket:9001` (gRPC bind ≈ post-`init_db_pool`/migrations) —
//!   the guard binds before `init_db_pool`, so an `httpGet`
//!   `startupProbe` would be vacuous.
//! - **D5 — the skew sentinel.** D1 is falsifiable in production: the
//!   sentinel exports each domain's executor-scheduling delay as
//!   `rio_scheduler_runtime_skew_seconds{domain}` and captures a
//!   thread table past [`GuardConfig::stall_threshold`]
//!   (`rio_scheduler_runtime_skew_stalls_total{domain}`), so the next
//!   freeze is attributable instead of a silent self-fence.
//!
...
//!    at the struct definition), shared with the dag-actor on the main
//!    domain.
...
//! 8. `SchedulerLeaseHooks` — an `mpsc::UnboundedSender<ActorCommand>`
//!    only (sync `.send()`, never blocks); the `lease-hook-forwarder`
//!    task it spawns stays on the MAIN runtime
//!    ([`crate::lease_hooks::SchedulerLeaseHooks::new`] is called from
//!    `main.rs` BEFORE the hooks value crosses into the guard), so
//!    nothing the guard polls depends on main-domain mailbox drain.
//!
...
//! spawn site carries its disposition, exhaustively:
//!
...
/// compile-line error under `--deny warnings`.
...
impl GuardHandle {
    ...
    /// `rio_scheduler_runtime_skew_stalls_total{domain="main"}`.
    ...
    /// instances; the latch type is module-private). Mirror of
    /// `rio_scheduler_runtime_skew_stalls_total{domain="guard"}`.
    ...
    /// survives admitted-load starvation instead of being violated by
    /// the 16.35s Tick (sh-002 Stage C). `run_lease_loop` builds its
    /// own kube client on THIS runtime — no main-domain pool sharing.
...
/// the readiness flag main.rs flips after the gRPC server binds.
/// Returns the clonable [`GuardHandle`] and the linear [`GuardJoin`]
/// token — the process owner MUST `.join()` the token after the
/// working domain drains (`sys.epilogue.drain`).
...
async fn sentinel(
    ...
    loop {
        ...
        metrics::gauge!("rio_scheduler_runtime_skew_seconds", "domain" => "guard")
        ...
        match watch.on_tick() {
            ...
            MainTick::Stall { bound } => warn!(
                ...
                "main-runtime scheduling delay past threshold; thread \
                 table captured (the sh-002C attribution record)"
            ),
...
/// `rio_scheduler_runtime_skew_stalls_total` emission live INSIDE
...
impl MainDomainWatch {
    ...
    fn export(&self, skew: Duration) {
        ...
        metrics::gauge!("rio_scheduler_runtime_skew_seconds", "domain" => "main")
...
/// sh-002C attribution record: at capture time the table says WHICH
...
mod tests {
    ...
    /// threads (the attribution half, unit face).
    #[test]
    ...
    /// ONE EdgeLatch semantics — observations within an episode count
    /// once at the rising edge; resolution re-arms.
    #[test]
    fn edge_latch_counts_episodes_not_observations() {
    ...
    /// Settle face: an episode that starts AND resolves between ticks
    /// — the probe answered late before any live observation — still
    /// counts exactly once.
    #[test]
    fn settled_between_ticks_episode_counts_once() {
        let episodes = Arc::new(AtomicU64::new(0));
    ...
    /// Retirement face: the probe sender dropping — the main runtime
    /// shut down — retires the lane: typed, reported once, no respawn.
    #[test]
    fn closed_probe_retires_lane() {
        let mut watch = MainDomainWatch::new(
            Arc::new(AtomicU64::new(0)),
            Arc::new(AtomicU64::new(0)),
            Duration::from_millis(50),
