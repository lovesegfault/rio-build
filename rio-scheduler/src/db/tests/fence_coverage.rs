...
///   reprobe/lane batches. `verify_preexisting_completed`'s 6c
///   batched stale-reset persist routes through the canonical
///   `persist_status_batch` (so its Err arm outbox-latches) and is
///   counted under completion.rs's 1.
///
