...
impl SchedulerDb {
    ...
    /// `nar_index.root_node`.
    ///
    /// `seeds` are the dispatch-time inputs (DAG children's outputs ∪
    /// `inputSrcs`). Paths not in `narinfo` still appear (with
    /// `root_node = None`) but contribute no further references.
    ///
