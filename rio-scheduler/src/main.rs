...
async fn main() -> anyhow::Result<()> {
    ...
    // health_reporter() returns (Reporter, Server). The Reporter is
    // Clone — the toggle loop already cloned it. We need a second
    // `HealthServer` instance for the plaintext listener, but there's
    // no `Reporter::server()` method. Trick: tonic-health's Server
    // is also Clone (it's just Arc<RwLock<HashMap>> internally).
    //
    // Actually `HealthServer` (the gRPC service) is NOT directly
    // Clone in the public API. But the underlying `HealthService`
    // struct IS Clone, and `HealthServer::new(service)` builds the
    // gRPC wrapper. We can't get the inner service back from
    // `health_service` though.
    //
    // Simpler: call `health_reporter()` ONCE (already done above),
    // and use the returned `health_service` for the MAIN server.
    // For the plaintext server, use the `health_reporter` clone
    // with a SECOND call to `health_reporter()` — NO, that creates
    // a fresh independent map.
    //
    // Correct solution: `health_service` is of type
    // `HealthServer<HealthService>`. tonic's `HealthServer<T>` is
    // generated by tonic-health's proto — it has `.clone()` if T
    // is Clone. `HealthService` IS Clone. So the whole thing clones.
...
mod tests {
    ...
    /// Phase 3b T4: the health service is Clone, so we can serve it
    /// from TWO tonic servers (mTLS main port + plaintext health port)
