...
impl DagActor {
    ...
    /// `failure_exec_id`, M_117) is read best-effort — attribution is
    ...
    async fn attribute_failfast_culprit(&mut self, build_id: Uuid, failed_hash: &DrvHash) {
        ...
        if let Some(build) = self.builds.get_mut(&build_id) {
            // The culprit is Poisoned by construction (find_poisoned_culprit
            // returns only Poisoned nodes), so this is exactly the
            // negative-cache classification — the same value
            // handle_derivation_failure would assign for the surfaced node
            // when it is itself Poisoned. Setting it here keeps the
            // classification when the surfaced node is a DependencyFailed
            // ancestor: the culprit is what failed.
            build.note_first_failure(crate::state::FirstFailure {
                summary,
                failed_drv: Some(culprit_path.clone()),
                status: Some(rio_proto::types::BuildResultStatus::CachedFailure),
            });
            build.culprit = Some(crate::state::FailfastCulprit {
                derivation_path: culprit_path,
                exec_id: failure_exec_id,
                error_message,
                failed_at: poisoned_epoch.and_then(rio_common::clamped::epoch_secs),
            });
        }
    }
