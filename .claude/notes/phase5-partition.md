...
<!-- P0247-OUTCOME: dependent_shape=REMOVED-UPSTREAM (schema v2), cardinality=0 (always {} — back-compat stub; NixOS/Nix@26842787 realisation.cc:105 + realisation.hh struct has no field + test comment "We no longer have a notion of dependent realisations"), ca_on_ca_pct=bimodal(~0% default | ~100% with config.contentAddressedByDefault=true — no middle ground, no per-package opt-in). Historical shape: flat {DrvOutput_string: StorePath_basename}. IMPACT: P0253 T1 (gateway parse-and-insert) obsolete — wire is always {}; realisation_deps populated by SCHEDULER during resolve, not gateway from wire. r[sched.ca.resolve] spec text stale. See ADR-018. -->
