...
`f343eb9` wired the fallback in `estimator.rs`. The fallback chain: EMA history → closure-size proxy → hardcoded default. The proxy: `estimated_memory = closure_size × MEMORY_MULTIPLIER` (empirically ~2.5× — compilers tend to hold input + output + intermediate state in memory); `estimated_duration_secs = closure_size / BUILD_THROUGHPUT_BYTES_PER_SEC` (empirically ~10MB/s for Rust/C++ builds). Both constants documented as rough heuristics — they'll be wrong by 10× for pathological builds, but "closure size correlates with build cost" is better than "everything takes 60s." The estimator records `HistoryEntry.input_closure_bytes` so when history DOES arrive, the correlation can be validated.
...
[
  ...
  {"path": "rio-scheduler/src/estimator.rs", "action": "MODIFY", "note": "fallback chain: EMA → closure-size proxy → hardcoded; MEMORY_MULTIPLIER, BUILD_THROUGHPUT constants"},
