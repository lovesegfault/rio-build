# Plan 523: wire `rename-unassigned` into docs-branch merger — pre-ff step
...
[
  ...
  {"path": ".claude/lib/test_scripts.py", "action": "MODIFY", "note": "T2: test_docs_merger_renames_placeholders_pre_ff fixture + assertions. P0304-T532 also touches this file (tracey_domains fix, diff section)"},
...
**Conflicts with:** `.claude/lib/onibus/merge.py` is touched by T-placeholder work ([P0304](plan-0304-trivial-batch-p0222-harness.md)-T513 already documented the per-doc→contiguous convention change). `test_scripts.py` is shared with P0304-T532 (tracey_domains fix) — diff section, both additive.
