...
### T963241201 — `test(scheduler):` FOD completion skips insert_build_sample — negative test
...
### T963241202 — `test(scheduler):` fod_queue_depth + fetcher_utilization metric emission + NaN guard
...
### T963241203 — `test(coverage):` investigate backgrounded .#coverage-full VM-test flakes (4 logs, 2 signatures)
...
- T963241201: `cargo nextest run -p rio-scheduler test_fod_completion_skips_build_sample` → passed
- T963241202: `cargo nextest run -p rio-scheduler fod_queue_depth_gauge_updates fetcher_utilization_nan_guard` → ≥2 passed
- T963241203: `.#cov-vm-lifecycle-wps-k3s` re-run outcome documented — if green: `grep 'lifecycle-wps\|coverage-full parallelism' .claude/known-flakes.jsonl` → ≥1 hit; if red: followup plan filed via `onibus state followup`
...
- `r[sched.dispatch.fod-to-fetcher]` — T963241201 verifies (FOD completion path — build_samples skip is the metrics-side of the hard-split)
- `r[obs.metric.scheduler]` — T963241202 verifies (fod_queue_depth + fetcher_utilization gauges; first `r[verify]` for these two specific metrics)
...
[
  ...
  {"path": "rio-scheduler/src/actor/tests/completion.rs", "action": "MODIFY", "note": "T963241201: +test_fod_completion_skips_build_sample (negative: COUNT==0 after FOD success). discovered_from=452"},
  {"path": "rio-scheduler/src/actor/tests/dispatch.rs", "action": "MODIFY", "note": "T963241202: +fod_queue_depth + fetcher_utilization CountingRecorder capture test incl total==0 NaN-guard. discovered_from=452"},
  {"path": ".claude/known-flakes.jsonl", "action": "MODIFY", "note": "T963241203: CONDITIONAL — coverage-full VM parallelism ceiling entry if investigation confirms environmental. discovered_from=coverage"}
]
```
