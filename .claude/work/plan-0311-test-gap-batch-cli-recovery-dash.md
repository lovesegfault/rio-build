...
### T991199107 — `test(store):` cross-batch cycle reclaim — cycle spanning `SWEEP_BATCH_SIZE` boundary
...
- T991199107: `cargo nextest run -p rio-store sweep_reclaims_cross_batch_cycle` → 1 passed; mutation check: change `:146` temp-table INSERT from `&unreachable` to `&batch` (hypothetical per-batch populate) → test FAILS with `paths_deleted<101` and `paths_resurrected>0`
- T991199107: `nix develop -c tracey query rule store.gc.sweep-cycle-reclaim` shows ≥2 `verify` sites (existing `sweep_reclaims_two_cycle` + new cross-batch sibling)
...
- `r[store.gc.sweep-cycle-reclaim]` — T991199107 verifies (cross-batch cycle; existing `sweep_reclaims_two_cycle` covers intra-batch only). Spec text at [`store.md:258-260`](../../docs/src/components/store.md) already says "full unreachable set" — this is coverage, not a spec change.
...
[
  ...
  {"path": "rio-store/src/gc/sweep.rs", "action": "MODIFY", "note": "T991199107: sweep_reclaims_cross_batch_cycle test after :716 — 101 paths, path[0]↔path[100] cycle spans SWEEP_BATCH_SIZE=100 boundary. r[verify store.gc.sweep-cycle-reclaim]. discovered_from=449"}
]
```
