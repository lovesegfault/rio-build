...
### T975348012 — `test(scheduler):` config_rejects_inf_backoff_multiplier — INFINITY explicit proof
...
- T975348012: `cargo nextest run -p rio-scheduler config_rejects_inf_backoff_multiplier` → 1 passed; `grep 'f64::INFINITY\|INFINITY must be rejected' rio-scheduler/src/main.rs` → ≥2 hits (test body + assert msg)
...
[
  ...
  {"path": "rio-scheduler/src/main.rs", "action": "MODIFY", "note": "T975348012: +config_rejects_inf_backoff_multiplier after existing NaN tests (post-P0415 ~:1067). discovered_from=415. HOT count=38 — additive test-fn"}
]
```
