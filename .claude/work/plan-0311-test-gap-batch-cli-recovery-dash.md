...
### T989430201 — `test(scheduler):` to_proto BucketedEstimate field map — populated-queue RPC test
...
### T989430202 — `test(scheduler):` config_rejects_inf_headroom_multiplier — T71's sibling
...
- T989430201: `cargo nextest run -p rio-scheduler to_proto_field_map` passes (option-b); mutation check: swap `est_memory_bytes: b.memory_bytes` → `b.cpu_millicores as u64` at `manifest.rs:38` → test FAILS
- T989430201: `nix develop -c tracey query rule sched.admin.capacity-manifest` shows ≥1 `verify` site (currently `impl`-only at `manifest.rs:15`)
- T989430202: `cargo nextest run -p rio-scheduler config_rejects_inf_headroom_multiplier` → 1 passed; `grep -c 'f64::INFINITY' rio-scheduler/src/main.rs` ≥ 1 (proves test body has the literal, not just NaN)
...
- `r[sched.admin.capacity-manifest]` — T989430201 verifies (first `r[verify]` for this marker; [`manifest.rs:15`](../../rio-scheduler/src/admin/manifest.rs) has `r[impl]` only; marker at [`scheduler.md:164`](../../docs/src/components/scheduler.md))

No marker for T989430202 — config validation is defensive plumbing, same category as T71 (no `r[sched.config.*]` domain).
...
[
  ...
  {"path": "rio-scheduler/src/admin/manifest.rs", "action": "MODIFY", "note": "T989430201: to_proto pub(crate) + to_proto_field_map_no_swap test in cfg(test) mod + r[verify sched.admin.capacity-manifest]. discovered_from=501"},
  {"path": "rio-scheduler/src/main.rs", "action": "MODIFY", "note": "T989430202: config_rejects_inf_headroom_multiplier after :1370 (T71 sibling). HOT count=42 — additive test-fn. discovered_from=501"}
]
```
