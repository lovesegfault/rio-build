...
Test shape: generate a NAR large enough to chunk into >64 pieces (at default chunk size ~256KB that's ~17MB; or override the chunk size downward for the test via whatever knob exists). Upload via `put_chunked`, then assert `manifests.updated_at > manifests.created_at` for the row — proving at least one heartbeat fired and bumped the timestamp mid-upload.

```rust
#[sqlx::test]
async fn heartbeat_fires_on_large_upload(pool: PgPool) -> TestResult {
    // Synthesize a NAR that produces >HEARTBEAT_CHUNK_INTERVAL chunks.
    // Either: (a) 17MB+ of random bytes at default 256KB chunk size, or
    // (b) override chunk size to 1KB and use 70KB (cheaper, if the knob exists).
    let nar = gen_large_nar(/* >64 chunks worth */);
    let backend = Arc::new(MemBackend::default());
    put_chunked(&pool, &backend, nar, /* hb= */ Some(...)).await?;

    let row = sqlx::query!("SELECT created_at, updated_at FROM manifests WHERE ...")
        .fetch_one(&pool).await?;
    assert!(row.updated_at > row.created_at, "heartbeat should have bumped updated_at");
    Ok(())
}
```
...
Add a VM-test assertion (or helm-lint check) that the scheduler image doesn't bundle rio-cli. Two routes:

**Route A (VM test):** in an existing k3s scenario that has the scheduler pod running, assert:
```python
# r[verify sec.image.control-plane-minimal]
with subtest("scheduler image minimal — no rio-cli"):
    # Should FAIL: rio-cli not in the image
    client.fail("kubectl -n rio-system exec deploy/rio-scheduler -- which rio-cli")
    # Bonus: jq, column also absent
    client.fail("kubectl -n rio-system exec deploy/rio-scheduler -- which jq")
```
Place the marker at the `subtests = [...]` entry in [`nix/tests/default.nix`](../../nix/tests/default.nix) per the wiring convention (NOT in the scenario file header).

**Route B (Nix-level assert):** if the scheduler image derivation is inspectable, assert its closure doesn't reference `rio-cli`'s store path. Faster (no VM spin-up) but requires the image to be a Nix-built derivation with a queryable closure.
...
**Route A (cheap, mock the error):** inject a `MetadataError::Deadlock` from the closure on first call, success on second. Proves the retry mechanism works without proving PG actually raises 40P01:

```rust
#[tokio::test]
async fn with_sorted_retry_retries_once_on_deadlock() {
    let calls = Arc::new(AtomicUsize::new(0));
    let c = calls.clone();
    let result = with_sorted_retry(vec![b"a".to_vec(), b"b".to_vec()], move |_keys| {
        let c = c.clone();
        async move {
            if c.fetch_add(1, Ordering::SeqCst) == 0 {
                Err(MetadataError::Deadlock(sqlx::Error::PoolClosed)) // any inner
            } else {
                Ok(42)
            }
        }
...
**Route B (skip — document why):** if Route A's mock feels too synthetic (the closure-retry mechanism is 3 lines; a test that mocks the error proves `match` works), add a `// TESTED-BY: integration — 40P01 retry is defensive, real deadlock requires index-page splits under load; see r[store.chunk.lock-order]` comment and move on. The reviewer flagged this as "may be more cost than value."

**Recommendation:** Route A. It's ~15 lines, catches a future regression where someone changes `Err(Deadlock)` to `Err(_)` (accidentally retry-on-everything) or drops the retry entirely. Carries `// r[verify store.chunk.lock-order]` — second verify site for the marker (first is whatever tests the sort). discovered_from=495.

### T499 — `test(scheduler):` to_proto BucketedEstimate field map — populated-queue RPC test

[`rio-scheduler/src/admin/manifest.rs:36-42`](../../rio-scheduler/src/admin/manifest.rs) `to_proto(b: BucketedEstimate) -> DerivationResourceEstimate` is a 3-field copy:
...
**Zero test coverage.** The existing populated-queue tests at [`actor/tests/misc.rs:542-652`](../../rio-scheduler/src/actor/tests/misc.rs) call `actor.compute_capacity_manifest(1.25)` directly — that returns `Vec<BucketedEstimate>` (the Rust-native type) and the tests assert on it. They never reach the proto conversion at [`:32`](../../rio-scheduler/src/admin/manifest.rs). The one RPC-path test is empty-queue only (asserts `estimates.is_empty()`).

**The hazard:** a field swap like `est_memory_bytes: b.cpu_millicores as u64` compiles (both `u64`), passes all current tests (none assert proto fields), and the controller's [P0503](plan-0503-manifest-reconciler-controller.md) manifest reconciler sizes pods on garbage — memory-for-cpu means every build OOMs or over-provisions by 1000×.

MODIFY [`actor/tests/misc.rs`](../../rio-scheduler/src/actor/tests/misc.rs) — extend ONE of the three populated tests (the cold-start-omission one at `:542` is simplest, has the clearest assertions) to ALSO call `get_capacity_manifest` (the RPC handler, not `compute_capacity_manifest`) and assert proto field identity:

```rust
// After the existing compute_capacity_manifest assert at ~:581:
// r[verify sched.admin.capacity-manifest]
// The RPC path goes Vec<BucketedEstimate> → to_proto → DerivationResourceEstimate.
// The actor-direct assert above proves the Vec is right; this proves the proto
// conversion didn't swap fields (both u64, compiles cleanly, controller sizes
// pods on garbage).
let resp = crate::admin::manifest::get_capacity_manifest(&handle, 1.25)
    .await
    .expect("RPC");
assert_eq!(resp.estimates.len(), manifest.len());
for (proto, rust) in resp.estimates.iter().zip(&manifest) {
    assert_eq!(proto.est_memory_bytes, rust.memory_bytes, "memory swap");
    assert_eq!(proto.est_cpu_millicores, rust.cpu_millicores, "cpu swap");
    assert_eq!(proto.est_duration_secs, rust.duration_secs, "duration swap");
}
```

If `get_capacity_manifest` needs an `ActorHandle` (it does — `actor: &ActorHandle` at [`:17`](../../rio-scheduler/src/admin/manifest.rs)) and the direct-actor tests don't have one wired, either (a) wire an `ActorHandle` in the test setup (check `test_spawn_actor` or similar fixture), or (b) make `to_proto` `pub(crate)` and test it in isolation with a hand-built `BucketedEstimate`. Option (b) is cheaper and tests exactly the 3-line function at risk:

```rust
#[test]
fn to_proto_field_map_no_swap() {
    let b = BucketedEstimate {
        memory_bytes: 4 * 1024 * 1024 * 1024,  // 4 GiB
        cpu_millicores: 2000,                   // 2 cores
        duration_secs: 300,                     // 5 min
    };
    let p = crate::admin::manifest::to_proto(b);
    // Distinct values per field — any swap flips ≥1 assert.
    assert_eq!(p.est_memory_bytes, 4 * 1024 * 1024 * 1024);
    assert_eq!(p.est_cpu_millicores, 2000);
    assert_eq!(p.est_duration_secs, 300);
}
```

Prefer (b) — it's 10 lines, lives in `admin/manifest.rs` `cfg(test)` mod, directly verifies the conversion. Carries `// r[verify sched.admin.capacity-manifest]`. discovered_from=501.

### T500 — `test(scheduler):` config_rejects_inf_headroom_multiplier — T71's sibling

[`rio-scheduler/src/main.rs:283`](../../rio-scheduler/src/main.rs) `cfg.headroom_multiplier.is_finite() && cfg.headroom_multiplier > 0.0` — the comment at [`:279-281`](../../rio-scheduler/src/main.rs) explicitly names the infinity hazard ("`inf → u64::MAX → div_ceil × bucket` overflows u64 (panic in debug, wrap in release)"). `is_finite()` DOES reject `f64::INFINITY` — code correct. Proof missing.

Existing tests at [`:1347`](../../rio-scheduler/src/main.rs) cover `0.0`, at [`:1360`](../../rio-scheduler/src/main.rs) cover `f64::NAN`. Neither tests `f64::INFINITY`.

This is the exact same gap-class as T71 (`config_rejects_inf_backoff_multiplier`) — same `is_finite()` branch, different config field. Add after `config_rejects_nan_headroom_multiplier` at `:1370`:

```rust
#[test]
fn config_rejects_inf_headroom_multiplier() {
    // Comment at :279-281 names inf explicitly: "inf → u64::MAX →
    // div_ceil × bucket overflows u64". is_finite() rejects it (same
    // branch as NaN) — this proves the inf arm. T71 sibling.
    let cfg = Config {
        headroom_multiplier: f64::INFINITY,
        ..test_valid_config()
    };
    let err = cfg.validate().unwrap_err().to_string();
    assert!(
        err.contains("headroom_multiplier must be finite and positive"),
        "INFINITY must be rejected (overflows u64 in div_ceil×bucket), got: {err}"
    );
}
```
...
### T501 — `test(controller):` is_floor_job / parse_bucket_from_labels complement — asymmetric label case
...
Our code never produces this state ([`bucket_labels`](../../rio-controller/src/reconcilers/builderpool/manifest.rs) at `:393-398` sets both together — `Some(b)` → both numeric, `None` → both `"floor"`). But the two predicates are NOT the clean complement the code structure implies. Document the gap with a test that names it:

```rust
// r[verify ctrl.pool.manifest-labels]
#[test]
fn is_floor_job_and_parse_bucket_complement_on_valid_labels() {
    // The two predicates partition the space of VALID labels (labels
    // produced by bucket_labels). is_floor_job==true ⇔ parse==None
    // ⇔ both classes are "floor". For INVALID labels (hand-edited,
    // asymmetric), the invariant breaks — documented below.
    for bucket in [None, Some((8 * GI, 4000)), Some((48 * GI, 16000))] {
        let job = make_test_job_with_bucket_labels(bucket);
        assert_eq!(is_floor_job(&job), bucket.is_none());
        assert_eq!(parse_bucket_from_labels(&job), bucket);
        // Complement: exactly one of {is_floor, parse.is_some} is true.
        assert_ne!(is_floor_job(&job), parse_bucket_from_labels(&job).is_some());
    }
}

#[test]
fn asymmetric_labels_fall_through_both_predicates() {
    // DOCUMENTS (not fixes) the asymmetry: is_floor_job checks
    // mem-class only; parse_bucket checks mem OR cpu. A corrupted
    // Job with mem=8Gi cpu=floor is invisible to both. Our
    // bucket_labels never produces this; this test is a canary for
    // the day someone relaxes the label contract.
    let mut job = make_test_job_with_bucket_labels(Some((8 * GI, 4000)));
    job.metadata.labels.as_mut().unwrap()
        .insert(CPU_CLASS_LABEL.into(), FLOOR_CLASS.into());  // corrupt
    assert!(!is_floor_job(&job), "mem-class != floor → not floor");
    assert_eq!(parse_bucket_from_labels(&job), None, "cpu==floor → None");
    // Both false/None → this job is invisible to both supply counts.
    // If this test ever FAILS (someone made them agree), delete it
    // with a cheer — the asymmetry is gone.
}
```

Export `is_floor_job` as `pub(super)` or `#[cfg(test)] pub(super)` so tests can reach it. `make_test_job_with_bucket_labels` is a thin wrapper around the existing test-job constructors + `bucket_labels`.

**Truncation-extract not here:** the followup also suggested extracting `truncate_plan(plan, budget)` at [`:240-272`](../../rio-controller/src/reconcilers/builderpool/manifest.rs) as a pure fn. That's absorbed into [P0507](plan-0507-manifest-truncation-starvation.md) T1 — P0507 MUST extract + rewrite that loop to fix the starvation livelock, so a separate extraction here would collide. discovered_from=503.

### T502 — `test(store):` cross-batch cycle reclaim — cycle spanning `SWEEP_BATCH_SIZE` boundary

[`sweep.rs:216-217`](../../rio-store/src/gc/sweep.rs) states: "The temp table holds the WHOLE `unreachable` set (not just `batch`) — a cycle may span `SWEEP_BATCH_SIZE` boundaries." The existing `sweep_reclaims_two_cycle` at [`:716`](../../rio-store/src/gc/sweep.rs) tests only 3 paths — all fit in one 100-path batch, so the cross-batch property is untested.

This property was ALSO untested pre-[P0449](plan-0449-gc-sweep-temp-table-anti-join.md) (the old `.bind(&unreachable)` had the same whole-set scope), but P0449 replaced the mechanism. A regression test catches the case where someone later swaps the temp-table populate to per-batch (a plausible "optimization" that would silently break cross-batch cycles).

Add after `sweep_reclaims_two_cycle`:

```rust
/// Cross-batch cycle: A↔Z where A is in batch[0] and Z in batch[1].
/// The temp-table anti-join must see the WHOLE unreachable set, not
/// just the current batch — otherwise when sweeping batch[0], Z
/// isn't yet in sweep_unreachable, re-check sees Z as a live
/// referrer, A resurrected. Regression guard: P0449 populates the
/// temp table once at sweep start (:146); this would break if
/// someone "optimizes" to per-batch populate.
// r[verify store.gc.sweep-cycle-reclaim]
#[tokio::test]
async fn sweep_reclaims_cross_batch_cycle() {
    let db = TestDb::new(&crate::MIGRATOR).await;

    // 99 filler paths (no references) + path[0]↔path[100] cycle.
    // SWEEP_BATCH_SIZE=100 → path[0] in batch 0, path[100] in batch 1.
    // 101 total = 2 batches.
    let path_first = test_store_path("cross-batch-first");
    let path_last = test_store_path("cross-batch-last");
    let hash_first = StoreSeed::raw_path(&path_first)
        .with_refs(&[&path_last])
        .seed(&db.pool)
        .await;
    let filler_hashes: Vec<_> = futures::future::join_all(
        (1..100).map(|i| {
            let p = test_store_path(&format!("filler-{i}"));
            async move { StoreSeed::raw_path(&p).seed(&db.pool).await }
        })
    ).await;
    let hash_last = StoreSeed::raw_path(&path_last)
        .with_refs(&[&path_first])
        .seed(&db.pool)
        .await;

    // Order: first, 99×filler, last. chunks(100) → [first+99 filler],
    // [last]. The first↔last cycle spans the boundary.
    let mut unreachable = vec![hash_first];
    unreachable.extend(filler_hashes);
    unreachable.push(hash_last);
    assert_eq!(unreachable.len(), 101, "2 batches at SWEEP_BATCH_SIZE=100");

    let stats = sweep(&db.pool, None, unreachable, false, &no_shutdown())
        .await.unwrap();
    assert_eq!(
        stats.paths_deleted, 101,
        "cross-batch cycle swept. If first resurrected, the temp-table \
         populate has been narrowed to per-batch — that's the regression \
         this test guards."
    );
...
**SUBSUMED by [P522](plan-522-warn-continue-escalation.md):** P522-T2 landed `spawn_manifest_jobs` (extracted spawn-loop helper) + `ApiServerVerifier` scenario-count tests at [`manifest_tests.rs`](../../rio-controller/src/reconcilers/builderpool/tests/manifest_tests.rs). `spawn_bails_after_consecutive_threshold_not_before` proves attempt-count EXACTLY via `guard.verified()` (panics on scenario-count mismatch: more attempts = no scenario to consume; fewer = unconsumed scenarios). `spawn_intermittent_fail_does_not_bail` proves sub-threshold failures continue (alternating 403/201, all 2N attempted, returns `Ok`). warn-THEN-bail regression: the test's mock has EXACTLY `SPAWN_FAIL_THRESHOLD` scenarios; if a bail fires below threshold, `verified()` panics on unconsumed scenarios. **T503 reduces to:** confirm `spawn_intermittent_fail_does_not_bail` + `spawn_name_collision_neither_increments_nor_resets` stay green — no additional mock infra needed.
...
#[tokio::test]
async fn spawn_error_continues_through_batch_and_reaches_reap() {
    // MockJobsApi: first create() → Err, second → Ok.
    // Plan has 2 directives. Reapable set has 1 idle Job.
    //
    // Expected: 1 warn (first spawn), 1 success (second spawn),
    //   1 delete (reap). All three operations attempted.
    // warn-THEN-bail regression: first spawn warns then Err(e)? →
    //   second spawn never attempted, reap never reached.
    //
    // Key assertions:
    //   - mock.create_call_count == 2 (both directives attempted)
    //   - mock.delete_call_count == 1 (reap ran)
    //   - reconcile returned Ok (no bail on first error)
}
```
...
| `is_docs_branch` | `has_placeholder_docs` | Arm | Test |
|---|---|---|---|
| T | T | fires | [`test_scripts.py:~1885`](../../.claude/lib/test_scripts.py) (implied — docs branch + P-placeholder) |
| T | F | fires | [`test_pre_ff_rename_fires_on_branch_name_alone` :1953](../../.claude/lib/test_scripts.py) |
| F | F | no-op | [`test_pre_ff_rename_noop_on_impl_branch` :1924](../../.claude/lib/test_scripts.py) |
| F | T | fires | **NONE** |

Docstring at [`merge.py:1069`](../../.claude/lib/onibus/merge.py) explicitly claims the (F,T) arm: "The second catches ad-hoc docs branches with nonstandard names." The code's `or` disjunction covers it; there's just no test proving it.

**Scenario:** branch `planning-adhoc` (doesn't match `_DOCS_BRANCH_RE` which wants `docs-NNNNNN`), worktree has `plan-999999901-foo.md` (placeholder present). `_find_placeholders` nonempty → `has_placeholder_docs=True` → `rename_unassigned` fires.

Add after `:1987`:

```python
def test_pre_ff_rename_fires_on_placeholders_alone(tmp_repo: Path, monkeypatch):
    """(F,T) arm: branch name does NOT match docs-NNNNNN but placeholder
    files ARE present. merge.py:1069 docstring claims this catches
    'ad-hoc docs branches with nonstandard names'. _find_placeholders
    fires, _DOCS_BRANCH_RE doesn't."""
    from onibus import merge as merge_mod
    from onibus.merge import pre_ff_rename

    impl = tmp_repo / ".claude" / "work"
    impl.mkdir(parents=True)
    _git(tmp_repo, "add", "-A")  # may be no-op; seed commit needs SOMETHING
    (impl / ".gitkeep").write_text("")
    _git(tmp_repo, "add", "-A")
    _git(tmp_repo, "commit", "-m", "seed", "--no-verify")
    # Nonstandard branch name — doesn't match ^docs-\d{6}$
    _git(tmp_repo, "checkout", "-b", "planning-adhoc")
    # P-placeholder file — _find_placeholders matches plan-9ddddddNN-*.md
    (impl / "plan-999999901-adhoc-thing.md").write_text(
        "# Plan 999999901: adhoc\n\n## Tasks\n\n### T1\n\nfoo.\n"
    )
    _git(tmp_repo, "add", "-A")
    _git(tmp_repo, "commit", "-m", "docs: adhoc plan", "--no-verify")

    monkeypatch.setattr(merge_mod, "_worktree_for", lambda _: tmp_repo)
    monkeypatch.setattr(merge_mod, "DOCS_DIR", impl)

    # _DOCS_BRANCH_RE does NOT match "planning-adhoc" (wants docs-\d{6}).
    # _find_placeholders DOES find plan-999999901-*.md.
    # → has_placeholder_docs=True → rename_unassigned fires.
    report = pre_ff_rename("planning-adhoc")

    assert report.commit is not None  # rename committed
    assert report.mapping  # P-placeholder → real number mapping nonempty
    # Placeholder file renamed:
    assert not (impl / "plan-999999901-adhoc-thing.md").exists()
```

**Dispatch note:** `rename_unassigned` needs an integration-branch to compute the next real plan number — check `tmp_repo` fixture setup (existing three tests at `:1885`/`:1924`/`:1953` have the wiring; mirror it). If the fixture doesn't establish a max-plan on the integration branch, `rename_unassigned` may fail to compute next-N; pre-seed `plan-0001-seed.md` on the base commit. discovered_from=reviewer.
...
- `r[store.gc.sweep-cycle-reclaim]` — T502 verifies (cross-batch cycle; existing `sweep_reclaims_two_cycle` covers intra-batch only). Spec text at [`store.md:258-260`](../../docs/src/components/store.md) already says "full unreachable set" — this is coverage, not a spec change.
...
- `r[sched.admin.capacity-manifest]` — T499 verifies (first `r[verify]` for this marker; [`manifest.rs:15`](../../rio-scheduler/src/admin/manifest.rs) has `r[impl]` only; marker at [`scheduler.md:164`](../../docs/src/components/scheduler.md))

No marker for T500 — config validation is defensive plumbing, same category as T71 (no `r[sched.config.*]` domain).
