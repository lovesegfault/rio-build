...
### T965470101 — `fix(harness):` dag_flip update-ref fallback — loud not silent
...
And add `ref_forced: bool = False` to `DagFlipResult` at [`models.py:488-509`](../../.claude/lib/onibus/models.py) so mergers can report it in `MergerReport`. Initialize `ref_forced = False` before the `if` at `:217`, pass to the final `DagFlipResult(...)` at `:230`. discovered_from=414. **Soft-conflict [P996547011](plan-996547011-dag-flip-already-done-double-bump.md):** both touch `merge.py:201-229` — T965470101 at `:217-223` (fallback block), P996547011-T3 at `:201-206` (already-done branch); non-overlapping hunks. Both also touch `models.py` `DagFlipResult` — T965470101 adds `ref_forced` field, P996547011-T5 edits `amend_sha` docstring; additive field-add + docstring-edit, rebase-clean.

### T965470102 — `refactor(scheduler):` find_cutoff_eligible DEAD — delete thin wrapper
...
- T965470101: `grep 'file=sys.stderr' .claude/lib/onibus/merge.py | grep -c 'dag_flip\|UNEXPECTED'` → ≥1 (stderr log in update-ref fallback); `grep 'ref_forced' .claude/lib/onibus/models.py` → ≥1 hit in DagFlipResult (field added)
- T965470102: `grep 'pub fn find_cutoff_eligible\b' rio-scheduler/src/dag/mod.rs` → 0 hits (thin wrapper deleted — `find_cutoff_eligible_speculative` stays); `cargo clippy -p rio-scheduler --all-targets -- --deny warnings` — no unused-fn (confirms deletion, not just private)
- T965470102: `grep 'find_cutoff_eligible\b' rio-scheduler/src/ -r | grep -v '_speculative'` → 0 non-`_speculative` hits in code (all 4 doc-comment mentions rewritten to reference `_speculative` or `cascade_cutoff`)
...
[
  ...
  {"path": ".claude/lib/onibus/merge.py", "action": "MODIFY", "note": "T965470101: :217-223 update-ref fallback +stderr print + ref_forced tracking. discovered_from=414. Soft-conflict P996547011-T3 at :201-206 (non-overlapping)"},
  {"path": ".claude/lib/onibus/models.py", "action": "MODIFY", "note": "T965470101: DagFlipResult +ref_forced bool field at :488-509. discovered_from=414. Soft-conflict P996547011-T5 (amend_sha docstring edit, rebase-clean)"},
  {"path": "rio-scheduler/src/dag/mod.rs", "action": "MODIFY", "note": "T965470102: :513-515 delete find_cutoff_eligible thin wrapper (0 callers post-P0405); :17 + :517 doc-comment rewrites. discovered_from=405. T169 SCOPE-+2: :635,:646 MAX_CASCADE rename targets added by P0405. Soft-conflict T169 (:14-21), P0405 (:567-646 — all DONE)"},
  {"path": "rio-scheduler/src/actor/completion.rs", "action": "MODIFY", "note": "T965470102: :77,:363 doc-comment rewrite (find_cutoff_eligible → _speculative/cascade_cutoff). T169 SCOPE-+2: :68,:84,:511 MAX_CASCADE rename targets added by P0405. discovered_from=405. HOT count=31 — comment-only, zero behavior"},
  {"path": "rio-scheduler/src/state/derivation.rs", "action": "MODIFY", "note": "T965470102: :242 doc-comment rewrite (consumed-by find_cutoff_eligible → cascade_cutoff via _speculative). discovered_from=405. count=20 — comment-only"}
]
```
