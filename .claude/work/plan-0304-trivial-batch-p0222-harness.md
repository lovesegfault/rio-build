...
Rename `MAX_CASCADE_DEPTH` → `MAX_CASCADE_NODES`. Update the warn-log field name (grep `depth,` near `tracing::warn!` in the cascade loop). Keep metric name `rio_scheduler_ca_cutoff_depth_cap_hits_total` (non-breaking); clarify semantics in its `describe_counter!` doc-string ("node-count cap, not tree-depth — operator should review if non-zero"). discovered_from=252. **Soft-conflict [P995943541](plan-995943541-all-deps-completed-skipped-hang.md)-T1:** both touch `dag/mod.rs` — T169 at `:14-21`, that plan at `:411-421`; non-overlapping.

### T170 — `docs(dashboard):` graphLayout.ts sourcePosition comment — delete or rewrite

rev-p280 doc-bug. MODIFY [`rio-dashboard/src/lib/graphLayout.ts`](../../rio-dashboard/src/lib/graphLayout.ts) at `:118-122`. Comment says `sourcePosition`/`targetPosition` use string literals (`"bottom"`/`"top"`) but code at `:128-139` does NOT set them. Edge routing works via [`DrvNode.svelte`](../../rio-dashboard/src/components/DrvNode.svelte) `<Handle>` components (`Position.Top`/`Position.Bottom`). Comment describes a mechanism that was removed or never implemented. Delete, or rewrite to explain Handles-drive-routing. **Post-P0280-merge.** discovered_from=280. **Soft-conflict [P995943542](plan-995943542-graph-page-skipped-worker-race.md)-T1:** same file, non-overlapping (`:43-83` vs `:118-122`).

### T171 — `refactor(dashboard):` Graph.svelte poll — stop once all nodes terminal

**SUPERSEDED BY [P995943542](plan-995943542-graph-page-skipped-worker-race.md)-T3.** This T-item tracked the rev-p280 trivial finding at [`Graph.svelte:186`](../../rio-dashboard/src/pages/Graph.svelte) (5s poll runs unconditionally after build terminal). That plan absorbs it as T3 since it's the same edit region as the Worker-race fix (T2 there). If P995943542 dispatches, skip T171. If P995943542 is deferred/rejected, T171 stands alone: `r.nodes.every(n => TERMINAL.has(n.status)) → clearInterval`. **Post-P0280-merge.** discovered_from=280.
...
- T171: SKIP if P995943542 dispatched (superseded); otherwise `grep 'TERMINAL\|clearInterval' rio-dashboard/src/pages/Graph.svelte` → ≥2 hits
...
[
  ...
  {"path": "rio-scheduler/src/dag/mod.rs", "action": "MODIFY", "note": "T169: :14-21 MAX_CASCADE_DEPTH→MAX_CASCADE_NODES rename + warn-log field update. discovered_from=252. Soft-conflict P995943541-T1 at :411 (non-overlapping)"},
  ...
  {"path": "rio-dashboard/src/lib/graphLayout.ts", "action": "MODIFY", "note": "T170: :118-122 delete/rewrite stale sourcePosition comment (post-P0280-merge). discovered_from=280. Soft-conflict P995943542-T1 at :43-83 (non-overlapping)"}
]
```
