...
- [P0451](plan-0451-crate-crd-proto-rename.md) merged (rio-worker→rio-builder crate rename; T237-T238 sweep the leftovers)
...
### T237 — `fix(builder):` root span + bootstrap component name "worker" → "builder"

[`rio-builder/src/main.rs:56`](../../rio-builder/src/main.rs) passes `"worker"` to `rio_common::server::bootstrap(...)`; [`:69`](../../rio-builder/src/main.rs) sets `tracing::info_span!("worker", component = "worker", ...)`. [P0455](plan-0455-tracey-markers-docs-rename.md) updated [`observability.md`](../../docs/src/observability.md) to spec `rio_builder_*` metrics, but the log/trace `component` field stays `"worker"` — log aggregation queries on `component=builder` miss all builder logs. Structured-logging-spec violation. Two-string fix: `"worker"` → `"builder"` at both sites. **Check [P0452](plan-0452-scheduler-fod-hard-split-routing.md) worktree first** — agent was steered to fix this inline; if `p452`'s `main.rs:56,69` already says `"builder"`, skip.

### T239 — `refactor:` worker→executor identifier sweep — fn names, local vars, tracing fields

Post-[P0451](plan-0451-crate-crd-proto-rename.md) rename leftovers in code identifiers:

- [`rio-builder/src/main.rs:805`](../../rio-builder/src/main.rs): `fn resolve_worker_identity` → `resolve_executor_identity` (+ callsite at `:62`)
- [`rio-scheduler/src/db/derivations.rs:92`](../../rio-scheduler/src/db/derivations.rs): `fn append_failed_worker` → `append_failed_executor` (+ 2 callsites)
- [`rio-scheduler/src/actor/build.rs:99,118`](../../rio-scheduler/src/actor/build.rs), [`completion.rs:514`](../../rio-scheduler/src/actor/completion.rs), [`dispatch.rs:308,464,477`](../../rio-scheduler/src/actor/dispatch.rs): `let Some(worker) = self.executors.get(...)` → `let Some(executor) = ...`
- [`rio-controller/src/reconcilers/builderpool/mod.rs:613,620`](../../rio-controller/src/reconcilers/builderpool/mod.rs): tracing field `worker=%executor_id` → `executor=%executor_id`

Mechanical rename — `cargo build` catches misses.

### T240 — `refactor(controller):` worker→builder k8s resource names — label, container, nix option

- [`rio-controller/src/reconcilers/builderpool/builders.rs:61`](../../rio-controller/src/reconcilers/builderpool/builders.rs): label `app.kubernetes.io/component: worker` → `builder`
- [`builders.rs:625`](../../rio-controller/src/reconcilers/builderpool/builders.rs): container `name: worker` → `builder`. Adjust [`builders_tests.rs`](../../rio-controller/src/reconcilers/builderpool/tests/builders_tests.rs) selectors that match on these.
- [`nix/modules/builder.nix:139`](../../nix/modules/builder.nix): option name `workerId` → `executorId` (writes correct `RIO_EXECUTOR_ID` env var already; option name is stale)

**Check [P0453](plan-0453-controller-builderpool-fetcherpool-split.md) worktree first** — its refactor extracts `builders.rs` to `reconcilers/common/sts.rs`; the `:61`/`:625` refs may have moved. If so, apply the rename at the new location.

### T241 — `refactor:` worker→builder/executor user-facing strings — logs, errors, help text, metric descriptions

~20 sites where log/error/help text says "worker" post-rename:

- [`rio-builder/src/executor/mod.rs:314,321`](../../rio-builder/src/executor/mod.rs) (leaked-mount msg)
- [`inputs.rs:140`](../../rio-builder/src/executor/inputs.rs) ("worker-side")
- [`cgroup.rs:481,497,545,547`](../../rio-builder/src/cgroup.rs)
- [`runtime.rs:692`](../../rio-builder/src/runtime.rs) ("task panicked")
- [`rio-scheduler/src/actor/completion.rs:379,543,1308`](../../rio-scheduler/src/actor/completion.rs)
- [`dispatch.rs:472,551`](../../rio-scheduler/src/actor/dispatch.rs)
- [`rio-scheduler/src/lib.rs:59,78,115,144,149`](../../rio-scheduler/src/lib.rs) (metric descriptions)
- [`rio-builder/src/main.rs:83,817`](../../rio-builder/src/main.rs) (`--worker-id` help text, `RIO_WORKER_ID`/`worker.toml` error msg)

Context-sensitive: in scheduler code the executor abstraction is "executor" (covers both builder+fetcher); in builder-specific code it's "builder". `git grep -in '\bworker\b' -- rio-*/src/ | grep -v 'test\|//\|r\['` for the sweep; hand-review each.

### T238 — `refactor:` worker→executor file renames + test fixtures + CSS

- `git mv rio-scheduler/src/admin/tests/workers_tests.rs rio-scheduler/src/admin/tests/executors_tests.rs` (content already uses `listExecutors`)
- `git mv rio-dashboard/src/pages/Workers.svelte rio-dashboard/src/pages/Executors.svelte` + `__tests__/Workers.test.ts` → `Executors.test.ts`; update route/nav refs
- [`rio-dashboard/src/components/DrvNode.svelte:71`](../../rio-dashboard/src/components/DrvNode.svelte): CSS `class=worker` → `class=executor` (+ matching `.worker` selector in `<style>`)
- [`DrvNode.svelte:77`](../../rio-dashboard/src/components/DrvNode.svelte): while here, replace `svelte-ignore a11y-click-events-have-key-events` suppression with an `onkeydown` handler (pre-existing, unrelated to rename)
- Test fixture literals `test-worker`/`worker-1`/`worker-0` → `test-builder`/`builder-1`/`builder-0`: [`spawn.rs:381`](../../rio-builder/src/executor/daemon/spawn.rs), [`stderr_loop.rs`](../../rio-builder/src/executor/daemon/stderr_loop.rs) (6 sites), [`executor/mod.rs:1517`](../../rio-builder/src/executor/mod.rs), [`log_stream.rs:210,241`](../../rio-builder/src/log_stream.rs), [`runtime.rs`](../../rio-builder/src/runtime.rs) (6 sites), [`builders_tests.rs:612`](../../rio-controller/src/reconcilers/builderpool/tests/builders_tests.rs)

### T242 — `chore:` add license field to workspace-hack + xtask Cargo.toml

`cargo deny check` baseline-red: [`workspace-hack/Cargo.toml`](../../workspace-hack/Cargo.toml) and [`xtask/Cargo.toml`](../../xtask/Cargo.toml) lack `license = "..."`. Both are internal/unpublished; add `license = "MIT OR Apache-2.0"` (or match the workspace root's license) and `publish = false` if not already set. Found by P0451 merger mode-4.

### T243 — `chore(crds):` regen CRD yaml to fix format drift

`crds-drift` check baseline-red: workerpool YAML has `|-` vs quoted-scalar format mismatch, carried through P0451's rename to builderpool. `nix develop -c cargo xtask regen crds` regenerates from `rio-crds/src/` definitions; commit the output.

### T244 — `fix(harness):` merge-count.txt bump race post-P0451

[`.claude/state/merge-count.txt`](../../.claude/state/merge-count.txt) stuck at `1` after two merges (P0450+P0451). P0451 merger's dag-flip amend raced with the count-bump write. Root cause: the count-bump and the `git commit --amend` aren't atomic. Fix: bump count BEFORE the amend (so a race leaves count high, not low — cadence agents trigger early rather than never), or use `flock` around the read-increment-write. Manually correct to `2` as part of this fix.

### T245 — `refactor(scheduler):` executor_service.rs — warn on unknown ExecutorKind enum value

[`rio-scheduler/src/grpc/executor_service.rs:377`](../../rio-scheduler/src/grpc/executor_service.rs): `ExecutorKind::try_from(req.kind).unwrap_or(Builder)` is silent on unknown values. During rolling-upgrade skew (newer executor sends a kind value the scheduler doesn't recognize), this degrades to Builder silently — operationally invisible. Add `tracing::warn!` on the `Err` arm before the `Builder` default so operators see the version mismatch. The `Builder` default itself is correct (safe: unrecognized-kind executors won't receive FODs, no airgap violation per comment at `:373-376`). discovered_from=452.

```rust
let kind = rio_proto::types::ExecutorKind::try_from(req.kind).unwrap_or_else(|_| {
    tracing::warn!(raw_kind = req.kind, "unknown ExecutorKind value — version skew? defaulting to Builder");
    rio_proto::types::ExecutorKind::Builder
});
```

### T246 — `refactor(dashboard):` Executors.svelte+test — drop semantically-wrong kind-gate tracey annotations
...
- [`netpol.nix:6`](../../nix/tests/scenarios/netpol.nix) — "allows ... fod-proxy:3128 ONLY" → drop the fod-proxy clause
- [`netpol.nix:28`](../../nix/tests/scenarios/netpol.nix) — "FOD proxy egress allowlist: P0243" → drop (P0243 is OBE)
- [`lib/derivations.nix:48,54`](../../nix/tests/lib/derivations.nix) — "for the fod-proxy scenario" → "for the netpol scenario" (the FOD test moved)
- [`lib/derivations/env-dump.nix:3`](../../nix/tests/lib/derivations/env-dump.nix) — "fod-proxy scenario asserts" → "netpol scenario asserts"
- [`lib/derivations/fod-fetch.nix:1`](../../nix/tests/lib/derivations/fod-fetch.nix) — same
- [`common.nix:541`](../../nix/tests/common.nix) — "(fod-proxy uses" → update or drop the parenthetical
- [`fixtures/k3s-full.nix:50-51,150,191`](../../nix/tests/fixtures/k3s-full.nix) — "dockerImages.fod-proxy" / "fodProxy.enabled=true" / "rio-fod-proxy.tar.zst" → these reference the deleted image; drop or note as historical

The two `default.nix` hits (`:68`, `:627`) are INTENTIONAL "removed per ADR-019" tombstones — leave those. discovered_from=bughunter.

### T251 — `fix(ci):` helm-lint nondeterminism — known-flake entry
...
### T252 — `docs(helm):` vmtest-full-nonpriv.yaml — add commented seccompInstaller null-clear stub
...
### T253 — `refactor(helm):` DRY DNS egress stanza → _helpers.tpl rio.netpol.dnsEgress
...
### T254 — `fix(xtask):` k8s namespace gaps for 4-ns split
...
### T255 — `refactor(store):` tenant_upstreams.trusted_keys — CHECK constraint or M_026 doc
...
### T256 — `refactor(store):` add #[instrument] spans to stub RPCs list/add/remove_upstream
...
### T257 — `perf(store):` rio_store_substitute_total — drop or bucket upstream URL label
...
### T258 — `refactor(cli):` --sig-mode ValueEnum
...
### T259 — `refactor(cli):` upstream add — require at least one --trusted-key
...
### T260 — `refactor(store):` check_available double-swallow — collapse to single error-handling layer
...
### T261 — `fix(tooling):` onibus _next_real — scan docs-branch worktree, not main
...
### T262 — `test(builder):` FLAKE triage — test_concurrent_waiters_no_eagain_during_slow_fetch
...
### T263 — `docs(dag):` backfill P0470-P0473 dag rows
...
jq -r '.panels[].targets[]?.expr' \
  ...
  | { ! grep -qv -- '-builders-'; } \
  || { echo "FAIL: builder-utilization.json has cAdvisor query WITHOUT -builders- infix" >&2; exit 1; }
```
...
def clear_halt() -> bool:
    ...
    try:
        _QUEUE_HALTED.unlink(missing_ok=False)
        return True
...
- Lift `from onibus.merge import coverage_maybe_halt` to module top (preferred — simpler, one import site)
- Or rewrite the comment to state the REAL rationale if one exists (lazy-load for startup cost? unlikely for a local module)
...
```yaml
- id: dag-validate
  name: validate dag.jsonl
  entry: .claude/bin/onibus dag validate
  language: system
  files: ^\.claude/dag\.jsonl$
...
.await
{
    tracing::debug!(error = %e, "heartbeat_uploading UPDATE failed (non-fatal)");
}
```
...
| Site | Form |
|---|---|
| [`xtask/src/k8s/k3s/smoke.rs:14-15`](../../xtask/src/k8s/k3s/smoke.rs) | `const SCHED_PORT`/`STORE_PORT` |
| [`xtask/src/k8s/eks/smoke.rs:30-31`](../../xtask/src/k8s/eks/smoke.rs) | `const SCHED_PORT`/`STORE_PORT` (duplicate decl) |
| [`xtask/src/k8s/status.rs:128`](../../xtask/src/k8s/status.rs) | inline `CliCtx::open(client, 19001, 19002)` |
| [`xtask/src/k8s/mod.rs:199,202`](../../xtask/src/k8s/mod.rs) | `#[arg(long, default_value_t = 19001)]` / `19002` |

Hoist to ONE `pub const` pair in [`xtask/src/k8s/mod.rs`](../../xtask/src/k8s/mod.rs) (near the `NS`/`NS_STORE` namespace consts):

```rust
/// Default local ports for scheduler/store port-forward. Chosen to
/// avoid the well-known 9001/9002 (MinIO/portainer) on dev machines.
pub const DEFAULT_SCHED_PORT: u16 = 19001;
pub const DEFAULT_STORE_PORT: u16 = 19002;
```
...
Recurring — coverage sink flagged it as "blocks dag-flip amend" with no file:line because the hang manifests as a silent stall, not an error. The merger appears busy; `/dag-tick` sees the lock held with no progress.

Two-part fix:

1. **Timeout the amend.** Wrap the `git("commit", "--amend", ...)` subprocess call with a timeout (e.g., 30s — generous for a no-edit amend). On timeout, raise with a diagnostic naming the likely cause:

```python
try:
    git("commit", "--amend", "--no-edit", cwd=REPO_ROOT, timeout=30)
except subprocess.TimeoutExpired:
    raise SystemExit(
        "dag-flip: git commit --amend hung >30s. Likely cause: "
        "commit.gpgsign=true + gpg.format=ssh with a dead SSH agent. "
        "Check: ssh-add -l (should list keys, not hang). "
        "Workaround: SSH_AUTH_SOCK= git commit --amend --no-edit "
        "(unsigned) or restart the agent."
    )
```

2. **Pre-flight probe (optional).** Before the amend, if `gpg.format=ssh` is configured, run `ssh-add -l` with a short timeout (2s). If it hangs or errors, warn early — but don't block (the user may have a non-agent signing key). The timeout in (1) is the real guard; the probe is faster feedback.
...
```rust
/// PostgreSQL `deadlock_detected`. Raised when two transactions hold
/// row locks in circular wait. See `with_sorted_retry` for the
/// sort-before-bind mitigation; a single defensive retry is permitted.
pub(crate) const SQLSTATE_DEADLOCK: &str = "40P01";
```
...
### T502 — `docs(scheduler):` test_inject_ready docstring — enumerate stubs

[`rio-scheduler/src/actor/mod.rs:1217-1222`](../../rio-scheduler/src/actor/mod.rs) `test_inject_ready` docstring names the one use-case ("populating `compute_capacity_manifest` preconditions") but omits the stub list. The body at [`:1230-1236`](../../rio-scheduler/src/actor/mod.rs) silently sets `required_features: vec![]`, `expected_output_paths: vec![]`, `output_names: vec!["out".into()]`, `is_fixed_output: false`, `is_ca: false`. On top of that, `from_recovery_row` at [`:1239`](../../rio-scheduler/src/actor/mod.rs) inherently can't populate `drv_content`, `ca_modular_hash`, or `assigned_size_class` (not columns on `RecoveryDerivationRow`).

[P0502](plan-0502-builderpool-sizing-enum.md) through [P0505](plan-0505-per-pod-idle-grace-manifest-scaledown.md) are all scheduled on the same ADR-020 area. A feature-aware manifest test (e.g., "verify `required_features` filters into the RPC response") that reaches for this helper gets silent empties — test passes for the wrong reason.

Extend the docstring at [`:1217`](../../rio-scheduler/src/actor/mod.rs):
...
///
/// **Stubbed** (reaching for any of these silently gets the default):
/// - `required_features = []`
/// - `expected_output_paths = []`
/// - `output_names = ["out"]` (single output)
/// - `is_ca = false`, `is_fixed_output = false`
/// - `drv_content`, `ca_modular_hash`, `assigned_size_class`: NOT
///   populated (`from_recovery_row` inherent — not columns on
///   `RecoveryDerivationRow`)
///
/// If your test needs any of the above, extend the signature or
/// use the full MergeDag path.
```
...
### T503 — `docs(gateway):` cancel_active_builds docstring — 3→4 paths + idle_timeout reason
...
### T504 — `docs(gateway):` r[gw.conn.cancel-on-disconnect+2] — add mid-opcode path + tracey bump
...
Per T503: `session.rs:24` and `gateway.md:540` are each one behind, each missing a DIFFERENT path. One sweep reconciles both to the code-reality of 4.
...
### T505 — `fix(controller):` SchedulerUnreachable condition — pass rpc_name param
...
### T506 — `fix(controller):` ephemeral label selector — add sizing filter
...
### T507 — `refactor(controller):` floor job name — drop redundant g/m suffixes
...
Cosmetic only — name is operator-ergonomic, labels carry the functional data. Fix since we're touching the area for P0507 anyway:
...
### T508 — `docs(scheduler):` scheduler.md:481 — fix ../../ → ../../../ link depth
...
### T509 — `docs(scheduler):` assignment.rs:806 — "different match arms" → combined arm
...
### T510 — `docs(scheduler):` estimator.rs:86 — "startup and Tick" → tick_count%6 only
...
### T511 — `perf(scheduler):` dispatch_ready — collapse 3 estimator lookups to 1
...
### T512 — `docs(nix):` flake.nix #crds comment — 3 CRDs + Helm + split-crds.sh path
...
The safer policy is contiguous-across-docs, which the merge.py check trivially passes. Update SKILL.md and the rio-planner system prompt to match — one authoritative scheme beats "the per-doc scheme works IF you never cross-reference."

Fix [`SKILL.md:41`](../../.claude/skills/plan/SKILL.md):

```markdown
**T-placeholder scheme (batch-appends):** `T9<runid><NN>` where `<NN>` is a
**single contiguous sequence across ALL batch docs in this run** — NOT
per-doc restart. First append to P0304 gets `T9<runid>01`, second `T9<runid>02`;
first append to P0311 continues at `T9<runid>03`. merge.py:843 asserts no
cross-doc collision on the placeholder key; contiguous-global trivially
satisfies it. docs-897333 hit a per-doc collision — one policy, no edge cases.
```
...
Missing `drv_name`. [`models.py:196,240`](../../.claude/lib/onibus/models.py) `KnownFlake` model REQUIRES `drv_name` for VM-test entries (`test.startswith("vm-")` → validator at `:242` raises if `drv_name is None`). `_VM_FAIL_RE` matches against `drv_name`, not `test`. P0509 plan doc omitted it; implementer caught at dispatch.

Fix the template at `:114`:

```json
{"test":"<flake-attr-name>","drv_name":"<rio-... for VM tests; omit for nextest>","symptom":"<brief>","root_cause":"<brief>","fix_owner":"P<NNNN>","fix_description":"<what>","retry":"Once"}
```

Add a note below: "`drv_name` is the `<N>` in `vm-test-run-<N>.drv` from `nix/tests/scenarios/*.nix` `name = \"rio-...\"` — NOT the default.nix attrset key. Required for VM-test entries; pydantic rejects without it."
...
Retry gate is unaffected (keys on `drv_name="rio-observability"` which IS correct) — but `onibus flakes remove`/`expire` keyed on `test` won't find this entry. Inconsistent with siblings.
...
P0503 mis-tagged this at write-time, assuming P0505 would cover both reconcile-scale-down AND finalizer-DrainExecutor. It didn't. This will appear in `grep 'TODO(P0505)' | <P0505-DONE>` stale-TODO audit.
...
Same pattern as pre-existing `error_counts` at [`:77`](../../rio-controller/src/reconcilers/mod.rs) — that one also never gets removed. Batch both:

```rust
// In finalizer cleanup, after Job-mode early-return check at :593:
let pool_key = format!("{ns}/{name}");
ctx.manifest_idle.lock().expect("poisoned").remove(&pool_key);
ctx.error_counts.lock().expect("poisoned").remove(&pool_key);
...
The heuristic's intent (`:534-536`): ≥3 **root scenarios** red = infrastructure break. Cascade products shouldn't count. Fix: strip the `-standalone` suffix and drop aggregate drvs before dedup. Or simpler — count only `vm-test-run-` captures (the root scenarios), not `rio-cov-` (cascade products of the per-test-cov pipeline):

```python
# :528-532 — tighten to root scenarios only
_COV_SCENARIO_FAIL_RE = re.compile(
    r"^error: Cannot build '/nix/store/[a-z0-9]+-"
    r"vm-test-run-([\w-]+)\.drv'",   # drop the (?:...|rio-cov) alternation
    re.MULTILINE,
)
# ≥3 distinct root scenarios. rio-cov-* drvs are cascade products
# (per-test lcov wrapper, aggregate merge) — one root scenario
# failing cascades to multiple rio-cov failures. mc=56 false-
# positive: ONE observability SIGTERM → 3 drv failures → halted.
```

Add a comment at `:549` explaining why the alternation was narrowed. **Followup said `tick.py`** but the logic lives in `merge.py:528-550` (`coverage_full_red`) — tick.py has no coverage-halt code. discovered_from=coordinator (mc=56 false-positive).

### T519 — `fix(tooling):` `_INFRA_ERROR_RE` — add exit-code-143 (SIGTERM) alongside exit-137 (SIGKILL)

[`build.py:189`](../../.claude/lib/onibus/build.py) `_INFRA_ERROR_RE` gained an exit-137 (SIGKILL, 128+9) pattern via P0508 after a 289-file corpus scan. The sibling **exit-143 (SIGTERM, 128+15)** hit the same `^[ \t]+Reason:` continuation-line format at mc=51 (fetcher-split) and mc=56 (observability): `Reason: builder failed with exit code 143.` — keynes under load SIGTERMing builders during cleanup. Corpus hits: `merge-151.log`, `merge-161.log`.

Same format, same fix — extend the alternation:

```python
# build.py — alongside the existing exit-137 arm
r"|^[ \t]+Reason: builder .*exit code (?:137|143)"   # 128+SIGKILL(9) | 128+SIGTERM(15)
```

Both are external-kill-on-remote-builder (not a test failure). discovered_from=coordinator (mc=51, mc=56 corpus).

### T520 — `fix(tooling):` `AgentRow.status` Literal rejects stale values — log+skip instead of crash

`AgentStatus` at [`models.py:27`](../../.claude/lib/onibus/models.py) is `Literal["running", "done", "consumed"]`. Prior-session `agents-running.jsonl` rows had `status="cancelled"` (from the Agent() tool-wiring-regression era) and `status="partial"` (coordinator-direct rows). `read_jsonl` at [`jsonl.py:40`](../../.claude/lib/onibus/jsonl.py) does `model.model_validate_json(line)` — pydantic raises `ValidationError` on the Literal mismatch → `/dag-tick` and `archive-agents` crash on ANY stale row in the file.

Manually scrubbed via `grep -vE` this time. Two routes:

**(a)** Extend the Literal: `Literal["running", "done", "consumed", "cancelled", "partial"]`. Cheap, but every future schema drift needs another extend.

**(b)** Make `read_jsonl` resilient: wrap `model_validate_json` in try/except, log the bad line + skip. A corrupt/stale row shouldn't brick the whole tick machinery.

```python
# jsonl.py:36-43 — option (b)
def read_jsonl(path: Path, model: type[T]) -> list[T]:
    ...
    for i, line in enumerate(path.read_text().splitlines(), 1):
        ...
        try:
            ...
            # Stale/corrupt row shouldn't brick tick/archive. Log
            # with line number so the operator can scrub manually.
            print(f"[read_jsonl] {path.name}:{i} skipped: {e.errors()[0]['msg']}", file=sys.stderr)
    return rows
```

**Prefer (b)** — more robust against the general class, and `agents-running.jsonl` is transient state (stale rows are expected after agent-framework changes). `known-flakes.jsonl` and `dag.jsonl` are less transient; a validation error there IS worth failing loudly on — but those use `read_jsonl` too. **Route (b) needs a way to opt OUT of skip-on-error for the files where strictness matters.** Consider a `strict: bool = True` kwarg; `agents-running.jsonl` callers pass `strict=False`. discovered_from=coordinator (manual scrub).

### T521 — `fix(tooling):` `-c commit.gpgsign=false` at `merge.py` amend+commit sites

[`merge.py:266`](../../.claude/lib/onibus/merge.py) `git commit --amend --no-edit` is documented at [`:258-265`](../../.claude/lib/onibus/merge.py) as having broken under `commit.gpgsign=true` (P0504 merger, no TTY → SSH signing helper fails). P0514 made the `GitError` self-diagnosing but did NOT fix the config-dependency. The comment says "If signing is re-enabled, this breaks again."

Also [`merge.py:1038`](../../.claude/lib/onibus/merge.py) — the plan-number-assignment commit — has the same no-TTY exposure (pre-existing, not P0514-introduced).

The amend changes the tree so the old signature is invalid regardless; adding `-c commit.gpgsign=false` makes it config-immune:

```python
# :266
git("-c", "commit.gpgsign=false", "commit", "--amend", "--no-edit", cwd=REPO_ROOT)
# :1038
git("-c", "commit.gpgsign=false", "commit", "-m", f"docs: assign ...", cwd=worktree)
```

Update the `:258-265` comment from "Resolved by the user disabling signing" to "config-immune via -c commit.gpgsign=false". discovered_from=514.

### T522 — `docs(store):` `sweep()` doc-comment — name the distinctness precondition

[P0449](plan-0449-gc-sweep-temp-table-anti-join.md) replaced the `<> ALL($2)` re-check with a temp-table anti-join. The `INSERT INTO sweep_unreachable` at [`sweep.rs:146`](../../rio-store/src/gc/sweep.rs) has no `ON CONFLICT` — duplicate hashes in `unreachable` → PRIMARY KEY violation → `SweepAbort::Db`. The old `<> ALL($2)` tolerated dups silently (array membership doesn't care about multiplicity).

Current caller `mark::compute_unreachable` guarantees distinct (narinfo PK join at [`mark.rs:96-98`](../../rio-store/src/gc/mark.rs)), so risk is zero today. Failure is loud (PK violation text in `SweepAbort::Db`), so the contract self-documents at runtime. But the doc-comment at [`sweep.rs:82-106`](../../rio-store/src/gc/sweep.rs) doesn't mention the precondition.

Add one line to the doc-comment, before `:107` (the `pub async fn sweep` line):

```rust
/// PRECONDITION: `unreachable` must contain distinct hashes. The
/// temp-table INSERT has no ON CONFLICT — duplicates fail with a
/// PRIMARY KEY violation (SweepAbort::Db). `mark::compute_unreachable`
/// guarantees this via its narinfo PK join.
```

Pure doc hygiene. discovered_from=449.

### T523 — `docs(plan):` P0513 scope-update — P0511 did NOT extract `is_failed_job`; now dedup THREE sites not one

[P0513](plan-0513-job-common-extraction.md) `§ Dependencies` soft-dep note at [`:129`](../../.claude/work/plan-0513-job-common-extraction.md) says: "If P0511 lands FIRST, its is_failed_job predicate goes into job_common too." **P0511 is DONE and did NOT create `is_failed_job`** — it inlined the Failed-Job filter predicate at TWO sites:

- [`manifest.rs:237`](../../rio-controller/src/reconcilers/builderpool/manifest.rs) — `failed_total` count: `.filter(|j| j.status.as_ref().and_then(|s| s.failed).unwrap_or(0) > 0)`
- [`manifest.rs:839`](../../rio-controller/src/reconcilers/builderpool/manifest.rs) — `select_failed_jobs` body: same byte-for-byte closure

Plus a THIRD site in tests: `failed_jobs_excluded_from_ceiling` at [`manifest_tests.rs:1253-1260`](../../rio-controller/src/reconcilers/builderpool/tests/manifest_tests.rs) copies the `active_jobs` predicate inline (`status.succeeded==0 && status.failed==0`). The comment at `:1251` is prose-cross-ref not code-cross-ref — **the test tests its own copy, not prod code**. A prod filter change would silently NOT be caught.

P0513 must now extract + dedupe, not just receive. Update P0513's plan doc:

1. **Segments table** (`:8-16`): add a row for `is_failed_job` (new extraction, 2 prod sites).
2. **T1 contents** (`:45`): add `pub(super) fn is_failed_job(j: &Job) -> bool` alongside `is_active_job`. Document them as inverses on the Failed axis (not full complements — `succeeded>0` is neither active nor failed).
3. **T2** (`:71`): mention both `:237` and `:839` as migration sites.
4. **Dependencies** `:129` note: replace "If P0511 lands FIRST, its is_failed_job predicate goes into job_common" with the reality: "P0511 DONE; inlined at :237 + :839 — P0513 extracts from both."
5. **NEW** T-task (T5 or extend T4): `manifest_tests.rs:1253` imports `is_active_job` from `job_common` instead of reimplementing the closure. Closes the test-copies-prod-filter gap — test body becomes `jobs.iter().filter(|j| is_active_job(j))`.

Crate convention is free-fn predicates (`is_floor_job` at [`manifest.rs:515`](../../rio-controller/src/reconcilers/builderpool/manifest.rs) is precedent). discovered_from=511. **Consolidates two followup rows** (predicate-dup doc-bug + test-copies-filter test-gap — both fold into P0513's scope, one plan-doc edit covers both).

### T524 — `refactor(store):` `make_trusted_key` test helper → delegate to `Signer::trusted_key_entry`

[`signing.rs:761-767`](../../rio-store/src/signing.rs) `make_trusted_key` constructs `{name}:{STANDARD.encode(pk)}` — byte-for-byte the body of [P0478](plan-0478-sig-visibility-gate-cluster-key-union.md)'s new [`Signer::trusted_key_entry`](../../rio-store/src/signing.rs) at `:242-246`. Both compute the same `name:base64(verifying_key)` format. The helper predates the method.

Replace the helper body with delegation — doubles as a regression assertion that the test helper and the prod method agree on the format:

```rust
fn make_trusted_key(name: &str, seed: &[u8; 32]) -> String {
    Signer::from_seed(name, seed).trusted_key_entry()
}
```

Four callers at `:776,:798,:815`+ stay unchanged. ~5L dropped. discovered_from=478.

### T525 — `refactor(tooling):` raw `subprocess.run` git sites → `git_ops` wrapper

[P0514](plan-0514-merge-amend-subprocess-env.md) added `GitError` (stderr in `__str__`) to [`git_ops.git()`](../../.claude/lib/onibus/git_ops.py) at `:33-59` specifically so exit-128 failures surface their stderr. But four raw `subprocess.run` git sites bypass the wrapper — the diagnostic improvement is incomplete.

**No-check rev-parse (3 sites, merge.py):** [`merge.py:83-87`](../../.claude/lib/onibus/merge.py) (lock: `main_at_acquire`), [`:121-125`](../../.claude/lib/onibus/merge.py) (lock_status: `current`), [`:170-174`](../../.claude/lib/onibus/merge.py) (MergeSha: `tip`). All do `capture_output=True, text=True` with no `check=True` → non-zero gives `stdout=""` → caller treats as falsy. Semantically equal to [`git_try()`](../../.claude/lib/onibus/git_ops.py) `:63-68` returning `None` (both falsy for `if tip:`). ~15L drops to ~3L:

```python
# :83 — lock()
"main_at_acquire": git_try("rev-parse", "--short", "HEAD") or "",
# :121 — lock_status()
current = git_try("rev-parse", "--short", INTEGRATION_BRANCH) or ""
# :170 — MergeSha append
tip = git_try("rev-parse", INTEGRATION_BRANCH) or ""
```

Add `from onibus.git_ops import git_try` to merge.py's imports if not present.

**check=True diff (1 site, cli.py):** [`cli.py:468-472`](../../.claude/lib/onibus/cli.py) `_cmd_plan` qa-check: `subprocess.run([...git diff...], check=True)` CAN raise opaque `CalledProcessError` — the exact exit-128-invisible-stderr hazard P0514 was about. This is the load-bearing one. Replace with `git_ops.git()`:

```python
from onibus.git_ops import git
changed = git(
    "diff", "--name-only", f"{INTEGRATION_BRANCH}..HEAD", "--",
    ".claude/work/plan-*.md", cwd=worktree,
).split()
```

discovered_from=consolidator. **Worth it if:** another merger-path exit-128 turns up (has happened twice); cli.py:468 has the hazard live today.

### T526 — `refactor(controller):` extract delete-Job 404-tolerant triad → `job_common::delete_job_tolerant`

[P0511](plan-0511-manifest-failed-job-sweep.md) added a SECOND copy of the delete-Job 404-triad. Sites:
- [`manifest.rs:338-360`](../../rio-controller/src/reconcilers/builderpool/manifest.rs) — surplus-idle delete (P0505)
- [`manifest.rs:411-427`](../../rio-controller/src/reconcilers/builderpool/manifest.rs) — failed-sweep delete (P0511)

Shape: `match jobs_api.delete → Ok(_) {info!} / Err(Api(404)) {debug! already-gone} / Err(e) {warn! retry-next-tick}`. Cousin at [`builderpool/mod.rs:665`](../../rio-controller/src/reconcilers/builderpool/mod.rs) (StatefulSet delete — same pattern, different API). Ok branches differ: surplus-idle logs `bucket = ?parse_bucket_from_labels(job)`, failed-sweep does not.

**P0513 DONE (mc=68)** — [`job_common.rs`](../../rio-controller/src/reconcilers/builderpool/job_common.rs) is the home. P0513's charter was manifest↔ephemeral sharing; this is intra-manifest, so it didn't cover it (correctly scoped).

Helper returns enum so callers control logging:

```rust
// job_common.rs
pub(super) enum DeleteOutcome {
    Deleted,
    AlreadyGone,  // 404 — another tick raced us, or ownerRef GC
    Failed(kube::Error),
}

pub(super) async fn delete_job_tolerant(
    api: &Api<Job>, name: &str,
) -> DeleteOutcome {
    match api.delete(name, &DeleteParams::default()).await {
        Ok(_) => DeleteOutcome::Deleted,
        Err(kube::Error::Api(ae)) if ae.code == 404 => DeleteOutcome::AlreadyGone,
...
```rust
// manifest.rs:338 — surplus-idle
match delete_job_tolerant(&jobs_api, job_name).await {
    DeleteOutcome::Deleted => info!(
        pool = %name, job = %job_name, bucket = ?parse_bucket_from_labels(job),
        "deleted surplus manifest Job (idle grace elapsed)"
    ),
    DeleteOutcome::AlreadyGone => debug!(pool = %name, job = %job_name, "Job already deleted"),
    DeleteOutcome::Failed(e) => warn!(
        pool = %name, job = %job_name, error = %e,
        "failed to delete surplus manifest Job; will retry next tick"
    ),
}
// manifest.rs:411 — failed-sweep — same shape, no bucket field
```

~30L → ~20L net. discovered_from=consolidator. **Pre-extract before** [P0516](plan-0516-manifest-quota-deadlock-sweep-before-spawn.md) moves the sweep block (no 3rd copy planned, but the reorder will touch this range — cleaner to extract first).

### T527 — `refactor(test):` BuilderPool heredoc YAML at 4 sites — extract `mk_builderpool_yaml` helper OR document threshold

P0512 added the 3rd and 4th inline BuilderPool heredoc YAML to [`lifecycle.nix`](../../nix/tests/scenarios/lifecycle.nix):
- [`:2079-2116`](../../nix/tests/scenarios/lifecycle.nix) — ephemeral apply (full, ~37L)
- [`:2384-2420`](../../nix/tests/scenarios/lifecycle.nix) — manifest apply (full, ~36L)
- [`:2048-2065`](../../nix/tests/scenarios/lifecycle.nix) — CEL-reject dry-run (deadlineSeconds, ~17L)
- [`:2354-2368`](../../nix/tests/scenarios/lifecycle.nix) — CEL-reject dry-run (Manifest+maxBuilds, ~14L)

[`:2076-2078`](../../nix/tests/scenarios/lifecycle.nix) comment says duplication is deliberate: "inline so the test doc is self-contained (no external fixture file to drift)." At 4 sites with ~15L overlap each (apiVersion/kind/metadata/namespace/spec.replicas/autoscaling/maxConcurrentBuilds/fuseCacheSize/systems), a helper is borderline.

**Route A (extract):** Python helper in the scenario prelude, not a Nix helper — the heredocs are inside Python strings already:

```python
def mk_builderpool_yaml(name, *, sizing=None, ephemeral=None, extra=""):
    s = "sizing: Manifest\n" if sizing else ""
    e = f"ephemeral: {str(ephemeral).lower()}\n" if ephemeral is not None else ""
    return (
        f"apiVersion: rio.build/v1alpha1\nkind: BuilderPool\n"
        f"metadata:\n  name: {name}\n  namespace: ${nsBuilders}\n"
        f"spec:\n  {s}{e}"
        f"  replicas: {{min: 0, max: 4}}\n"
        f"  autoscaling: {{metric: queueDepth, targetValue: 2}}\n"
        f"  maxConcurrentBuilds: 1\n  fuseCacheSize: 5Gi\n"
        f"  systems: [x86_64-linux]\n  image: rio-all:dev\n{extra}"
    )
```

CEL-reject cases pass the invalid field combo via `extra=`. Apply cases use defaults.

**Route B (document threshold, do nothing):** Add to the `:2076-2078` comment: "4 sites as of P0512. Extract to mk_builderpool_yaml if a 5th appears — threshold chosen because CEL-reject cases need deliberately-invalid specs that a helper's 'valid defaults' would mask."

**Prefer Route B.** The CEL-reject cases NEED to show the exact invalid YAML to a reader; wrapping them in a helper loses the "self-contained test doc" property the comment defends. The 2 full-apply cases have more divergence (tlsSecretName, image tag comments, privileged) than overlap. discovered_from=512. **Judgment call — Route A only if a 5th appears.**

### T528 — `refactor(controller):` `Bucket` alias home → `builderpool/mod.rs`

[`job_common.rs:35`](../../rio-controller/src/reconcilers/builderpool/job_common.rs) `pub(crate) type Bucket = (u64, u32)` — 0 uses in ephemeral.rs, 0 in job_common.rs itself (only the definition), 24 in manifest.rs. Placed in job_common.rs for `pub(crate)` visibility (`mod job_common` is `pub(super)`, `mod manifest` is not).

Creates two import paths: [`manifest.rs:86`](../../rio-controller/src/reconcilers/builderpool/manifest.rs) re-exports `pub(super) use super::job_common::Bucket` so `manifest_tests.rs:25` can `use ...::manifest::{Bucket, ...}`. But [`reconcilers/mod.rs:35`](../../rio-controller/src/reconcilers/mod.rs) uses `builderpool::job_common::Bucket` directly for `ManifestIdleState`.

Cleaner: move to [`builderpool/mod.rs`](../../rio-controller/src/reconcilers/builderpool/mod.rs) — neutral, same `pub(super)` visibility tier, drops the re-export. `manifest.rs` imports `super::Bucket`. `reconcilers/mod.rs:35` becomes `builderpool::Bucket`. Tests import `builderpool::Bucket`.

```rust
// builderpool/mod.rs — near the top, after mod declarations
/// `(memory_bytes, cpu_millicores)` — scheduler pre-bucketed
/// (admin_types.proto:234, 4GiB/2000m grid). Manifest-reconciler
/// grouping key; also `Ctx::manifest_idle`'s per-pool idle state.
...
| Line | Token |
|---|---|
| [`:571`](../../nix/tests/scenarios/lifecycle.nix) | `Tracey: r[verify sec.jwt.pubkey-mount] lives at the default.nix` |
| [`:3013`](../../nix/tests/scenarios/lifecycle.nix) | `Tracey: r[verify ctrl.fetcherpool.reconcile] + ...` |
| [`:3138`](../../nix/tests/scenarios/lifecycle.nix) | `Tracey: r[verify sched.store-client.reconnect] at default.nix` |
| [`:3284`](../../nix/tests/scenarios/lifecycle.nix) | `Tracey: r[verify sec.psa.control-plane-restricted] at` (P0500 new) |
| [`:3433`](../../nix/tests/scenarios/lifecycle.nix) | `Tracey: r[verify sched.grpc.leader-guard] at default.nix` (P0500 new) |

CLAUDE.md VM-test placement rule: scenario header blocks MAY keep prose descriptions, MUST NOT carry the marker token itself. Currently invisible to tracey (`config.styx:76` `test_include = nix/tests/default.nix` only) — hygiene risk if widened. P0500 followed the in-file precedent at `:571/:3013/:3138`.

**Related but distinct:** [P0295](plan-0295-doc-rot-batch-sweep.md)-T497 covers the `# ── r[rule.id]:` divider pattern at `:2454/:2481/:2498` + `dashboard-gateway.nix:310`. That's a different pattern (box-drawing section headers). This task covers the `Tracey: r[verify ...]` prose-breadcrumb pattern. Both are scannable-token pollution; different grep signatures.

Reword to keep the human breadcrumb, drop the scannable token. E.g. `:3284`:

```python
# Tracey: sec.psa.control-plane-restricted (verify) wired at
...
[`health-shared` subtest at `:694`](../../nix/tests/scenarios/lifecycle.nix) has the same pattern: `standby = next(p for p in all_sched if p != leader)` — would `StopIteration` if all pods == leader (impossible but same missing guard).

One-line assert before each:

```python
# :3442 (bootstrap-tenant) — before standby_pods = [...]
assert leader in all_sched, (
    f"Lease holderIdentity {leader!r} not in running pods {all_sched}. "
    f"Stale Lease — pod deleted but Lease not yet reacquired. Retry "
    f"or wait for leader-election to settle."
)
```
...
// After (clippy-recommended):
residues.sort_by_key(|r| std::cmp::Reverse(r.1));
```
...
### T532 — [SUPERSEDED by P0531] `fix(tooling):` TRACEY_DOMAINS drift — spec has builder/fetcher, constant has worker

> **Superseded:** [P0531](plan-0531-onibus-pytest-ci-gate.md) T1 includes this 2-string sync AND adds the `.#ci` gate that makes the drift-detector actually visible. Part (a)+(b) together — part (a) alone (this T) leaves the test un-gated and the next domain-add will rot the same way. Idempotent: if this T merges first anyway, P0531-T1 is an empty diff.
...
[`tracey.py:15-17`](../../.claude/lib/onibus/tracey.py):
```python
TRACEY_DOMAINS: frozenset[str] = frozenset({
...
The test scans `docs/src/` for `r[domain.*]` prefixes and asserts equality. If spec has `r[builder.*]` and `r[fetcher.*]` markers but the constant doesn't list them, `TRACEY_MARKER_RE` (built from `TRACEY_DOMAIN_ALT`) won't MATCH those markers — `onibus plan tracey-markers` silently skips them, `tracey query` via onibus misses them.

**Not in `.#ci` gates** — no pytest check in `checks.x86_64-linux`. P0517 implementer noticed running pytest locally. Red-on-local green-on-CI divergence.

Fix route: (a) sync constant to spec (add `builder`, `fetcher`; investigate `worker` — is it stale or does spec need it?), (b) add pytest to `.#ci`. **Do (a) here; (b) is a broader scope decision** (all test_scripts.py tests would gate). discovered_from=517.

### T533 — `refactor(tooling):` pad `fix_owner: P518` → `P0518` in known-flakes + lifecycle.nix

[`known-flakes.jsonl:14`](../../.claude/known-flakes.jsonl) `"fix_owner":"P518"` — sibling entries use padded `P0179`/`P0176`/`P0489`/`P0509`. [`_OWNER_RE` at models.py:177](../../.claude/lib/onibus/models.py) is `^P\d+( T\d+)?$` (accepts both) and `owner_plan` at `:251` `int()`-normalizes, so no functional bug — but `grep P0518` misses this entry during audits.

Same unpadded form at [`lifecycle.nix:2603`](../../nix/tests/scenarios/lifecycle.nix): `# P518: 60s was tight under coverage-mode instrumentation`.

Fix: `s/P518/P0518/` in both spots. **Optional hardening:** tighten `_OWNER_RE` to `^P0\d{3,}( T\d+)?$` — forces padding, rejects `P518` at write time. That's a behavior change (new validation error) so check for other unpadded entries first: `grep -oP '"fix_owner":"P\d+"' .claude/known-flakes.jsonl | grep -v 'P0'`. discovered_from=518.

### T534 — `fix(tooling):` coverage-full subshell re-import merge.py — P0517 fix had a ~25min delayed-activation window

Stale-subshell halt at 2026-03-30T01:26:41Z: halt message showed OLD-code scenario names (`rio-lifecycle-core`, `vm-lifecycle-core-k3s`, `vm-total` — the cascade pattern P0517 fixed). P0517 merged at mc=74 (~01:05). Live `coverage_full_red` test returns n=1 on the same logs.

**Theory:** merger step-6 backgrounded coverage subshell (`nohup nix build .#coverage-full &`) loaded `merge.py` BEFORE P0517's ff-merge, called `coverage_maybe_halt` AFTER (long-lived process, cached imports). Python caches module bytecode at import time; a 25min coverage run launched at mc=73 sees pre-P0517 logic even though the source file changed on disk.

**Harmless today** (stale halt, coordinator clears) but means ANY merge.py fix has a delayed-activation window equal to the longest in-flight coverage runtime (~25min). A CORRECTNESS fix to `coverage_maybe_halt` would appear to not work for 25 minutes after merge.

**Fix options, pick at dispatch:**

- **(a) Re-exec:** step-6 subshell `os.execv(sys.executable, [sys.executable, __file__, ...])` before calling `coverage_maybe_halt`. Fresh interpreter, fresh imports. Heavyweight — kills any in-process state.
- **(b) Re-import:** `importlib.reload(merge)` before the call. Lighter but brittle (reload doesn't cascade to transitive imports; `from merge import X` bindings in the caller's namespace DON'T update).
- **(c) Split:** move `coverage_maybe_halt` to a standalone script step-6 `subprocess.run`s — fresh interpreter per-call, no reload games. Cleanest if the function is self-contained.
- **(d) Defer:** step-6 doesn't call `coverage_maybe_halt` directly — it writes a `CoverageResult` row and `/dag-tick` (a FRESH process per-tick) does the halt decision. If already structured this way, the stale import was somewhere else — re-trace.

Option (c) or (d) preferred. discovered_from=517.

### T535 — `docs(controller):` `select_failed_jobs` doc-comment stale — `sweep_cap()` extracted the cap formula

[`manifest.rs:834-835`](../../rio-controller/src/reconcilers/builderpool/manifest.rs) doc-comment: "Callers compute `cap` as `max(FAILED_SWEEP_MIN, spec.replicas.max)`". Stale post-[P0520](plan-0520-sweep-cap-extract-clamp.md) extraction — callers now call [`sweep_cap(replicas.max)` at :242](../../rio-controller/src/reconcilers/builderpool/manifest.rs). The inline expression no longer exists at any call site and omits the `.max(0)` clamp that motivated P0520.

Fix: either (a) `s/compute .cap. as .*/call [\`sweep_cap\`] — tracks replicas.max with a FAILED_SWEEP_MIN floor and an i32 .max(0) clamp/`, or (b) drop the clause entirely — [`sweep_cap`'s own doc-comment at :133-141](../../rio-controller/src/reconcilers/builderpool/manifest.rs) covers the formula. Prefer (a) — cross-reference keeps `select_failed_jobs` readers from having to grep for where `cap` comes from. discovered_from=520.

### T536 — `docs(merger):` note nix remote-build double-dispatch hang — kill+rerun works

P0519 `coverage-full` hung 32 minutes. P0518's parallel coverage on a neighboring worktree finished the SAME `cov-vm-*` drvs (identical hashes). P0519's `nix` had already dispatched to `ssh-ng://keynes`, waited indefinitely for a zombie remote build whose outputs already existed in the local store. `kill`+rerun completed in <10min (cached).

**Possible gap:** remote-builder timeout config. `nix.conf` `builders-use-substitutes = true` is set (remote builders check for existing paths before building), but once the build is DISPATCHED the local nix waits for the remote to finish, even if a neighbor process puts the same path in the local store mid-build.

**Low-priority** (rare: requires parallel coverage runs + identical-hash drv + dispatch race). `kill`+rerun works. But it's operational friction under parallel CI — 32min of wasted wallclock.

**Fix options, note in [`.claude/skills/merge-impl.md`](../../.claude/skills/merge-impl.md) or [`.claude/rules/coverage.md`](../../.claude/rules/coverage.md):**

- **(a) Timeout:** `timeout 1800 nix build .#coverage-full || nix build .#coverage-full` — second attempt picks up anything the first finished + cached.
- **(b) Local-only for coverage:** `--builders ''` forces local build for coverage-full. Coverage runs are ~25min either way; eliminating the remote dispatch race costs nothing if the local host has KVM capacity.
- **(c) Documentation only:** add a "known hang pattern" note in the merger skill: if coverage-full exceeds 30min with no log progress, `kill` and rerun — it's likely the double-dispatch hang, not a real failure.

Option (c) is the minimal fix. Option (b) if parallel-CI frequency increases. discovered_from=519.

### T537 — `fix(store):` `SignerError::TenantKeyLookup` docstring + message stale — used for cluster-key-history load too

[`signing.rs:131-139`](../../rio-store/src/signing.rs) docstring: "DB lookup of a tenant's signing key" and "cluster-key fallback never touches the DB so never hits this variant". **Both claims false post-[P0521](plan-0521-cluster-key-rotation-contradiction.md):** [`load_prior_cluster` at :307-311](../../rio-store/src/signing.rs) wraps `load_cluster_key_history` in `SignerError::TenantKeyLookup(e.to_string())` — that's a DB hit, cluster-side not tenant-side.

**User-facing error:** `cluster_key_history load: tenant key lookup: <db error>` — double-context that contradicts itself ("cluster key history" then "tenant key").

**Fix routes, prefer (b):**

- **(a) Rename:** `TenantKeyLookup` → `KeyLookup`. Broadens the name to cover both. Breaks any `match` on the variant — grep callers first.
- **(b) New variant:** add `ClusterKeyHistory(String)` alongside `TenantKeyLookup`. `:310` swaps to the new variant. Docstring at `:131-137` stays correct (tenant-specific). New variant gets its own docstring. Message: `"cluster key history load: {0}"` — no "tenant" in it. **Cleaner:** preserves the tenant-vs-cluster distinction at the type level.
- **(c) Minimal:** update docstring to acknowledge cluster use, change message to `"key lookup: {0}"`. Name stays misleading but it's one-line.

[P0528](plan-0528-cluster-key-history-load-validation.md) touches `load_prior_cluster` (`:307-311`) — coordinate: if this lands first, P0528's edited function uses the new variant; if P0528 lands first, this plan edits the P0528-expanded function. Both additive, clean either order. discovered_from=521.

### T538 — `refactor(nix):` removeAttrs exclusion assert-guard — silent no-op on rename

[`flake.nix:1224`](../../flake.nix) `removeAttrs (...) [ "vm-lifecycle-prod-parity-k3s" ]` excludes prod-parity from `vmTestsCov` (PSA test vs coverage-mode PSA bump — [`:1219-1223`](../../flake.nix) has the rationale). `removeAttrs` is silent on missing keys (verified) — if [`nix/tests/default.nix:855`](../../nix/tests/default.nix) ever renames `vm-lifecycle-prod-parity-k3s`, the exclusion silently becomes a no-op and coverage-full goes red again (prod-parity re-enters the coverage set, asserts `readOnlyRootFilesystem=true` under privileged PSA, fails).

Precedent for the fix EXISTS in the same file: [`codecov-matrix-sync` at :2203](../../flake.nix) uses `assert pkgs.lib.assertMsg`.
...
vmTestsCov =
  let
    ...
    excl = "vm-lifecycle-prod-parity-k3s";
  in
  assert pkgs.lib.assertMsg (builtins.hasAttr excl allCov) ''
    vmTestsCov exclusion ${excl} not found — renamed in nix/tests/default.nix?
    Either update this exclusion or the test was deleted (then drop this exclusion).
  '';
  ...
  # fails. ... [rest of :1219-1223 comment unchanged]
  removeAttrs allCov [ excl ];
```

[P0525](plan-0525-ci-missing-codecov-matrix-sync.md) touches `flake.nix` `:1267-1293` — DIFFERENT section (`:1209-1224` vs `:1267`), clean rebase. discovered_from=519.

### T539 — `refactor(nix):` dead `cpuHints.vm-fod-proxy-k3s` — test deleted per ADR-019

[`flake.nix:1179`](../../flake.nix) `vm-fod-proxy-k3s = 8;` in `cpuHints`, but the test was deleted per ADR-019 ([`nix/tests/default.nix:753-755`](../../nix/tests/default.nix) confirms: "vm-fod-proxy-k3s removed per ADR-019 — Squid proxy deleted. FODs route to FetcherPool"). `cpuHints.${name} or 4` at [`:1198`](../../flake.nix) falls through silently — zero runtime impact. Maintenance noise; misleads readers about which tests exist.

**While there:** audit all `cpuHints` keys against the actual test attrset — same silent-fallthrough class as T538's removeAttrs issue. An assert-guard is possible:

```nix
# After cpuHints definition, before mapAttrs:
let deadHints = builtins.filter (k: !(builtins.hasAttr k allTests)) (builtins.attrNames cpuHints);
in assert pkgs.lib.assertMsg (deadHints == [ ]) ''
  cpuHints has entries for nonexistent tests: ${builtins.toJSON deadHints}
  Test deleted/renamed without cleaning up its cpuHints entry.
'';
```
...
### T540 — `refactor(nix):` githubActions.matrix.checks — second manual-list drift footgun

[`flake.nix:1533-1546`](../../flake.nix) `githubActions.matrix.checks` is a manual `inherit (miscChecks) deny tracey-validate helm-lint crds-drift tfvars-fresh;` allowlist. When a constituent is added to `miscChecks` at `:556+`, it must ALSO be manually added here or it doesn't run on GitHub CI. Second manual-list footgun — same drift class as the cpuHints table (T539, [P0533](plan-0533-cpuhints-k3s-suffix-fallthrough.md)) and `removeAttrs` exclusion (T538).

[P0531](plan-0531-onibus-pytest-ci-gate.md) T3 hits this directly: adds `onibus-pytest` to miscChecks AND has to remember to add it to the inherit list. That plan does the one-line add; this T is the structural fix.
...
### T541 — `refactor(controller):` SpawnOutcome — add #[must_use] to match doc-comment claim
...
### T542 — `fix(tooling):` known-flakes dup-key guard — validate on read, not just on add
...
Prefer (a) — matches `onibus dag validate` pattern. Add a call to it in [P0531](plan-0531-onibus-pytest-ci-gate.md)'s `onibus-pytest` checkPhase if that lands, or as its own `checks.known-flakes-validate` constituent (overkill for one assert — prefer folding into onibus-pytest via a test fn). discovered_from=bughunter.

### T543 — `docs(controller):` SPAWN_FAIL_THRESHOLD const-assert bound — clarify 20 is NOT FAILED_SWEEP_MIN
...
### T544 — `test(store):` parse_64_byte_nix_format — assert fake pubkey is actually ignored
...
### T545 — `refactor(scheduler):` peak_memory_bytes clamp — warn when it fires
...
### T546 — `refactor(test):` extract complete_with_metrics helper — 6th inline occurrence
...
- T237: `grep 'component = "worker"' rio-builder/src/main.rs` → 0 hits; `grep '"builder"' rio-builder/src/main.rs` → ≥2 hits (bootstrap arg + span)
- T239: `grep 'resolve_worker_identity\|append_failed_worker' rio-*/src/` → 0 hits
- T240: `grep 'component: worker\|name: worker' rio-controller/src/` → 0 hits (or moved to common/sts.rs post-P0453)
- T241: `git grep -c '\bworker\b' -- rio-builder/src/ rio-scheduler/src/lib.rs | grep -v test` substantially reduced vs baseline
- T238: `ls rio-scheduler/src/admin/tests/executors_tests.rs rio-dashboard/src/pages/Executors.svelte` — both exist; old paths gone
- T242: `cargo deny check licenses` → green (workspace-hack + xtask licensed)
- T243: `nix develop -c cargo xtask regen crds && git diff --exit-code` — regen idempotent
- T244: `cat .claude/state/merge-count.txt` ≥ 2; merger script bump precedes amend
...
- T251: `grep 'helm-lint\|enableMutatingMethods' .claude/known-flakes.jsonl` → ≥1 hit
- T252: `grep 'seccompInstaller' infra/helm/rio-build/values/vmtest-full-nonpriv.yaml` → ≥1 hit (commented stub present)
- T253: `grep -c 'rio.netpol.dnsEgress' infra/helm/rio-build/templates/_helpers.tpl infra/helm/rio-build/templates/networkpolicy.yaml` → ≥6 (1 define + 5 includes)
- T254: `grep 'rio-store\|rio-builders\|rio-fetchers' xtask/src/k8s/rook.rs xtask/src/k8s/eks/destroy.rs xtask/src/k8s/k3s/mod.rs` → correct namespaces at flagged sites
- T255: `grep 'empty.*trusted_keys\|trusted_keys.*empty' rio-store/src/migrations.rs rio-store/src/grpc/admin.rs` → ≥2 hits (doc + runtime check)
- T256: `grep -c '#\[instrument' rio-store/src/grpc/admin.rs` increased by 3 (list/add/remove_upstream)
...
- T260: `grep 'warn!' rio-store/src/substitute.rs` near `:591` → ≥1 hit (swallow-once with log)
...
- T503: `grep -c 'four session-exit paths\|idle_timeout' rio-gateway/src/session.rs` ≥ 2 (docstring + reason string); `grep 'three session-exit' rio-gateway/src/session.rs` → 0 hits
- T504: `nix develop -c tracey query rule gw.conn.cancel-on-disconnect` shows `+3` version; `grep 'All three paths\|Both paths' docs/src/components/gateway.md` near `:540` → 0 hits; `grep 'All four paths' docs/src/components/gateway.md` → ≥1 hit
- T505: `grep 'rpc_name: &str' rio-controller/src/reconcilers/builderpool/ephemeral.rs` → ≥1 hit; `grep '"ClusterStatus RPC failed"' rio-controller/src/reconcilers/builderpool/ephemeral.rs` → 0 hits (hardcoded string gone)
- T506: `grep 'SIZING_EPHEMERAL' rio-controller/src/reconcilers/builderpool/ephemeral.rs` → ≥2 hits (selector + label-set); `grep 'SIZING_LABEL.*SIZING_EPHEMERAL' rio-controller/src/reconcilers/builderpool/ephemeral.rs` → ≥1 hit
- T507: `grep 'mf-floor-' rio-controller/src/reconcilers/builderpool/manifest.rs` → ≥1 hit; `grep 'floorg-floorm' rio-controller/src/reconcilers/builderpool/tests/manifest_tests.rs` → 0 hits
- T508: `grep '\.\./\.\./rio-builder' docs/src/components/scheduler.md` → 0 hits; `grep '\.\./\.\./\.\./rio-builder/src/cgroup' docs/src/components/scheduler.md` → ≥1 hit
- T509: `grep 'different match arms' rio-scheduler/src/assignment.rs` → 0 hits; `grep 'combined.*None | Some(0)' rio-scheduler/src/assignment.rs` → ≥1 hit
...
- T518: `grep 'rio-cov' .claude/lib/onibus/merge.py` → 0 hits inside `_COV_SCENARIO_FAIL_RE` body (alternation narrowed to `vm-test-run-` only); `pytest .claude/lib/test_scripts.py -k cov_scenario` → green if a test exists, or add one asserting `coverage_full_red` returns count=1 for a log with one vm-test-run + two rio-cov cascades
- T519: `grep 'exit code (?:137|143)\|exit code 143' .claude/lib/onibus/build.py` → ≥1 hit; grep `merge-151.log` / `merge-161.log` with the updated regex (if corpus retained) → match
- T520: either `grep 'cancelled\|partial' .claude/lib/onibus/models.py` → ≥2 hits in `AgentStatus` Literal (route-a), OR `grep 'ValidationError' .claude/lib/onibus/jsonl.py` → ≥1 hit + `grep 'strict' .claude/lib/onibus/jsonl.py` → ≥1 hit (route-b); `pytest .claude/lib/test_scripts.py -k 'read_jsonl.*stale\|read_jsonl.*skip'` → ≥1 passed
- T521: `grep -c 'commit.gpgsign=false' .claude/lib/onibus/merge.py` → ≥2 hits (both `:266` amend + `:1038` commit); `grep 'Resolved by the user disabling' .claude/lib/onibus/merge.py` → 0 hits (stale resolution note replaced)
- T522: `grep 'PRECONDITION.*distinct\|distinct hashes' rio-store/src/gc/sweep.rs` → ≥1 hit in `sweep()` doc-comment before `:107`
- T523: `grep 'inlined at :237.*:839\|did NOT create is_failed_job' .claude/work/plan-0513-job-common-extraction.md` → ≥1 hit; `grep 'manifest_tests.rs:1253\|test-copies-prod' .claude/work/plan-0513-job-common-extraction.md` → ≥1 hit (third-site test-import coverage noted)
- T524: `grep 'Signer::from_seed.*trusted_key_entry' rio-store/src/signing.rs` → ≥1 hit in `make_trusted_key` body; `grep 'STANDARD.encode(pk' rio-store/src/signing.rs` → ≤1 hit (only `trusted_key_entry` itself, not the test helper)
- T525: `grep -c 'subprocess.run.*git' .claude/lib/onibus/merge.py` → significantly reduced (3 rev-parse sites gone); `grep 'git_try.*rev-parse' .claude/lib/onibus/merge.py` → ≥3 hits; `grep 'subprocess.run.*git.*diff' .claude/lib/onibus/cli.py` → 0 hits near `:468`
...
- T533: `grep -c 'P0518' .claude/known-flakes.jsonl` ≥ 1; `grep -c '"fix_owner":"P518"' .claude/known-flakes.jsonl` → 0 (padded); `grep 'P518:' nix/tests/scenarios/lifecycle.nix` → 0 hits; `grep 'P0518:' nix/tests/scenarios/lifecycle.nix` → ≥1 hit at `:2603`
- T534: re-import/re-exec present at step-6 coverage-maybe-halt callsite, OR `coverage_maybe_halt` moved to standalone script (option-c), OR `/dag-tick` owns the halt decision (option-d — verify structure)
- T535: `grep "Callers compute .cap. as" rio-controller/src/reconcilers/builderpool/manifest.rs` → 0 hits; `grep 'sweep_cap' rio-controller/src/reconcilers/builderpool/manifest.rs` near `:834` → either cross-ref present (route-a) or clause dropped (route-b)
- T536: `grep 'double-dispatch\|zombie remote\|kill.*rerun' .claude/skills/merge-impl.md .claude/rules/coverage.md` → ≥1 hit (hang pattern documented)
...
- T540: `grep -A5 'inherit (miscChecks)' flake.nix | grep -c '^\s*[a-z]'` equals `nix eval --impure --expr 'builtins.length (builtins.attrNames (import ./flake.nix).miscChecks)'` (option-a blocklist parity) OR `grep 'removeAttrs miscChecks' flake.nix` → ≥1 hit at `:~1537` OR `grep 'SYNC-WITH: miscChecks' flake.nix` → ≥2 hits (option-b comment-pair)
- T541: `grep '#\[must_use' rio-controller/src/reconcilers/builderpool/job_common.rs` → ≥1 hit at `:~113`; `nix develop -c cargo clippy --all-targets -- --deny warnings` green (existing match sites already exhaustive)
- T542: `onibus flake validate` exits 0 on clean known-flakes.jsonl; manually add dup `test` key → exits nonzero with a message naming BOTH line numbers
- T543: `grep 'coincidental\|neither derives' rio-controller/src/reconcilers/builderpool/manifest.rs` → ≥1 hit in `:163-171` comment region
- T544: `grep 'fake pubkey.*ignored\|trusted the wire pubkey' rio-store/src/signing.rs` → ≥1 hit after `:543`; `cargo nextest run -p rio-store parse_64_byte_nix_format` passes
- T545: `grep 'reported.*EiB\|clamping to i64::MAX' rio-scheduler/src/actor/completion.rs` → ≥1 hit before `:1016`
- T546: `grep 'fn complete_with_metrics\|complete_with_metrics(' rio-scheduler/src/actor/tests/completion.rs` → ≥5 hits (1 def + ≥4 callers); `wc -l rio-scheduler/src/actor/tests/completion.rs` reduced by ≥30 lines
...
[
  ...
  {"path": "rio-builder/src/main.rs", "action": "MODIFY", "note": "T237: :56,:69 component worker→builder. T239: :805 resolve_worker_identity rename. T241: :83,:817 help/error text. discovered_from=451"},
  {"path": "rio-scheduler/src/db/derivations.rs", "action": "MODIFY", "note": "T239: :92 append_failed_worker→append_failed_executor. discovered_from=451"},
  {"path": "rio-scheduler/src/actor/build.rs", "action": "MODIFY", "note": "T239: :99,:118 local var worker→executor. discovered_from=451"},
  {"path": "rio-scheduler/src/actor/dispatch.rs", "action": "MODIFY", "note": "T239: :308,:464,:477 local var. T241: :472,:551 log text. discovered_from=451"},
  {"path": "rio-controller/src/reconcilers/builderpool/mod.rs", "action": "MODIFY", "note": "T239: :613,:620 tracing field worker→executor. discovered_from=451"},
  {"path": "rio-controller/src/reconcilers/builderpool/builders.rs", "action": "MODIFY", "note": "T240: :61 k8s label, :625 container name worker→builder. Check P0453 — may have moved to common/sts.rs. discovered_from=451"},
  {"path": "nix/modules/builder.nix", "action": "MODIFY", "note": "T240: :139 option workerId→executorId. discovered_from=451"},
  {"path": "rio-builder/src/executor/mod.rs", "action": "MODIFY", "note": "T241: :314,:321 leaked-mount msg. T238: :1517 fixture literal. discovered_from=451"},
  {"path": "rio-builder/src/executor/inputs.rs", "action": "MODIFY", "note": "T241: :140 worker-side text. discovered_from=451"},
  {"path": "rio-builder/src/cgroup.rs", "action": "MODIFY", "note": "T241: :481,:497,:545,:547 log text. discovered_from=451"},
  {"path": "rio-builder/src/runtime.rs", "action": "MODIFY", "note": "T241: :692 task-panicked msg. T238: 6 fixture literals. discovered_from=451"},
  {"path": "rio-scheduler/src/lib.rs", "action": "MODIFY", "note": "T241: :59,:78,:115,:144,:149 metric descriptions. discovered_from=451"},
  {"path": "rio-scheduler/src/admin/tests/workers_tests.rs", "action": "RENAME", "note": "T238: → executors_tests.rs. discovered_from=451"},
  {"path": "rio-dashboard/src/pages/Workers.svelte", "action": "RENAME", "note": "T238: → Executors.svelte + route refs. discovered_from=451"},
  {"path": "rio-dashboard/src/components/DrvNode.svelte", "action": "MODIFY", "note": "T238: :71 CSS class, :77 a11y handler. discovered_from=451"},
  {"path": "rio-builder/src/executor/daemon/spawn.rs", "action": "MODIFY", "note": "T238: :381 fixture literal. discovered_from=451"},
  {"path": "rio-builder/src/executor/daemon/stderr_loop.rs", "action": "MODIFY", "note": "T238: 6 fixture literals. discovered_from=451"},
  {"path": "rio-builder/src/log_stream.rs", "action": "MODIFY", "note": "T238: :210,:241 fixture literals. discovered_from=451"},
  {"path": "workspace-hack/Cargo.toml", "action": "MODIFY", "note": "T242: add license field"},
  {"path": "xtask/Cargo.toml", "action": "MODIFY", "note": "T242: add license field"},
  {"path": ".claude/state/merge-count.txt", "action": "MODIFY", "note": "T244: correct to ≥2"},
  ...
  {"path": ".claude/known-flakes.jsonl", "action": "MODIFY", "note": "T251: helm-lint nondeterminism entry. discovered_from=coverage"},
  {"path": "infra/helm/rio-build/values/vmtest-full-nonpriv.yaml", "action": "MODIFY", "note": "T252: commented seccompInstaller null-clear stub :87. discovered_from=459"},
  {"path": "infra/helm/rio-build/templates/networkpolicy.yaml", "action": "MODIFY", "note": "T253: DRY DNS egress → _helpers.tpl rio.netpol.dnsEgress. discovered_from=459"},
  {"path": "infra/helm/rio-build/templates/_helpers.tpl", "action": "MODIFY", "note": "T253: add rio.netpol.dnsEgress named template"},
  {"path": "xtask/src/k8s/rook.rs", "action": "MODIFY", "note": "T254: rio-s3-creds → rio-store ns :85-94. discovered_from=454"},
  {"path": "xtask/src/k8s/eks/destroy.rs", "action": "MODIFY", "note": "T254: builderpool→rio-builders + add fetcherpool→rio-fetchers :19"},
  {"path": "xtask/src/k8s/k3s/mod.rs", "action": "MODIFY", "note": "T254: teardown rio-postgres in rio-store :132-135"},
  ...
  {"path": "rio-store/src/substitute.rs", "action": "MODIFY", "note": "T260: collapse double-swallow at :591. discovered_from=462"},
  ...
  {"path": "rio-gateway/src/session.rs", "action": "MODIFY", "note": "T503: docstring :24 three→four paths + idle_timeout reason at :42-44. discovered_from=444"},
  {"path": "docs/src/components/gateway.md", "action": "MODIFY", "note": "T504: r[gw.conn.cancel-on-disconnect+2] :540 three→four paths (add mid-opcode from P0331) + tracey bump →+3. discovered_from=444"},
  {"path": "rio-controller/src/reconcilers/builderpool/ephemeral.rs", "action": "MODIFY", "note": "T505: scheduler_unreachable_condition :351 add rpc_name param; T506: :139 selector + :413 labels add SIZING_EPHEMERAL. discovered_from=503"},
  {"path": "rio-controller/src/reconcilers/builderpool/manifest.rs", "action": "MODIFY", "note": "T505: :291 pass rpc_name; T507: :552-557 special-case bucket None → -mf-floor-. HOT — P0507 also touches this file (diff section :240-272). discovered_from=503"},
  {"path": "rio-controller/src/reconcilers/builderpool/tests/manifest_tests.rs", "action": "MODIFY", "note": "T507: floor-name assertion :600-619 floorg-floorm→-mf-floor-. discovered_from=503"},
  {"path": "docs/src/components/scheduler.md", "action": "MODIFY", "note": "T508: :481 link depth ../../ → ../../../. discovered_from=504"},
  {"path": "rio-scheduler/src/assignment.rs", "action": "MODIFY", "note": "T509: :806 test comment — different match arms → combined arm. discovered_from=504"},
  ...
  {"path": ".claude/lib/onibus/merge.py", "action": "MODIFY", "note": "T518: _COV_SCENARIO_FAIL_RE :528-532 drop rio-cov alternation (cascade products over-count); T521: -c commit.gpgsign=false at :266 amend + :1038 commit. discovered_from=coordinator+514"},
  {"path": ".claude/lib/onibus/build.py", "action": "MODIFY", "note": "T519: _INFRA_ERROR_RE add exit-143 (SIGTERM, 128+15) alongside exit-137 at :189+. Corpus: merge-151.log, merge-161.log. discovered_from=coordinator"},
  {"path": ".claude/lib/onibus/models.py", "action": "MODIFY", "note": "T520 route-a: extend AgentStatus Literal :27 with cancelled+partial. discovered_from=coordinator"},
  {"path": ".claude/lib/onibus/jsonl.py", "action": "MODIFY", "note": "T520 route-b (preferred): read_jsonl :36-43 try/except ValidationError + log+skip, strict kwarg opt-out. discovered_from=coordinator"},
  {"path": "rio-store/src/gc/sweep.rs", "action": "MODIFY", "note": "T522: sweep() doc-comment add PRECONDITION distinct-hashes line before :107. Pure doc. discovered_from=449"},
  {"path": ".claude/work/plan-0513-job-common-extraction.md", "action": "MODIFY", "note": "T523: P0513 scope-update — P0511 did NOT extract is_failed_job, inlined at manifest.rs:237+:839; +manifest_tests.rs:1253 test-copies-prod site. Segments table +row, T2 +2 sites, deps note rewrite. discovered_from=511"},
  {"path": "rio-store/src/signing.rs", "action": "MODIFY", "note": "T524: make_trusted_key :761-767 body → Signer::from_seed().trusted_key_entry(). discovered_from=478"},
  {"path": ".claude/lib/onibus/merge.py", "action": "MODIFY", "note": "T525: :83,:121,:170 raw subprocess.run rev-parse → git_try. discovered_from=consolidator. NOTE: P517 edits :528-549 same file — non-overlapping"},
  {"path": ".claude/lib/onibus/cli.py", "action": "MODIFY", "note": "T525: :468-472 subprocess.run git-diff check=True → git_ops.git(). Load-bearing exit-128 hazard. discovered_from=consolidator"},
  {"path": "rio-controller/src/reconcilers/builderpool/job_common.rs", "action": "MODIFY", "note": "T526: +DeleteOutcome enum +delete_job_tolerant. T528: delete Bucket alias :25-35. discovered_from=consolidator+513"},
  {"path": "rio-controller/src/reconcilers/builderpool/manifest.rs", "action": "MODIFY", "note": "T526: :338-360,:411-427 404-triad → delete_job_tolerant. T528: delete :82-86 Bucket re-export, import super::Bucket. Coordinates with P0516 sweep reorder (extract BEFORE P0516). discovered_from=consolidator+513"},
  {"path": "nix/tests/scenarios/lifecycle.nix", "action": "MODIFY", "note": "T527: :2076-2078 comment — document 4-site threshold (Route B prefer). discovered_from=512"},
  ...
  {"path": ".claude/known-flakes.jsonl", "action": "MODIFY", "note": "T533: :14 fix_owner P518 → P0518. discovered_from=518"},
  {"path": "nix/tests/scenarios/lifecycle.nix", "action": "MODIFY", "note": "T533: :2603 comment P518 → P0518. HOT — T529/T530 touch diff sections. discovered_from=518"},
  {"path": ".claude/lib/onibus/models.py", "action": "MODIFY", "note": "T533 (optional): tighten _OWNER_RE :177 to ^P0\\d{3,}. T1 also touches :322 (diff section). discovered_from=518"},
  {"path": ".claude/lib/onibus/build.py", "action": "MODIFY", "note": "T534: step-6 coverage subshell re-import/re-exec before coverage_maybe_halt, OR move to standalone script. P0527 touches :233/:240 (diff section — excusable logic). discovered_from=517"},
  ...
  {"path": "flake.nix", "action": "MODIFY", "note": "T538: :1209-1224 removeAttrs → let-assert-removeAttrs. T539: :1179 delete dead cpuHints entry + optional deadHints assert near :1198. T540: :1533-1546 matrix.checks allowlist→blocklist or comment-pair. HOT (count=48) — P0525 touches :1267-1293, P0531 touches :556+/:1537, P0533 touches :1209 (all diff sections). discovered_from=519/consolidator"},
  {"path": "rio-controller/src/reconcilers/builderpool/job_common.rs", "action": "MODIFY", "note": "T541: :113 add #[must_use] to SpawnOutcome enum. T526 touches delete_job_tolerant in same file (diff section). discovered_from=consolidator"},
  {"path": ".claude/lib/onibus/cli.py", "action": "MODIFY", "note": "T542: add flake validate subcommand near :404 _cmd_flake. T525 touches subprocess.run git sites (diff section). discovered_from=bughunter"},
  {"path": "rio-controller/src/reconcilers/builderpool/manifest.rs", "action": "MODIFY", "note": "T543: :163-171 comment — clarify 20≠FAILED_SWEEP_MIN coincidence. HOT — T531/:680, T535/:834, P0526/:360-386. Pure comment-add. discovered_from=reviewer"},
  {"path": "rio-store/src/signing.rs", "action": "MODIFY", "note": "T544: :543+ add fake-pubkey-rejected assert. T524/T537 touch diff sections (make_trusted_key, TenantKeyLookup docstring). discovered_from=bughunter"},
  {"path": "rio-scheduler/src/actor/completion.rs", "action": "MODIFY", "note": "T545: before :1016 add warn! when clamp fires. count=26 HOT — P0445 area (DONE), T54/T61 read-only refs. 5-line add. discovered_from=reviewer"},
  {"path": "rio-scheduler/src/actor/tests/completion.rs", "action": "MODIFY", "note": "T546: extract complete_with_metrics helper, swap ≥5 inline sites. P0311-T54/T61/T88 all add tests here (additive, diff test-fn names). discovered_from=consolidator"}
]
```
