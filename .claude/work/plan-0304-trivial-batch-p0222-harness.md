...
### T99931680101 — `refactor(scheduler):` size_classes cutoff ensure → validate_config()
...
### T99931680102 — `docs(notes):` bughunter-log.md — mc224 null-result entry
...
- T169: `grep 'MAX_CASCADE_NODES\|MAX_CASCADE_DEPTH' rio-scheduler/src/dag/mod.rs` → `MAX_CASCADE_NODES` ≥1 hit, `MAX_CASCADE_DEPTH` 0 hits (renamed)
...
- T99931680101: `grep 'cutoff_secs.is_finite' rio-scheduler/src/main.rs` → ≥1 hit INSIDE `validate_config` body (moved from :347 inline); `grep -A5 'for class in &cfg.size_classes' rio-scheduler/src/main.rs | grep -c 'anyhow::ensure'` → 1 (only the validate_config copy, not a duplicate left at :347)
- T99931680101: `cargo nextest run -p rio-scheduler config_` — no regressions (all P0409 rejection tests still pass; optionally +config_rejects_nan_cutoff passes)
- T99931680102: `grep 'mc=224' .claude/notes/bughunter-log.md` → ≥1 hit (entry added)
...
[
  ...
  {"path": "rio-scheduler/src/dag/mod.rs", "action": "MODIFY", "note": "T169: :14-21 MAX_CASCADE_DEPTH→MAX_CASCADE_NODES rename + warn-log field update. discovered_from=252. Soft-conflict P0399-T1 at :411 (non-overlapping)"},
  ...
  {"path": "rio-scheduler/src/main.rs", "action": "MODIFY", "note": "T99931680101: move size_classes cutoff_secs ensure from :347-358 inline → into validate_config() after :221. Keep :359-360 gauge-emit + :362-367 info! inline (pure fn). Optional +config_rejects_nan_cutoff test. discovered_from=409"},
  {"path": ".claude/notes/bughunter-log.md", "action": "MODIFY", "note": "T99931680102: +mc224 null-result cadence entry (dd7f370d..3d2f7f20, mc217-224, 5 target-verdicts all CLEAN). discovered_from=bughunter-mc224"}
]
```
