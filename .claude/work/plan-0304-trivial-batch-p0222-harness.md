...
### T991618101 — `refactor(store):` `make_trusted_key` test helper → delegate to `Signer::trusted_key_entry`
...
### T991618102 — `refactor(tooling):` raw `subprocess.run` git sites → `git_ops` wrapper
...
### T991618103 — `refactor(controller):` extract delete-Job 404-tolerant triad → `job_common::delete_job_tolerant`
...
### T991618104 — `refactor(test):` BuilderPool heredoc YAML at 4 sites — extract `mk_builderpool_yaml` helper OR document threshold
...
### T991618105 — `refactor(controller):` `Bucket` alias home → `builderpool/mod.rs`
...
- T991618101: `grep 'Signer::from_seed.*trusted_key_entry' rio-store/src/signing.rs` → ≥1 hit in `make_trusted_key` body; `grep 'STANDARD.encode(pk' rio-store/src/signing.rs` → ≤1 hit (only `trusted_key_entry` itself, not the test helper)
- T991618102: `grep -c 'subprocess.run.*git' .claude/lib/onibus/merge.py` → significantly reduced (3 rev-parse sites gone); `grep 'git_try.*rev-parse' .claude/lib/onibus/merge.py` → ≥3 hits; `grep 'subprocess.run.*git.*diff' .claude/lib/onibus/cli.py` → 0 hits near `:468`
- T991618103: `grep 'DeleteOutcome\|delete_job_tolerant' rio-controller/src/reconcilers/builderpool/job_common.rs` → ≥2 hits; `grep -c 'ae.code == 404' rio-controller/src/reconcilers/builderpool/manifest.rs` → 0 hits (both triad sites delegate)
- T991618104: either `grep 'mk_builderpool_yaml' nix/tests/scenarios/lifecycle.nix` → ≥3 hits (Route A), OR `grep '5th appears\|threshold chosen' nix/tests/scenarios/lifecycle.nix` → ≥1 hit near `:2076` (Route B — prefer)
- T991618105: `grep 'type Bucket' rio-controller/src/reconcilers/builderpool/mod.rs` → 1 hit; `grep 'type Bucket' rio-controller/src/reconcilers/builderpool/job_common.rs` → 0 hits; `grep 'job_common::Bucket' rio-controller/src/` -r → 0 hits; `grep 'pub(super) use super::job_common::Bucket' rio-controller/src/reconcilers/builderpool/manifest.rs` → 0 hits (re-export dropped)
...
[
  ...
  {"path": "rio-store/src/signing.rs", "action": "MODIFY", "note": "T991618101: make_trusted_key :761-767 body → Signer::from_seed().trusted_key_entry(). discovered_from=478"},
  {"path": ".claude/lib/onibus/merge.py", "action": "MODIFY", "note": "T991618102: :83,:121,:170 raw subprocess.run rev-parse → git_try. discovered_from=consolidator. NOTE: P991618101 edits :528-549 same file — non-overlapping"},
  {"path": ".claude/lib/onibus/cli.py", "action": "MODIFY", "note": "T991618102: :468-472 subprocess.run git-diff check=True → git_ops.git(). Load-bearing exit-128 hazard. discovered_from=consolidator"},
  {"path": "rio-controller/src/reconcilers/builderpool/job_common.rs", "action": "MODIFY", "note": "T991618103: +DeleteOutcome enum +delete_job_tolerant. T991618105: delete Bucket alias :25-35. discovered_from=consolidator+513"},
  {"path": "rio-controller/src/reconcilers/builderpool/manifest.rs", "action": "MODIFY", "note": "T991618103: :338-360,:411-427 404-triad → delete_job_tolerant. T991618105: delete :82-86 Bucket re-export, import super::Bucket. Coordinates with P0516 sweep reorder (extract BEFORE P0516). discovered_from=consolidator+513"},
  {"path": "nix/tests/scenarios/lifecycle.nix", "action": "MODIFY", "note": "T991618104: :2076-2078 comment — document 4-site threshold (Route B prefer). discovered_from=512"},
  {"path": "rio-controller/src/reconcilers/builderpool/mod.rs", "action": "MODIFY", "note": "T991618105: +Bucket type alias near mod declarations. discovered_from=513"},
  {"path": "rio-controller/src/reconcilers/mod.rs", "action": "MODIFY", "note": "T991618105: :35 builderpool::job_common::Bucket → builderpool::Bucket. discovered_from=513"},
  {"path": "rio-controller/src/reconcilers/builderpool/tests/manifest_tests.rs", "action": "MODIFY", "note": "T991618105: :25 import Bucket from builderpool not manifest. discovered_from=513"}
]
```
