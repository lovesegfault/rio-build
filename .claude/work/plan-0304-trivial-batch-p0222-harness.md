...
### T991199101 — `fix(tooling):` coverage-halt heuristic over-counts drv-cascade from ONE scenario
...
### T991199102 — `fix(tooling):` `_INFRA_ERROR_RE` — add exit-code-143 (SIGTERM) alongside exit-137 (SIGKILL)
...
### T991199103 — `fix(tooling):` `AgentRow.status` Literal rejects stale values — log+skip instead of crash
...
### T991199104 — `fix(tooling):` `-c commit.gpgsign=false` at `merge.py` amend+commit sites
...
### T991199105 — `docs(store):` `sweep()` doc-comment — name the distinctness precondition
...
### T991199106 — `docs(plan):` P0513 scope-update — P0511 did NOT extract `is_failed_job`; now dedup THREE sites not one
...
- T991199101: `grep 'rio-cov' .claude/lib/onibus/merge.py` → 0 hits inside `_COV_SCENARIO_FAIL_RE` body (alternation narrowed to `vm-test-run-` only); `pytest .claude/lib/test_scripts.py -k cov_scenario` → green if a test exists, or add one asserting `coverage_full_red` returns count=1 for a log with one vm-test-run + two rio-cov cascades
- T991199102: `grep 'exit code (?:137|143)\|exit code 143' .claude/lib/onibus/build.py` → ≥1 hit; grep `merge-151.log` / `merge-161.log` with the updated regex (if corpus retained) → match
- T991199103: either `grep 'cancelled\|partial' .claude/lib/onibus/models.py` → ≥2 hits in `AgentStatus` Literal (route-a), OR `grep 'ValidationError' .claude/lib/onibus/jsonl.py` → ≥1 hit + `grep 'strict' .claude/lib/onibus/jsonl.py` → ≥1 hit (route-b); `pytest .claude/lib/test_scripts.py -k 'read_jsonl.*stale\|read_jsonl.*skip'` → ≥1 passed
- T991199104: `grep -c 'commit.gpgsign=false' .claude/lib/onibus/merge.py` → ≥2 hits (both `:266` amend + `:1038` commit); `grep 'Resolved by the user disabling' .claude/lib/onibus/merge.py` → 0 hits (stale resolution note replaced)
- T991199105: `grep 'PRECONDITION.*distinct\|distinct hashes' rio-store/src/gc/sweep.rs` → ≥1 hit in `sweep()` doc-comment before `:107`
- T991199106: `grep 'inlined at :237.*:839\|did NOT create is_failed_job' .claude/work/plan-0513-job-common-extraction.md` → ≥1 hit; `grep 'manifest_tests.rs:1253\|test-copies-prod' .claude/work/plan-0513-job-common-extraction.md` → ≥1 hit (third-site test-import coverage noted)
...
[
  ...
  {"path": ".claude/lib/onibus/merge.py", "action": "MODIFY", "note": "T991199101: _COV_SCENARIO_FAIL_RE :528-532 drop rio-cov alternation (cascade products over-count); T991199104: -c commit.gpgsign=false at :266 amend + :1038 commit. discovered_from=coordinator+514"},
  {"path": ".claude/lib/onibus/build.py", "action": "MODIFY", "note": "T991199102: _INFRA_ERROR_RE add exit-143 (SIGTERM, 128+15) alongside exit-137 at :189+. Corpus: merge-151.log, merge-161.log. discovered_from=coordinator"},
  {"path": ".claude/lib/onibus/models.py", "action": "MODIFY", "note": "T991199103 route-a: extend AgentStatus Literal :27 with cancelled+partial. discovered_from=coordinator"},
  {"path": ".claude/lib/onibus/jsonl.py", "action": "MODIFY", "note": "T991199103 route-b (preferred): read_jsonl :36-43 try/except ValidationError + log+skip, strict kwarg opt-out. discovered_from=coordinator"},
  {"path": "rio-store/src/gc/sweep.rs", "action": "MODIFY", "note": "T991199105: sweep() doc-comment add PRECONDITION distinct-hashes line before :107. Pure doc. discovered_from=449"},
  {"path": ".claude/work/plan-0513-job-common-extraction.md", "action": "MODIFY", "note": "T991199106: P0513 scope-update — P0511 did NOT extract is_failed_job, inlined at manifest.rs:237+:839; +manifest_tests.rs:1253 test-copies-prod site. Segments table +row, T2 +2 sites, deps note rewrite. discovered_from=511"}
]
```
