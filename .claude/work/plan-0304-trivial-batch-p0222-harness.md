...
- [P0451](plan-0451-crate-crd-proto-rename.md) merged (rio-worker→rio-builder crate rename; T961851701-T961851705 sweep the leftovers)
...
### T961851701 — `fix(builder):` root span + bootstrap component name "worker" → "builder"
...
### T961851702 — `refactor:` worker→executor identifier sweep — fn names, local vars, tracing fields
...
### T961851703 — `refactor(controller):` worker→builder k8s resource names — label, container, nix option
...
### T961851704 — `refactor:` worker→builder/executor user-facing strings — logs, errors, help text, metric descriptions
...
### T961851705 — `refactor:` worker→executor file renames + test fixtures + CSS
...
### T961851706 — `chore:` add license field to workspace-hack + xtask Cargo.toml
...
### T961851707 — `chore(crds):` regen CRD yaml to fix format drift
...
### T961851708 — `fix(harness):` merge-count.txt bump race post-P0451
...
### T963241201 — `refactor(scheduler):` executor_service.rs — warn on unknown ExecutorKind enum value
...
### T963241202 — `refactor(dashboard):` Executors.svelte+test — drop semantically-wrong kind-gate tracey annotations
...
- T961851701: `grep 'component = "worker"' rio-builder/src/main.rs` → 0 hits; `grep '"builder"' rio-builder/src/main.rs` → ≥2 hits (bootstrap arg + span)
- T961851702: `grep 'resolve_worker_identity\|append_failed_worker' rio-*/src/` → 0 hits
- T961851703: `grep 'component: worker\|name: worker' rio-controller/src/` → 0 hits (or moved to common/sts.rs post-P0453)
- T961851704: `git grep -c '\bworker\b' -- rio-builder/src/ rio-scheduler/src/lib.rs | grep -v test` substantially reduced vs baseline
- T961851705: `ls rio-scheduler/src/admin/tests/executors_tests.rs rio-dashboard/src/pages/Executors.svelte` — both exist; old paths gone
- T961851706: `cargo deny check licenses` → green (workspace-hack + xtask licensed)
- T961851707: `nix develop -c cargo xtask regen crds && git diff --exit-code` — regen idempotent
- T961851708: `cat .claude/state/merge-count.txt` ≥ 2; merger script bump precedes amend
- T963241201: `grep 'tracing::warn.*unknown ExecutorKind\|version skew' rio-scheduler/src/grpc/executor_service.rs` → ≥1 hit
- T963241202: `grep 'builder.executor.kind-gate' rio-dashboard/src/pages/Executors.svelte rio-dashboard/src/pages/__tests__/Executors.test.ts` → 0 hits
...
[
  ...
  {"path": "rio-builder/src/main.rs", "action": "MODIFY", "note": "T961851701: :56,:69 component worker→builder. T961851702: :805 resolve_worker_identity rename. T961851704: :83,:817 help/error text. discovered_from=451"},
  {"path": "rio-scheduler/src/db/derivations.rs", "action": "MODIFY", "note": "T961851702: :92 append_failed_worker→append_failed_executor. discovered_from=451"},
  {"path": "rio-scheduler/src/actor/build.rs", "action": "MODIFY", "note": "T961851702: :99,:118 local var worker→executor. discovered_from=451"},
  {"path": "rio-scheduler/src/actor/dispatch.rs", "action": "MODIFY", "note": "T961851702: :308,:464,:477 local var. T961851704: :472,:551 log text. discovered_from=451"},
  {"path": "rio-controller/src/reconcilers/builderpool/mod.rs", "action": "MODIFY", "note": "T961851702: :613,:620 tracing field worker→executor. discovered_from=451"},
  {"path": "rio-controller/src/reconcilers/builderpool/builders.rs", "action": "MODIFY", "note": "T961851703: :61 k8s label, :625 container name worker→builder. Check P0453 — may have moved to common/sts.rs. discovered_from=451"},
  {"path": "nix/modules/builder.nix", "action": "MODIFY", "note": "T961851703: :139 option workerId→executorId. discovered_from=451"},
  {"path": "rio-builder/src/executor/mod.rs", "action": "MODIFY", "note": "T961851704: :314,:321 leaked-mount msg. T961851705: :1517 fixture literal. discovered_from=451"},
  {"path": "rio-builder/src/executor/inputs.rs", "action": "MODIFY", "note": "T961851704: :140 worker-side text. discovered_from=451"},
  {"path": "rio-builder/src/cgroup.rs", "action": "MODIFY", "note": "T961851704: :481,:497,:545,:547 log text. discovered_from=451"},
  {"path": "rio-builder/src/runtime.rs", "action": "MODIFY", "note": "T961851704: :692 task-panicked msg. T961851705: 6 fixture literals. discovered_from=451"},
  {"path": "rio-scheduler/src/lib.rs", "action": "MODIFY", "note": "T961851704: :59,:78,:115,:144,:149 metric descriptions. discovered_from=451"},
  {"path": "rio-scheduler/src/admin/tests/workers_tests.rs", "action": "RENAME", "note": "T961851705: → executors_tests.rs. discovered_from=451"},
  {"path": "rio-dashboard/src/pages/Workers.svelte", "action": "RENAME", "note": "T961851705: → Executors.svelte + route refs. discovered_from=451"},
  {"path": "rio-dashboard/src/components/DrvNode.svelte", "action": "MODIFY", "note": "T961851705: :71 CSS class, :77 a11y handler. discovered_from=451"},
  {"path": "rio-builder/src/executor/daemon/spawn.rs", "action": "MODIFY", "note": "T961851705: :381 fixture literal. discovered_from=451"},
  {"path": "rio-builder/src/executor/daemon/stderr_loop.rs", "action": "MODIFY", "note": "T961851705: 6 fixture literals. discovered_from=451"},
  {"path": "rio-builder/src/log_stream.rs", "action": "MODIFY", "note": "T961851705: :210,:241 fixture literals. discovered_from=451"},
  {"path": "workspace-hack/Cargo.toml", "action": "MODIFY", "note": "T961851706: add license field"},
  {"path": "xtask/Cargo.toml", "action": "MODIFY", "note": "T961851706: add license field"},
  {"path": ".claude/state/merge-count.txt", "action": "MODIFY", "note": "T961851708: correct to ≥2"},
  {"path": "rio-scheduler/src/grpc/executor_service.rs", "action": "MODIFY", "note": "T963241201: :377 unwrap_or → unwrap_or_else with tracing::warn on unknown kind. discovered_from=452"},
  {"path": "rio-dashboard/src/pages/Executors.svelte", "action": "MODIFY", "note": "T963241202: :8 drop r[impl builder.executor.kind-gate] (semantically wrong, tracey-invisible). discovered_from=456"},
  {"path": "rio-dashboard/src/pages/__tests__/Executors.test.ts", "action": "MODIFY", "note": "T963241202: :80 drop r[verify builder.executor.kind-gate] (tests display filter, not security gate). discovered_from=456"}
]
```
