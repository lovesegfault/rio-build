...
Also rewrite `plan-<placeholder>-*.md` link targets to the real filenames (the plan docs themselves were renamed by `rename-unassigned`; only the REFERENCES to them are stale). Note-body prose mentions (e.g., `P996394101` / `P998902141-T3`) in deps-fence `note` strings also need rewrite — grep hits include raw integers in JSON soft_deps AND P-prefixed prose. STRUCTURAL: fires every docs-merge that batch-appends cross-refs until T30 lands. discovered_from=consolidator-mc205.
...
### T975348001 — `fix(dashboard):` eslint gen/*_pb glob — widen for subdirectory depth
...
### T975348002 — `docs(scheduler):` stale db.rs string cites — fn-name anchors post-P0411
...
### T975348003 — `docs(components):` scheduler.md db.rs → db/ post-P0411
...
### T975348004 — `refactor(scheduler):` connect_worker_no_ack — promote to helpers.rs, delegate
...
### T975348005 — `refactor(harness):` merge.py raw subprocess.run → git()/git_try()
...
**NOTE:** [P0417](plan-0417-dag-flip-already-done-double-bump.md) + [P0418](plan-0418-onibus-rename-canonicalization-hardening.md) + [P997534801](plan-997534801-count-bump-toctou-write-order.md) + [P997534802](plan-997534802-merge-py-rename-cluster-extract.md) all touch merge.py — fold this into whichever lands last, or land standalone after they drain. discovered_from=consolidator (mc235 + rev-p417 row 15 semantic-divergence note combined).

### T975348006 — `fix(harness):` merge.py scan break — last-match not first
...
### T975348007 — `fix(controller):` autoscaler _secs — ensure >0 on scale_up/scale_down/min_interval
...
Lower-severity than poll_secs (degraded not panic). rev-p415 flagged; seed(d) delegated here. discovered_from=416. Soft-conflict [P997534806](plan-997534806-ensure-required-helper-ten-site.md) (same validate_config body, different ensures — additive).

### T975348008 — `refactor(scheduler):` db/mod.rs row-struct domain-move (LOW-priority)
...
### T975348010 — `docs(notes):` bughunter-log mc235-mc238 — verdict archive entries
...
T2 merge.py split = [P997534802](../work/plan-997534802-merge-py-rename-cluster-extract.md).
...
T5 db/mod.rs residual = T975348008 above (LOW-priority row-struct move).
...
[P997534805](../work/plan-997534805-sizeclassconfig-cpu-limit-validation.md).
...
already-done double-bump; P997534801 closes the count_bump write-order
TOCTOU. Three-plan chain (P0414→P0417→P997534801) covers the full
...
### T975348011 — `docs(scheduler):` db/recovery.rs — cohesion NOTE for admin_reads candidates
...
LOW-priority rename/extract deferred to future plan; this task just documents the known-misplacement. discovered_from=411. Orthogonal to T975348008 (row-struct placement — different cohesion angle).
...
- T172: `grep 'P0353\|P0362\|P0374\|P0375' .claude/work/plan-0304-*.md | grep -v 'T172\|sed map\|T30'` → ≥4 hits (self-referential 996394101/997105502/998902141/998902142 rewritten in soft_deps + note body, excluding T172's own description of the mapping)
...
- T975348001: `grep '\*\*/gen/\*\*/\*_pb' rio-dashboard/eslint.config.js` → ≥2 hits (both patterns widened); `grep "'\*\*/gen/\*_pb'" rio-dashboard/eslint.config.js` → 0 hits at :39 (single-star form gone)
- T975348002: `grep 'db\.rs:537\|db\.rs will decode' rio-scheduler/src/actor/tests/build.rs rio-scheduler/src/actor/completion.rs` → 0 hits; `grep 'TERMINAL_STATUS_SQL const\|db/recovery.rs' rio-scheduler/src/actor/tests/build.rs rio-scheduler/src/actor/completion.rs | grep -c '//'` → ≥2 (grep-stable cites)
- T975348003: `grep 'rio-scheduler/src/db\.rs' docs/src/components/scheduler.md` → 0 hits; `grep 'rio-scheduler/src/db/' docs/src/components/scheduler.md` → ≥1 hit (:664 updated)
- T975348004: `grep 'pub(crate) async fn connect_worker_no_ack' rio-scheduler/src/actor/tests/helpers.rs` → 1 hit (promoted); `grep 'async fn connect_worker_no_ack' rio-scheduler/src/actor/tests/worker.rs` → 0 hits (moved out); `cargo nextest run -p rio-scheduler 'actor::tests::worker'` → all pass (delegation preserves behavior)
- T975348005: `grep 'subprocess\.run.*rev-parse' .claude/lib/onibus/merge.py | grep -v '^\s*#'` → ≤1 hit (the :551 merge-base --is-ancestor stays raw; 3 rev-parse folded to git() OR added git_try variant); `nix develop -c pytest .claude/lib/test_scripts.py -k 'dag_flip or count_bump'` → all pass
- T975348006: `grep -A3 'if row.plan == plan_num' .claude/lib/onibus/merge.py | grep -c 'break'` → 0 (loop runs through, last match wins) OR `grep 'reversed(.*read_jsonl' .claude/lib/onibus/merge.py` → ≥1 hit (reversed iter + break-on-first)
- T975348007: `grep 'scale_up_window_secs must be\|scale_down_window_secs must be\|min_interval_secs must be' rio-controller/src/main.rs` → ≥3 hits (all three ensures added); `cargo nextest run -p rio-controller validate_config` → all pass
- T975348008: `wc -l rio-scheduler/src/db/mod.rs` → ≤180 (row structs moved to domain files); `python3 -c "import subprocess; out=subprocess.check_output(['cargo','check','-p','rio-scheduler'], stderr=subprocess.STDOUT, text=True); assert 'error' not in out.lower()"` — compiles (pub-use re-exports keep external callers working)
- T975348009: `grep 'cargo-mutants 26\|MUTATION_MARKER_COMMENT' flake.nix` → ≥1 hit near check-mutants-marker hook (version-anchor comment)
- T975348010: `grep 'mc=235\|mc235\|mc=236\|mc236\|mc=238\|mc238' .claude/notes/bughunter-log.md` → ≥1 hit (archive entries added)
- T975348011: `grep 'NOTE(fault-line)\|db/admin_reads' rio-scheduler/src/db/recovery.rs` → ≥1 hit (cohesion comment at load_build_graph/read_event_log)
...
[
  ...
  {"path": ".claude/work/plan-0287-trace-linkage-submitbuild-metadata.md", "action": "MODIFY", "note": "T172: :234 soft_deps 996394104→356 (stale placeholder ref — P0356 grpc-mod-split). discovered_from=consolidator-mc205"},
  {"path": ".claude/work/plan-0295-doc-rot-batch-sweep.md", "action": "MODIFY", "note": "T172: :500 P997443701→P0363 cross-link; :700 plan-998902142→plan-0375 table cell; soft_deps fence 996394104→356. discovered_from=consolidator-mc205"},
  {"path": ".claude/work/plan-0311-test-gap-batch-cli-recovery-dash.md", "action": "MODIFY", "note": "T172: :1175,:1196,:1198,:1453,:2128,:2196,:2248 + soft_deps fence — 997443701→0363, 998902141→0374, 997105501→0361 placeholder rewrites. discovered_from=consolidator-mc205"},
  {"path": ".claude/work/plan-0304-trivial-batch-p0222-harness.md", "action": "MODIFY", "note": "T172: THIS DOC soft_deps fence + note body — 996394101→0353, 997105502→0362, 998902141→0374, 998902142→0375 (QA-caught self-referential). discovered_from=consolidator-mc205"},
  ...
  {"path": "rio-dashboard/eslint.config.js", "action": "MODIFY", "note": "T975348001: :39 **/gen/*_pb → **/gen/**/*_pb (both patterns). discovered_from=407. Low-traffic"},
  {"path": "rio-scheduler/src/actor/tests/build.rs", "action": "MODIFY", "note": "T975348002: :277 stale db.rs cite → db/recovery.rs fn-name anchor. discovered_from=411"},
  {"path": "rio-scheduler/src/actor/completion.rs", "action": "MODIFY", "note": "T975348002: :995 stale db.rs:537 → TERMINAL_STATUS_SQL const cite. discovered_from=411. HOT count=32 — comment-only"},
  {"path": "docs/src/components/scheduler.md", "action": "MODIFY", "note": "T975348003: :664 db.rs → db/ post-P0411. discovered_from=411. count=24 — single-line doc fix"},
  {"path": "rio-scheduler/src/actor/tests/helpers.rs", "action": "MODIFY", "note": "T975348004: +pub(crate) connect_worker_no_ack (moved from worker.rs:819); connect_worker delegates. discovered_from=consolidator. Soft-conflict P997534803 (CaFixture helper adds after :26; T975348004 adds after :86 — non-overlapping)"},
  {"path": "rio-scheduler/src/actor/tests/worker.rs", "action": "MODIFY", "note": "T975348004: delete connect_worker_no_ack :819-845 (moved to helpers.rs). discovered_from=consolidator"},
  {"path": ".claude/lib/onibus/merge.py", "action": "MODIFY", "note": "T975348005: :76,:114,:154 raw subprocess.run→git() OR git_try(). T975348006: already-done scan loop — drop break OR reversed-iter (last-match). discovered_from=consolidator+rev-p417. Soft-conflict P0417(DONE)+P0418+P997534801+P997534802 — all merge.py, non-overlapping hunks"},
  {"path": "rio-controller/src/main.rs", "action": "MODIFY", "note": "T975348007: validate_config +3 ensures (scale_up/scale_down/min_interval _secs > 0). discovered_from=416. HOT count=26 — additive ensures"},
  {"path": "rio-scheduler/src/db/mod.rs", "action": "MODIFY", "note": "T975348008: LOW-PRIORITY row-struct domain-move (9 structs → tenants.rs/builds.rs/assignments.rs/history.rs/batch.rs/recovery.rs/derivations.rs + pub-use re-exports). discovered_from=consolidator"},
  {"path": "rio-scheduler/src/db/tenants.rs", "action": "MODIFY", "note": "T975348008: +TenantRow struct (moved from mod.rs). discovered_from=consolidator"},
  {"path": "rio-scheduler/src/db/builds.rs", "action": "MODIFY", "note": "T975348008: +BuildListRow + LIST_BUILDS_SELECT. discovered_from=consolidator"},
  {"path": "rio-scheduler/src/db/assignments.rs", "action": "MODIFY", "note": "T975348008: +AssignmentStatus enum. discovered_from=consolidator"},
  {"path": "rio-scheduler/src/db/history.rs", "action": "MODIFY", "note": "T975348008: +BuildHistoryRow + EMA_ALPHA const. discovered_from=consolidator"},
  {"path": "rio-scheduler/src/db/batch.rs", "action": "MODIFY", "note": "T975348008: +DerivationRow. discovered_from=consolidator"},
  {"path": "rio-scheduler/src/db/recovery.rs", "action": "MODIFY", "note": "T975348008: +RecoveryBuildRow/RecoveryDerivationRow/GraphNodeRow/GraphEdgeRow (moved). T975348011: NOTE(fault-line) comment at load_build_graph + read_event_log (cohesion — not-really-recovery fns). discovered_from=consolidator+rev-p411"},
  {"path": "rio-scheduler/src/db/derivations.rs", "action": "MODIFY", "note": "T975348008: +PoisonedDerivationRow. discovered_from=consolidator"},
  {"path": "flake.nix", "action": "MODIFY", "note": "T975348009: check-mutants-marker hook — +version-pin comment (cargo-mutants 26.2.0 MUTATION_MARKER_COMMENT anchor). discovered_from=402. HOT count=38 — comment-only adjacent to hook"},
  {"path": ".claude/notes/bughunter-log.md", "action": "MODIFY", "note": "T975348010: +mc235-238 verdict archive (P0412 proc-macro NOT WORTH; consol-mc236 5-target verdict; bughunt-mc238 cpu_limit_cores finding→P997534805; P0414-chain complete; rev-p411 plan-doc-accuracy non-actionable). discovered_from=consolidator+bughunter+coordinator+rev-p411"}
]
```
