...
### T992487501 — `refactor(tooling):` pad `fix_owner: P518` → `P0518` in known-flakes + lifecycle.nix
...
### T992487502 — `fix(tooling):` coverage-full subshell re-import merge.py — P0517 fix had a ~25min delayed-activation window
...
### T992487503 — `docs(controller):` `select_failed_jobs` doc-comment stale — `sweep_cap()` extracted the cap formula
...
### T992487504 — `docs(merger):` note nix remote-build double-dispatch hang — kill+rerun works
...
### T992487505 — `fix(store):` `SignerError::TenantKeyLookup` docstring + message stale — used for cluster-key-history load too
...
[P992487504](plan-992487504-cluster-key-history-load-validation.md) touches `load_prior_cluster` (`:307-311`) — coordinate: if this lands first, P992487504's edited function uses the new variant; if P992487504 lands first, this plan edits the P992487504-expanded function. Both additive, clean either order. discovered_from=521.

### T992487506 — `refactor(nix):` removeAttrs exclusion assert-guard — silent no-op on rename
...
[P992487501](plan-992487501-ci-missing-codecov-matrix-sync.md) touches `flake.nix` `:1267-1293` — DIFFERENT section (`:1209-1224` vs `:1267`), clean rebase. discovered_from=519.

### T992487507 — `refactor(nix):` dead `cpuHints.vm-fod-proxy-k3s` — test deleted per ADR-019
...
**While there:** audit all `cpuHints` keys against the actual test attrset — same silent-fallthrough class as T992487506's removeAttrs issue. An assert-guard is possible:
...
**Minimal fix:** delete `:1179`. **Thorough fix:** delete `:1179` + add the assert. Prefer thorough (matches T992487506's pattern — catch the class, not the instance). discovered_from=519.
...
- T992487501: `grep -c 'P0518' .claude/known-flakes.jsonl` ≥ 1; `grep -c '"fix_owner":"P518"' .claude/known-flakes.jsonl` → 0 (padded); `grep 'P518:' nix/tests/scenarios/lifecycle.nix` → 0 hits; `grep 'P0518:' nix/tests/scenarios/lifecycle.nix` → ≥1 hit at `:2603`
- T992487502: re-import/re-exec present at step-6 coverage-maybe-halt callsite, OR `coverage_maybe_halt` moved to standalone script (option-c), OR `/dag-tick` owns the halt decision (option-d — verify structure)
- T992487503: `grep "Callers compute .cap. as" rio-controller/src/reconcilers/builderpool/manifest.rs` → 0 hits; `grep 'sweep_cap' rio-controller/src/reconcilers/builderpool/manifest.rs` near `:834` → either cross-ref present (route-a) or clause dropped (route-b)
- T992487504: `grep 'double-dispatch\|zombie remote\|kill.*rerun' .claude/skills/merge-impl.md .claude/rules/coverage.md` → ≥1 hit (hang pattern documented)
- T992487505: either `grep 'ClusterKeyHistory' rio-store/src/signing.rs` → ≥2 hits (new variant, route-b) OR `grep 'never touches the DB' rio-store/src/signing.rs` → 0 hits near `:136` (docstring corrected, route-c); `grep 'tenant key lookup' rio-store/src/signing.rs` near `:310` usage → 0 hits in cluster context (message no longer contradicts)
- T992487506: `grep 'assertMsg.*hasAttr.*excl\|exclusion.*not found' flake.nix` → ≥1 hit near `:1224`; mutation: rename `vm-lifecycle-prod-parity-k3s` in `nix/tests/default.nix` → `nix eval .#vmTestsCov` FAILS with the assertMsg
- T992487507: `grep 'vm-fod-proxy-k3s' flake.nix` → 0 hits in cpuHints (`:1179` deleted); optional thorough-fix: `grep 'deadHints\|nonexistent tests' flake.nix` → ≥1 hit (assert present)
...
[
  ...
  {"path": ".claude/known-flakes.jsonl", "action": "MODIFY", "note": "T992487501: :14 fix_owner P518 → P0518. discovered_from=518"},
  {"path": "nix/tests/scenarios/lifecycle.nix", "action": "MODIFY", "note": "T992487501: :2603 comment P518 → P0518. HOT — T529/T530 touch diff sections. discovered_from=518"},
  {"path": ".claude/lib/onibus/models.py", "action": "MODIFY", "note": "T992487501 (optional): tighten _OWNER_RE :177 to ^P0\\d{3,}. T1 also touches :322 (diff section). discovered_from=518"},
  {"path": ".claude/lib/onibus/build.py", "action": "MODIFY", "note": "T992487502: step-6 coverage subshell re-import/re-exec before coverage_maybe_halt, OR move to standalone script. P992487503 touches :233/:240 (diff section — excusable logic). discovered_from=517"},
  {"path": "rio-controller/src/reconcilers/builderpool/manifest.rs", "action": "MODIFY", "note": "T992487503: :834-835 doc-comment stale cap formula → sweep_cap() cross-ref. HOT — T531 touches :680, P992487502 touches :360-386, P0520/P0522 touch other sections. discovered_from=520"},
  {"path": ".claude/skills/merge-impl.md", "action": "MODIFY", "note": "T992487504: note nix remote-build double-dispatch hang pattern + kill+rerun workaround. May go to .claude/rules/coverage.md instead. discovered_from=519"},
  {"path": "rio-store/src/signing.rs", "action": "MODIFY", "note": "T992487505: :131-139 SignerError::TenantKeyLookup docstring stale post-P0521. Route-b adds ClusterKeyHistory variant, :310 swaps. P992487504 touches :55/:61-66/:307-311 (diff section + some overlap at :310 — coordinate). discovered_from=521"},
  {"path": "flake.nix", "action": "MODIFY", "note": "T992487506: :1209-1224 removeAttrs → let-assert-removeAttrs. T992487507: :1179 delete dead cpuHints entry + optional deadHints assert near :1198. HOT (count=47) — P992487501 touches :1267-1293 (diff section). discovered_from=519"}
]
```
