...
The constraint: under a namespace `ResourceQuota` on `count/jobs.batch` (GKE Autopilot default, common in hardened clusters), a crash-loop fills the quota with Failed Jobs. Spawn-before-sweep → `jobs_api.create` 403s on quota exhaustion → `return Err` → sweep never runs → deadlock. Sweep-first clears dead weight; spawn then has room.

Amend `:157` (append sentence, **run `tracey bump`** → `+3` — this is a behavior-constraint addition, not just phrasing):

```markdown
r[ctrl.pool.manifest-failed-sweep+3]
The manifest reconciler MUST delete Failed Jobs alongside idle-surplus deletes, and MUST run the Failed-Job sweep BEFORE the spawn pass. Under a namespace `ResourceQuota` on `count/jobs.batch`, spawn-before-sweep deadlocks: crash-loop fills quota with Failed Jobs → spawn 403s → bail → sweep never runs → can't clear quota to make room. With `backoff_limit=0` ... [rest unchanged]
```
...
| Line | Style | Example |
|---|---|---|
| `:147` | bold-inline | `**Superseded by P0NNN** — ...` |
| `:2461` | bold-caps-colon | `**SUPERSEDED:** P0NNN ...` |
| `:2523`, `:2673` | blockquote | `> **Superseded** by ...` |
| `:2632` | bold-caps-bracket | `**[SUPERSEDED by P0NNN]** ...` |
| `:4090` | header-prefix | `### T518 — [SUPERSEDED by P0517] ...` (added by P0517) |

Header-prefix is best — visible in `grep '^###'`, visible in TOC, visible in `onibus`'s T-enumeration. But tooling that auto-skips superseded T-items would need 5 regexes today.

**Unify:**
- T-item-level supersession: header-prefix `### TNNN — [SUPERSEDED by P0NNN] ...` (keep `:4090` style)
- Inline body notes (e.g., "Route A was superseded mid-plan"): blockquote `> **Superseded** by ...` (keep `:2523` style)
- Sweep the other 3 (`:147`, `:2461`, `:2632`) to whichever tier applies

Also check [P0311](plan-0311-test-gap-batch-cli-recovery-dash.md) and [P0295](plan-0295-doc-rot-batch-sweep.md) for the same drift — batch docs accrete supersession markers over time. discovered_from=517.

### T992487508 — `docs(store):` step 5 says "delete cluster_key_history row" — design is retire-not-delete

[`store.md:216`](../../docs/src/components/store.md) Key Rotation step 5: "remove the old key from `trusted-public-keys` and delete its `cluster_key_history` row". Contradicts the schema design:

- [`cluster_key_history.rs:22-23`](../../rio-store/src/metadata/cluster_key_history.rs): "operator ends the grace period by setting `retired_at`, not by deleting the row"
- [`migrations.rs:296`](../../rio-store/src/migrations.rs) `M_027` doc-const: "row retained for audit only"

Operator following the spec loses the audit trail. **Functionally equivalent for the gate** (both `WHERE retired_at IS NULL` and "row doesn't exist" make the key untrusted) — but it defeats the `retired_at` column's purpose.

**Fix:** `s/delete its cluster_key_history row/set retired_at on its cluster_key_history row/`.

**While there:** step 5 also says "After a grace period (default: 30 days)" — verify the 30-day default is real (config, const, or aspirational). If there's no code enforcing 30 days, either drop "(default: 30 days)" or make it "(e.g., 30 days)".

[P992487505](plan-992487505-admin-cli-key-tables.md) adds a `r[store.key.admin-cli]` marker after `:219` AND a `rio-cli keys cluster-retire` subcommand that does `UPDATE ... SET retired_at = now()` (not DELETE). This T-item and P992487505's spec-addition are ADJACENT (`:216` vs `:221+`) — coordinate at dispatch; prefer this T lands first (1-word fix), then P992487505's marker insert is a clean append. discovered_from=521.
...
- T992487508: `grep 'delete its cluster_key_history' docs/src/components/store.md` → 0 hits; `grep 'set retired_at\|set .retired_at.' docs/src/components/store.md` → ≥1 hit at `:216`; no `tracey bump` on `r[store.key.rotation-cluster-history]` (step-5 is operator procedure, not the marker text at `:218-219`)
...
[
  ...
  {"path": "docs/src/components/store.md", "action": "MODIFY", "note": "T992487508: :216 s/delete its cluster_key_history row/set retired_at on .../. T496 touches :230 (diff section). P992487505 adds r[store.key.admin-cli] marker after :219 (ADJACENT — coordinate, prefer T992487508 lands first). discovered_from=521"}
]
```
