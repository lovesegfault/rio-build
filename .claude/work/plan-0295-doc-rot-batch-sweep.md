...
### T963241201 — `docs(builder):` lib.rs:47 "Worker Metrics" → "Builder Metrics" section name
...
### T963241202 — `docs(adr):` ADR-005 body — 3× stale rio-worker/ crate paths
...
### T963241203 — `docs(adr):` ADR-012 body — rio-worker/src/cgroup.rs → rio-builder/src/cgroup.rs

[`docs/src/decisions/012-privileged-builder-pods.md:42`](../../docs/src/decisions/012-privileged-builder-pods.md): references `rio-worker/src/cgroup.rs`. Same P0455 rename-miss as T963241202. One-site fix. discovered_from=455.

### T963241204 — `docs(remediations):` 3× half-renamed [worker.md] link text → [builder.md]
...
### T963241205 — `docs(obs):` observability.md:166 — add fod_queue_depth + fetcher_utilization table rows
...
- T963241201: `grep 'Worker Metrics' rio-builder/src/lib.rs` → 0 hits
- T963241202: `grep 'rio-worker/' docs/src/decisions/005-builder-store-model.md` → 0 hits
- T963241203: `grep 'rio-worker/' docs/src/decisions/012-privileged-builder-pods.md` → 0 hits
- T963241204: `grep '\[worker.md\]' docs/src/remediations/phase4a/{02,09,15}-*.md` → 0 hits
- T963241205: `grep 'rio_scheduler_fod_queue_depth\|rio_scheduler_fetcher_utilization' docs/src/observability.md | grep '^|'` → ≥2 hits (table rows added)
- T963241205: `grep 'rows added when the emitters land' docs/src/observability.md` → 0 hits (blockquote rewritten past-tense)
...
[
  ...
  {"path": "rio-builder/src/lib.rs", "action": "MODIFY", "note": "T963241201: :47 Worker Metrics → Builder Metrics section name. discovered_from=455"},
  {"path": "docs/src/decisions/005-builder-store-model.md", "action": "MODIFY", "note": "T963241202: :10,16 rio-worker/ → rio-builder/ (3 sites). discovered_from=455"},
  {"path": "docs/src/decisions/012-privileged-builder-pods.md", "action": "MODIFY", "note": "T963241203: :42 rio-worker/src/cgroup.rs → rio-builder/. discovered_from=455"},
  {"path": "docs/src/remediations/phase4a/09-build-timeout-cgroup-orphan.md", "action": "MODIFY", "note": "T963241204: :792 [worker.md] link text → [builder.md]. discovered_from=455"},
  {"path": "docs/src/remediations/phase4a/15-shutdown-signal-cluster.md", "action": "MODIFY", "note": "T963241204: :527 [worker.md] link text → [builder.md]. discovered_from=455"},
  {"path": "docs/src/remediations/phase4a/02-empty-references-nar-scanner.md", "action": "MODIFY", "note": "T963241204: :1238 [worker.md] link text → [builder.md]. discovered_from=455"},
  {"path": "docs/src/observability.md", "action": "MODIFY", "note": "T963241205: :82 Scheduler Metrics table +2 rows (fod_queue_depth, fetcher_utilization); :166 blockquote → past tense. discovered_from=452"}
]
```
