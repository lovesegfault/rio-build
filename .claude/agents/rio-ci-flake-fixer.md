...
|---|---|---|---|
| **k3s airgap import timing** | VM test flakes on agent readiness — airgap imports serially/alphabetically before kubelet | phase3a k3s tests | Gate on server-node-exists (validated 3/3 — agent-Ready 106.70→1.9s, 56×). Budget for tail, not typical (builder variance 5×). |
...
| **Parallel test order-dependence** | Passes solo, fails under `nextest` parallelism | (shared fs state, global mutable) | `#[serial]` attribute; or actually fix the shared state |
...
- Example: `test(flake): vm-phase3a agent-ready — gate on server-node-exists not timeout`
