...
|---|---|---|---|
| **`#[ignore]` added** | `git diff main..<branch> \| grep '^\+.*#\[ignore\]'` | Never for a flake fix — this is deleting the test | Always FAIL this |
...
```bash
git diff main..<branch> --stat    # shape
git diff main..<branch>           # detail
```
...
```bash
git diff main..<branch> | grep -E '^\+.*(#\[ignore\]|#\[retry|#\[serial\]|\.contains\()'
git diff main..<branch> | grep -E '^\+.*assert' && git diff main..<branch> | grep -E '^\-.*assert'
```
