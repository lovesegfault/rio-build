...
```bash
python3 .claude/skills/merge-impl/rename_unassigned.py $ARGUMENTS
```
...
```bash
python3 .claude/skills/merge-impl/atomicity_check.py $ARGUMENTS
```
...
The merger emits a fenced ```json `MergerReport` block (`state.MergerReport` — `python3 .claude/lib/state.py schema MergerReport`). Parse it; match on `report.status` and `report.abort_reason`.
