...
| **KWOK Stage machinery silent after post-CRD restart** | `vm-sla-sizing-kwok` `Registered=True` wait times out at 60s; on-failure dump shows `nodeclaims items: []` (health reap deleted the unregistered claim) and the restarted kwok-controller log has only its 4 startup lines (no Stage activity); coverage-mode run of the same commit passes | Re-run first. If it recurs: gate on positive evidence the Stage machinery is live after the restart — a throwaway canary NodeClaim must reach `Launched=True` within ~15s, else re-restart kwok-controller and retry. Do NOT widen the 60s wait (the dropped resourceRef is permanent). |
