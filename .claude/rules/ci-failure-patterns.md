...
| **procsub tee capture raced by next command** | stop-parity/`stops=set()` (or truncated event capture) while the build exited 0; capture cat finishes while tee still echoes | Synchronous redirect + tail-on-failure (landed in `substitute.nix` substitute-progress-e2e); never read a procsub-captured file in the next command — bash doesn't wait for `2> >(tee …)`. |
