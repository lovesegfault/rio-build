...
scope: with scope; ''
  ...
  #   sent_dcache 4 KiB — sentinel input read AFTER the five traversals;
  #                      its node-cache appearance is the host-side
  #                      "script got past the traversals" gate.
  ...
  with subtest("seed-core: generate + push core inputs as vm-castore"):
      ...
      p_big_a, p_head4k, p_dedup_a, p_dedup_b, p_meta_tree, p_sent_dcache = add_and_push(
