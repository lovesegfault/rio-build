...
pkgs.testers.runNixOSTest {
  ...
  testScript = ''
    ...
    with subtest("reset_peer on worker_store: prefetch RST -> redispatch or upload retry succeeds"):
        ...
        # The scheduler re-dispatches (generation stays 1, same drv
        # re-assigned) so the overall retry path is scheduler→worker
        # redispatch, not the upload.rs internal loop. The
