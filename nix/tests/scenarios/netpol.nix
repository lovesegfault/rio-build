...
# default-builders-0 gives the right PID.
#
...
pkgs.testers.runNixOSTest {
  ...
  testScript = ''
    ...
    # schedule default-builders-0 to either k3s-server or k3s-agent.
    ...
    cid = worker_vm.succeed(
        "k3s crictl ps -q "
        "--label io.kubernetes.pod.name=default-builders-0 | head -1"
    ).strip()
    assert cid, "no running container found for default-builders-0"
