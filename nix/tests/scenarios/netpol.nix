...
# rio-builders-0 gives the right PID.
#
...
pkgs.testers.runNixOSTest {
  ...
  testScript = ''
    ...
    # schedule rio-builders-0 to either k3s-server or k3s-agent.
    ...
    worker_node = kubectl(
        "get pod rio-builders-0 -o jsonpath='{.spec.nodeName}'",
        ns="${nsBuilders}",
    ).strip()
    worker_vm = k3s_agent if worker_node == "k3s-agent" else k3s_server
    print(f"netpol: rio-builders-0 is on {worker_node}")
    ...
    cid = worker_vm.succeed(
        "k3s crictl ps -q "
        "--label io.kubernetes.pod.name=rio-builders-0 | head -1"
    ).strip()
    assert cid, "no running container found for rio-builders-0"
