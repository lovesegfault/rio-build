# scheduling subtest fragment — composed by scenarios/scheduling.nix mkTest.
scope: with scope; ''
  # ══════════════════════════════════════════════════════════════════
  # cgroup — memory.peak + cpu.stat land in build_samples
  # ══════════════════════════════════════════════════════════════════
  # Chain: BuildCgroup.memory_peak() → ExecutionResult →
  # CompletionReport → scheduler handle_completion →
  # db.write_build_sample (raw row, ADR-023 SLA fit feed).
  #
  # cgroup memory.peak captures the WHOLE TREE (daemon + builder +
  # sleep subprocess). Per-PID VmHWM would only measure nix-daemon's
  # own ~10MB RSS — the builder is a fork()ed child whose footprint
  # never appears there. Even a trivial busybox+sleep build has
  # ~3-10MB tree RSS.
  with subtest("cgroup: memory.peak + cpu.stat → build_samples"):
      build("${cgroupDrv}")

      # psql -qtA: NULL → empty line. grep matches ≥7 digits = ≥1MB.
      # wait_until_succeeds: window between client-sees-built and the
      # scheduler-actor DB commit. The happy path commits in <100ms,
      # but under a contended builder (full check-matrix runs) the
      # commit has repeatedly taken >10s — the wait returns as soon as
      # the row appears, so the wide ceiling only matters on the slow
      # tail.
      ${gatewayHost}.wait_until_succeeds(
          "sudo -u postgres psql rio -qtAc "
          "\"SELECT peak_memory_bytes FROM build_samples "
          "WHERE pname = 'rio-sched-cgroup'\" | "
          "grep -qE '^[0-9]{7,}$'",
          timeout=60,
      )

      # CPU: the drv sleeps 3s specifically so the 1Hz poll fires
      # at least once. sleep uses negligible CPU but the poll
      # baseline captures daemon overhead; value > 0 proves the
      # poll → peak_cpu_cores → CompletionReport → write_build_sample
      # chain ran. Non-null is the real signal (completion.rs filters
      # 0 → None).
      ${gatewayHost}.wait_until_succeeds(
          "sudo -u postgres psql rio -qtAc "
          "\"SELECT peak_cpu_cores FROM build_samples "
          "WHERE pname = 'rio-sched-cgroup' "
          "AND peak_cpu_cores IS NOT NULL "
          "AND peak_cpu_cores > 0\" | "
          "grep -qE '^[0-9]'",
          timeout=60,
      )
''
