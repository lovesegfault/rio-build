  ...
  privileged-hardening-e2e =
    ...
    pkgs.testers.runNixOSTest {
      ...
      testScript = ''
        ...
        # already waited for rio-builders-0 Ready, which TRANSITIVELY
        ...
        # waitReady already proved rio-builders-0 condition=Ready.
        ...
        with subtest("nonpriv-admitted: privileged:false + device-plugin rendered and admitted"):
            pod_json = kubectl("get pod rio-builders-0 -o json", ns="${nsBuilders}")
        ...
        with subtest("cgroup-remount: /sys/fs/cgroup writable + /leaf created"):
            ...
            log = kubectl("logs rio-builders-0", ns="${nsBuilders}")
