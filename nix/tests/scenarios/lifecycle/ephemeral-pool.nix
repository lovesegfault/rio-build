...
scope: with scope; ''
  ...
  with subtest("ephemeral-pool: Job spawned, pod reaped, second build = new Job"):
      ...
      # fragment): pull pods never register with the scheduler, so the
      ...
      # mount + cgroup (~5s) + PullAssignment + build (mkTrivial ~1s)
      # + ReportOutcome + worker exit. ~30-40s typical; 120s margin.
      ...
      # their pods, but each pod gets a termination grace period.
      # reconcile_ephemeral may have spawned a second Job (job_count
      # ≤ 2 asserted above) whose pod is mid-pull when SIGTERM lands;
      # until it exits, a follow-on subtest's pods-gone precondition
      # (the pull-era equivalent of the old workers_active==0 wait)
      # is not sound. Every subtest that deletes a pool MUST wait its
      # own pods gone before the next subtest starts.
      #
      # 300s: pod grace + ownerRef-GC controller tick + margin.
      k3s_server.wait_until_succeeds(
