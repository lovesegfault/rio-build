...
pkgs.testers.runNixOSTest {
  ...
  testScript = ''
    ...
    # above (the dispatcher runs its EBS branch in QEMU). The
    # booted-node check below covers the tmpfiles half: the dirs the
    # mountd DaemonSet hostPath-mounts with type:Directory must exist
    # even on nodes where the instance-store branch did not run.
    with subtest("rio-kubelet-mount binds /var/rio onto the prjquota XFS"):
    ...
    with subtest("rio-ebs-mount provides prjquota /var/rio on EBS-only nodes"):
        ...
        assert "rio-kubelet-mount.service" in kubelet_after, (
            f"kubelet is not ordered After=rio-kubelet-mount: {kubelet_after}"
        )
