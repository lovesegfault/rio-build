...
pkgs.runCommand "cilium-rendered"
  ...
  ''
    ...
    ${pkgs.lib.optionalString gatewayEnabled ''
      ...
      # single source — see nix/pins.toml for the why-tlsroutes
      # rationale). infra/eks/addons.tf reads the SAME pin via tfvars,
      # and the gateway-api-excluded-crds check asserts both lanes still
      # consume it — so when the cilium 1.20 bump empties the list, the
      # VM gate and prod move together.
      yq 'select(${
        if pins.addons.gateway_api.excluded_crds == [ ] then
          "true"
        else
          pkgs.lib.concatMapStringsSep " and " (
            crd: ''.metadata.name != "${crd}"''
          ) pins.addons.gateway_api.excluded_crds
      })' \
