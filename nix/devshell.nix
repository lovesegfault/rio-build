...
let
  ...
  shellPackages = with pkgs; [
    ...
    # Parallel evaluator used by the build-replay campaign recorder
    # (`cargo xtask replay record`) for local/scoped archive recording;
    # the same pinned binary nix-fast-build and gen-matrix already use.
    nix-eval-jobs
