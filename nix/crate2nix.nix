...
let
  ...
  # callsite. The cache is COMMITTED (regenerated by `cargo xtask regen
  # sqlx`); a missing or empty .sqlx is always corruption, never a
  # legitimate state, so fail the EVAL loudly instead of staging an
  # empty fileset. (The previous `maybeMissing` tolerance silently
  # produced an empty source dir when a regen run swept the cache to
  # zero — every query!() crate then re-keyed to a derivation whose
  # offline resolution dangled and which could never build: sqlx fell
  # through to `$CARGO metadata` and panicked with `env var "CARGO"
  # must be set to use the query macros`, 66 checks red from one
  # purge.)
  sqlxCacheFileset =
    if !builtins.pathExists ../.sqlx || builtins.readDir ../.sqlx == { } then
      throw ".sqlx offline query cache is missing or empty — restore it (it is committed) or run `cargo xtask regen sqlx`; an empty cache makes every sqlx::query!() crate unbuildable"
    else
  ...
  # workspace Cargo.lock), so (3) panics if ever reached. The env var
  # is set DIRECTLY on the derivation — (1) then short-circuits before
  # any filesystem probing or `$CARGO` dependence. The `.env` shim is
  # kept as belt-and-braces for tools that scrub the environment but
  # honor dotenv (and it documents the path in the unpacked source).
  ...
  defaultCrateOverrides = pkgs.defaultCrateOverrides // {
    ...
    # rio-migrations: sqlx::migrate!() itself reads ./migrations from
    # the member fileset and needs no cache, and the crate has no
    # query!() callsites today — the override is prophylactic so the
    # first future query!() in this crate (or in its tests) cannot
    # reopen the offline-resolution hole.
    rio-migrations = _: sqlxOffline;
