...
pub fn build_node<D: DerivationLike>(drv_path: &str, drv: &D) -> types::DerivationNode {
    ...
    types::DerivationNode {
        ...
        // ADR-024: populated by populate_drv_digests() AFTER the
...
/// ADR-024: populate `drv_digest`/`input_drv_digests` on every
...
/// the legacy edges path, which the scheduler still accepts.
