...
impl StoreClients {
    /// Wrap the store, chunk, directory, and log clients over a single
    /// `Channel` with the standard max-message-size headroom (matches
    /// `connect_single`'s convention). One channel: all four RPC
    ...
    pub fn from_channel(ch: Channel) -> Self {
        ...
        Self {
            ...
            chunk: ChunkServiceClient::new(ch.clone())
                .max_decoding_message_size(max)
                .max_encoding_message_size(max),
            directory: DirectoryServiceClient::new(ch.clone())
                .max_decoding_message_size(max)
                .max_encoding_message_size(max),
