...
pub(super) async fn upload_output(
    ...
    tracing::info!(
        ...
        deriver = %deriver,
        "uploading output (streaming tee)"
    );
...
async fn do_upload_streaming(
    ...
    // blocking task produces them. `with_timeout_status` bounds the gRPC
    // call; a SEPARATE join-timeout below bounds the dump task — a
    // blocking thread parked in a sync read()/open() (FIFO, wedged FUSE)
    // never reaches blocking_send to observe rx-drop, and spawn_blocking
    // is non-abortable, so without it the await would hang forever.
    ...
    // Join the blocking task. Three cases:
    ...
    //   (b) put_path failed AND dump_task is in blocking_send → rx-drop
    //       makes blocking_send return Err; the task exits with
