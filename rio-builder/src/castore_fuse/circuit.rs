...
//! tokio primitives; no `.await`. Port of `fuse/circuit.rs` (which goes
//! away with the rest of the old FUSE at the P0560 cutover); the only
//! deltas are the metric name and the spec rule it implements.
//!
//! A breaker around the `open()` JIT-fetch path (`ReadBlob` /
//! `GetChunks`): on sustained rio-store unreachability the build fails
//! fast with one `EIO` instead of every `open()` timing out
//! individually for `jit_fetch_timeout` each.
//!
//! Two trip conditions — EITHER opens the circuit:
//!   (a) `threshold` (default 5) consecutive fetch failures
...
/// `CastoreFs`). All methods take `&self` — interior mutability
...
impl<C: Clock> CircuitBreaker<C> {
    ...
    pub fn check(&self) -> Result<(), Errno> {
            ...
            && self.consecutive_failures.load(Ordering::Relaxed) > 0
        {
            ...
            if guard.since.is_none() {
                ...
                metrics::gauge!("rio_builder_castore_fuse_circuit_open").set(1.0);
                tracing::warn!(
    ...
    ///
    /// Called immediately after a remote fetch attempt returns. Only
    /// the thread that owns the fetch (the `FillState` winner) records;
    /// a loser waiting on the winner's result does NOT — one fetch, one
    /// record.
    ///
    /// Pure sync — no `.await`.
    pub fn record(&self, ok: bool) {
        ...
        // leak the gate, but the fetch path between them (the ReadBlob
        // stream + blake3 verify) is network+fs I/O that returns Err,
        ...
        if ok {
            ...
            *self.last_success.lock().ignore_poison() = Some(now);
            if was_open {
                metrics::gauge!("rio_builder_castore_fuse_circuit_open").set(0.0);
