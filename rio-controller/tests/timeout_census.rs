...
fn scan_file(src: &str, rel: &str, out: &mut Vec<Site>) {
    ...
    // The production-region walk is the SHARED derivation
    // (census_universe::production_line_mask — extracted verbatim;
    // the cfgtest-green plant still pins it through this consumer).
    let prod = production_line_mask(&stripped);
