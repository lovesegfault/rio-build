...
pub(crate) mod tests {
    ...
    /// LOG_BUDGET` ≈ raw `disk_bytes` for tests that don't exercise the
    /// disk axis. Tests that DO care call `simulate` directly.
    ...
    /// log`), NOT raw `disk_bytes`. Node 25Gi free; 4 intents each
    /// `disk_bytes=8Gi headroom=1.5` → footprint = 8×1.5 + 1 = 13Gi
    /// each → only 1 fits. With raw `disk_bytes` FFD would pack 3
    /// (decrementing 8Gi each); kube-scheduler binds 1, the rest sit
    /// Pending with no covering NodeClaim.
    #[test]
    fn ffd_disk_uses_pod_footprint() {
        let n = node("n", "h", CapacityType::Spot, 32, 64 * GI, 25 * GI);
        ...
        // footprint = 8×1.5 + 1 (log) = 13Gi → ⌊25/13⌋ = 1.
        assert_eq!(
            p.len(),
            1,
            "footprint-based fit (was 3 with raw disk_bytes)"
        );
        assert_eq!(u.len(), 3);
    }
