...
/// fleet-wide core budget remaining and the per-tick cell cap.
/// [`sizing`] computes the same
...
pub fn sizing(cell: &Cell, u: &[&SpawnIntent], cfg: &SizingCfg) -> (Vec<(u32, u64, u64)>, f64) {
    ...
    let (sum_c, sum_m, sum_d, max_c, max_m, max_d) =
        fits.iter().map(|i| intent_pod_footprint(i)).fold(
...
mod tests {
    ...
    fn sizing_respects_budget_and_per_tick_cap() {
        ...
        // Σc=80, n_lo=⌈80/32⌉=3; sim_packs finds n_pack=5 (each pod's
        // footprint = (8c, 8Gi, 5×1.5+1 = 8.5Gi); 3 bins of
        // (27c, 26.7Gi, 28.3Gi) fit 3 each = 9 < 10, 4 bins fit 8,
        // 5 bins of (16c, 16Gi, 17Gi) fit 2 each = 10).
