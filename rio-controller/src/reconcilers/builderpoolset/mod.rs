...
mod tests {
    ...
    fn test_owned_wps(name: &str, ns: &str) -> BuilderPoolSet {
        let mut wps = test_wps_with_classes(&["small"]);
        wps.metadata.name = Some(name.into());
    ...
    fn is_wps_owned_by_matches_uid_not_just_kind() {
        let wps_a = test_owned_wps("wps-a", "rio");
        let wps_b = test_owned_wps("wps-b", "rio");

        // Child owned by wps-a (UID = "wps-a-uid" per test_owned_wps).
        ...
        let mut wps_no_uid = test_owned_wps("no-uid", "rio");
