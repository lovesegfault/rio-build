...
fn fingerprint_hit_and_invalidation() -> anyhow::Result<()> {
    ...
    let key = EvalStore::method_key(CaMethod::NixArchive, &[]);
    ...
    // A different method key must not hit.
    let other_key = EvalStore::method_key(CaMethod::Flat, &[]);
    assert_eq!(store.fingerprint_lookup(fs_path_str, &other_key)?, None);
