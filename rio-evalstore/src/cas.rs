...
/// A recorded `fingerprint → store path` mapping, scoped by the content-
/// address method and references that produced the path (the same file
/// added flat vs recursive yields different store paths).
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct FingerprintRecord {
    pub fingerprint: Fingerprint,
    /// Discriminates CA method + refs: `"<method>|ref1|ref2"`.
