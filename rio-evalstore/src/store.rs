...
impl EvalStore {
    ...
    fn resolve(&self, basename: &str, rel: &str) -> Result<Option<DagNode>> {
        if !is_store_basename(basename) {
            return Err(EvalStoreError::ForeignPath(format!(
                "{}/{basename}",
    ...
    pub fn read_directory(&self, basename: &str, rel: &str) -> Result<BTreeMap<String, DagNode>> {
        ...
        match self.resolve(basename, rel)? {
            ...
            None => Err(EvalStoreError::ForeignPath(format!(
                "{}/{basename}/{rel}",
                store_path::STORE_DIR
            ))),
        }
    ...
    /// Build the method discriminator for fingerprint records.
    pub fn method_key(ca_method: CaMethod, references: &[String]) -> String {
        let mut key = ca_method.as_str().to_string();
    ...
    pub fn fingerprint_lookup(&self, fs_path: &str, method_key: &str) -> Result<Option<String>> {
        ...
        if now == rec.fingerprint && self.cas.index_contains(basename) {
