...
/// `SubstitutePath` without tenant context MUST fail loudly: a
/// `NotFound` whose message names the cause ("no tenant context"), and
/// a `rio_store_substitute_skipped_total{reason=no_tenant}` increment.
///
/// This is the exact silent branch behind the 2026-05-23 incident: the
/// scheduler's detached substitute fetch got a bare `NotFound`,
/// concluded "the upstream HEAD probe lied", and demoted hundreds of
/// cache.nixos.org-cached paths (glibc included) to from-source builds
/// — with no way to tell a genuine upstream miss from a request that
/// never carried tenant identity. The status *message* is the only
/// channel that reaches the scheduler's demotion log, so it must be
/// diagnostic; the counter is the alertable signal.
///
/// Goes through `rio_proto::client::substitute_path_with_progress` (the
/// scheduler's actual call path) rather than the raw client, so this
/// also pins the wrapper's contract: a NotFound terminal propagates as
/// `Err(Status)` carrying the server's message — it is NOT swallowed
/// into a message-less `Ok(None)`.
#[tokio::test]
async fn substitute_path_without_tenant_is_loud() -> TestResult {
    use rio_test_support::metrics::CountingRecorder;

    let db = TestDb::new(&MIGRATOR).await;

    // Substituter is wired (so the failure is *specifically* the missing
    // tenant, not a missing substituter).
    let sub = Arc::new(Substituter::new(db.pool.clone(), None));
    let store_svc = StoreServiceImpl::new(db.pool.clone()).with_substituter(sub);

    // SwitchableTenant defaults to None → no `TenantClaims` extension →
    // `request_tenant_id` falls through to the (absent) service-token
    // path → no tenant. Same shape as a scheduler whose
    // `SubstituteAuth::Jwt(vec![])` sends no auth headers at all.
    let switch = SwitchableTenant::new();
    let router = Server::builder()
        .layer(tonic::service::InterceptorLayer::new(switch.interceptor()))
        .add_service(StoreServiceServer::new(store_svc));
    ...
    let path = test_store_path("subskip-no-tenant");
    let res = rio_proto::client::substitute_path_with_progress(
        &mut client,
        &path,
    ...
    let err = res.expect_err(
        "a tenant-less SubstitutePath must surface as Err(NotFound) so the \
         caller can log the cause — not as a message-less Ok(None)",
    );
    assert_eq!(err.code(), tonic::Code::NotFound, "status: {err:?}");
    assert!(
        err.message().contains("no tenant context"),
        "the NotFound message must name the cause so the scheduler's \
         demotion log is actionable; got: {:?}",
        err.message()
    );
    assert_eq!(
        rec.get("rio_store_substitute_skipped_total{reason=no_tenant}"),
        1,
        "a tenant-less substitution request must increment \
         rio_store_substitute_skipped_total{{reason=no_tenant}}; keys={:?}",
        rec.all_keys()
    );
