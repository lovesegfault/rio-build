...
pub async fn stage_chunked(
    ...
    // exists independent of any tokio runtime. The per-file loop here
    // is serial; the rayon fan-out is inside each chunk_nar call.
    let t = Instant::now();
...
async fn do_upload(
    ...
    // semaphore-charged `nar_data` (r[store.put.nar-bytes-budget+6]).
    //
