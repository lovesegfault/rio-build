...
impl StoreServiceImpl {
    ...
    ///   - `Ok(Some(guard))` → we own the placeholder (claimed
    ///     outright, or taken over after the concurrent uploader
    ///     aborted); the guard is pre-armed (sh-023,
    ///     `r[store.put.drop-cleanup+3]`). Proceed.
