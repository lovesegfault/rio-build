...
impl StoreServiceImpl {
    pub(super) async fn get_path_impl(
        ...
        // ADR-024 native submissions carry the derivation ONLY as a drv
        // blob (`PutDrvBlobs`) — there is no narinfo row for the `.drv`
        // path. Serve it from `drv_blobs` before answering NotFound so
        // the builder's `fetch_drv_from_store` (and any other GetPath
        // consumer of `.drv` content) works unchanged.
        macro_rules! not_found_or_drv_blob {
            () => {{
        ...
        let (info, vis) = match local {
            ...
            None => {
                match self
                    ...
                    .await?
                {
                    Some(sub) => (sub, sign::PathVisible::substituted_for_tenant()),
                    None => not_found_or_drv_blob!(),
                }
            }
        };
