...
//! (`HasChunks`) is tenant-scoped: the probe answers
...
impl ChunkServiceImpl {
    ...
    /// it resolves a full tenant (ADR-024 tenant-scoped presence,
    /// see `has_chunks`).
...
impl ChunkService for ChunkServiceImpl {
    ...
    ///
    /// Tenant-scoped (ADR-024, `r[store.chunk.has-chunks-tenant]`):
