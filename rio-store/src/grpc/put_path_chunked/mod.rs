...
use super::put_path::common::{IngestAuthority, PlaceholderGuard, PutAuth, drain_stream};
...
/// Claims for a tenant-JWT caller under an ARMED verifier (ADR-024
...
impl StoreServiceImpl {
    ...
    fn authorize_chunked<T>(&self, request: &Request<T>) -> Result<(PutAuth, bool), Status> {
        ...
        if self.hmac_verifier.is_some() && no_token_headers {
            ...
            if tenant_id.is_some() {
                return Ok((
                    PutAuth {
                        authority: IngestAuthority::TenantJwt,
                        tenant_id,
                    },
                    true,
                ));
    ...
    pub(super) async fn put_path_chunked_impl(
        ...
        let claims = match auth.builder_claims() {
            Some(c) => c.clone(),
            // r[impl store.put.chunked-jwt-source]
        ...
        // back to the SIGNED `claims.tenant` — the same derivation the
        ...
        for (i, out) in validated.outputs.iter().enumerate() {
                ...
                .await
            {
                ...
                Ok(PlaceholderClaim::Owned(g)) => {
                    // sh-023: the guard is pre-armed inside
                    // `claim_placeholder` — push it as-is, no
                    // `spawn_placeholder_guard` callsite.
                    output_claims.push(OutputClaim {
                        store_path_hash,
    ...
    async fn claim_ca_placeholders(
        ...
        for (i, out) in validated.outputs.iter().enumerate() {
                ...
                .await
            {
                ...
                Ok(PlaceholderClaim::Owned(g)) => {
                    // sh-023: pre-armed guard — no spawn callsite.
                    output_claims[i].claim = Some(g.claim_id());
                    guards.push(g);
                }
                Ok(PlaceholderClaim::Concurrent) => {
