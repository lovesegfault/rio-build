...
async fn sweep_one_batch(
    ...
    //
    // `dir_counts`/`file_digests` are the castore analogue: directory
    // refcounts decrement by N-referencing-paths; file digests are the
    // candidate set for `file_blob_tenants` orphan cleanup.
