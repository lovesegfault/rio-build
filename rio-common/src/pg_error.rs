...
mod tests {
    ...
    impl std::fmt::Display for FakeDbError {
        fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
            ...
            write!(f, "SQLSTATE {} ({})", self.0, self.message())
        }
    }
    impl std::error::Error for FakeDbError {}
    // The unused-by-the-classifier trait methods are required by
    // sqlx::error::DatabaseError but never invoked — keep them on
    // single lines so the dead surface is minimal.
    #[rustfmt::skip]
    ...
    /// The fakes must render their SQLSTATE: every classifier-test
    /// failure message formats these errors, and a fake that prints
    /// nothing useful turns a red classifier test into archaeology.
    #[test]
    fn fake_db_error_displays_its_sqlstate() {
        let rendered = db_err("57P03").to_string();
        assert!(rendered.contains("57P03"), "got: {rendered}");
    }
