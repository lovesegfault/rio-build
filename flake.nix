                  ...
                  pkgs.writeShellScript "sqlx-prepare-check" ''
                    ...
                    if git diff --cached --name-only -- '*.rs' \
                      ...
                      SQLX_OFFLINE=true cargo check --quiet -p rio-scheduler -p rio-store \
                        || { echo 'sqlx query cache stale — run `just sqlx-prepare`'; exit 1; }
                    fi
                  ''
                );
                files = "\\.rs$";
          ...
          devShells =
            let
              shellPackages = with pkgs; [
                ...
                postgresql_18
                sqlx-cli # `cargo sqlx prepare` (just sqlx-prepare) + `cargo sqlx migrate`
              ...
              mkRioShell =
                ...
                pkgs.mkShell (
                  ...
                  // {
                    ...
                    # `just sqlx-prepare` unsets this locally to regenerate.
                    SQLX_OFFLINE = "true";
