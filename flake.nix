        ...
        in
        {
          ...
          pre-commit = {
            ...
            settings.hooks = {
              ...
              # kubeconform hook removed: it downloads schemas from
              # raw.githubusercontent.com at runtime, which fails in
              # the nixbuild.net sandbox (no network). deploy.sh runs
              # `kubectl apply --dry-run=server` before apply — that
              # catches schema errors against the REAL apiserver, plus
              # admission-time validation that kubeconform can't see.
            };
          };
