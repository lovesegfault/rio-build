//! Binary-cache (substituter) access for `xtask k8s replay` — narinfo
//! probing and NAR fetching over HTTPS and S3.
//!
//! The replay engine asks caches two questions:
//!
//! - "do you have `<hash>`?" ([`Substituter::narinfo`]) — used to decide
//!   which store paths the replay target can substitute on its own and
...
/// [`Substituter::parse`] and shared by the supply planner and prewarm
    ...
    pub async fn parse(url: &str) -> Result<Self> {
        ...
        match parsed.scheme() {
            ...
            "s3" => {
                ...
                // `from_env()` provider chain otherwise.
